/**
 * CTW Leadership Skill Result Service
 * API calls for CTW Leadership Skill results management
 */

import apiClient from '@/libs/auth/api-client';
import { getToken } from '@/libs/auth/auth-token';
import type {
  CtwLeadershipSkillResult,
  CtwLeadershipSkillResultCreateData
...
export const ctwLeadershipSkillResultService = {
  ...
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwLeadershipSkillResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
  ...
  async createResult(data: CtwLeadershipSkillResultCreateData): Promise<CtwLeadershipSkillResult | null> {
    try {
      const token = getToken();
  ...
  async updateResult(id: number, data: Partial<CtwLeadershipSkillResultCreateData>): Promise<CtwLeadershipSkillResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwLeadershipSkillResultService;
