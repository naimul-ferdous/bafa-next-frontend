/**
 * CTW One Mile Result Service
 * API calls for CTW One Mile results management
 */
...
import type {
  CtwOneMileResult,
  CtwOneMileResultCreateData
} from '@/libs/types/ctwOneMile';
...
export const ctwOneMileResultService = {
  /**
   ...
   * Get single result
   */
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwOneMileResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
   ...
   * Create new result
   */
  async createResult(data: CtwOneMileResultCreateData): Promise<CtwOneMileResult | null> {
    try {
      const token = getToken();
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<CtwOneMileResultCreateData>): Promise<CtwOneMileResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwOneMileResultService;
