...
import { Ftw11SqnCounselingCadetApproval } from '@/libs/types/ftw11sqnCounselingCadetApproval';
...
interface ApprovalPaginatedResponse {
  data: Ftw11SqnCounselingCadetApproval[];
  current_page: number;
  per_page: number;
  total: number;
  last_page: number;
...
export const ftw11sqnCounselingCadetApprovalService = {
  getApprovals: async (params?: ApprovalQueryParams): Promise<ApprovalPaginatedResponse> => {
    try {
      ...
      const endpoint = `/ftw11sqn-counseling-cadet-approvals${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  store: async (data: Partial<Ftw11SqnCounselingCadetApproval>): Promise<Ftw11SqnCounselingCadetApproval | null> => {
    try {
      const token = getToken();
      const result = await apiClient.post<SingleApprovalApiResponse>('/ftw11sqn-counseling-cadet-approvals', data, token);
  ...
  update: async (id: number, data: Partial<Ftw11SqnCounselingCadetApproval>): Promise<Ftw11SqnCounselingCadetApproval | null> => {
    try {
      const token = getToken();
      const result = await apiClient.put<SingleApprovalApiResponse>(`/ftw11sqn-counseling-cadet-approvals/${id}`, data, token);
      return result?.data || null;
    } catch (error) {
  ...
  bulkApprove: async (payload: {
    ...
    try {
      ...
      const result = await apiClient.post<{ success: boolean; message: string }>(
        '/ftw11sqn-counseling-cadet-approvals/bulk',
        payload,
        token
      );
  ...
  destroy: async (id: number): Promise<boolean> => {
    try {
      const token = getToken();
      const result = await apiClient.delete<{ success: boolean }>(`/ftw11sqn-counseling-cadet-approvals/${id}`, token);
      return result?.success || false;
    } catch (error) {
...
export default ftw11sqnCounselingCadetApprovalService;
