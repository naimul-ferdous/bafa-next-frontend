...
/**
 * FTW 12SQN Flying Examination Approval Status Service
...
export const ftw12sqnFlyingExamApprovalStatusService = {
  /**
  ...
  async getAll(params?: QueryParams): Promise<PaginatedResponse> {
    try {
      ...
      const endpoint = `/ftw-12sqn-flying-exam-approval-status${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  async getById(id: number): Promise<ApprovalStatus | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleApiResponse>(`/ftw-12sqn-flying-exam-approval-status/${id}`, token);
  ...
  async getLatestByCourseAndSemester(courseId: number, semesterId: number): Promise<ApprovalStatus | null> {
    try {
      ...
      const endpoint = `/ftw-12sqn-flying-exam-approval-status/latest?${query.toString()}`;
      const token = getToken();
  ...
  async getByCourse(courseId: number, params?: QueryParams): Promise<PaginatedResponse> {
    try {
      ...
      const endpoint = `/ftw-12sqn-flying-exam-approval-status?${query.toString()}`;
      const token = getToken();
      const result = await apiClient.get<ApiResponse>(endpoint, token);
  ...
  async create(data: {
    ...
    try {
      ...
      const result = await apiClient.post<ActionApiResponse>('/ftw-12sqn-flying-exam-approval-status', data, token);
  ...
  async update(
    ...
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.put<ActionApiResponse>(`/ftw-12sqn-flying-exam-approval-status/${id}`, data, token);
  ...
  async delete(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<ActionApiResponse>(`/ftw-12sqn-flying-exam-approval-status/${id}`, token);
...
export default ftw12sqnFlyingExamApprovalStatusService;
