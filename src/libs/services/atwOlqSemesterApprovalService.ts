...
export const atwOlqSemesterApprovalService = {
  getApprovals: async (params?: SemesterApprovalQueryParams): Promise<SemesterApprovalPaginatedResponse> => {
    try {
      ...
      const endpoint = `/atw-olq-semester-approvals${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  store: async (data: Partial<AtwOlqSemesterApproval>): Promise<AtwOlqSemesterApproval | null> => {
    try {
      const token = getToken();
      const result = await apiClient.post<SingleSemesterApprovalApiResponse>('/atw-olq-semester-approvals', data, token);
  ...
  update: async (id: number, data: Partial<AtwOlqSemesterApproval>): Promise<AtwOlqSemesterApproval | null> => {
    try {
      const token = getToken();
      const result = await apiClient.put<SingleSemesterApprovalApiResponse>(`/atw-olq-semester-approvals/${id}`, data, token);
      return result?.data || null;
    } catch (error) {
  ...
  destroy: async (id: number): Promise<boolean> => {
    try {
      const token = getToken();
      const result = await apiClient.delete<{ success: boolean }>(`/atw-olq-semester-approvals/${id}`, token);
...
export default atwOlqSemesterApprovalService;
