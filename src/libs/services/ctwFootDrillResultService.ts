/**
 * CTW Foot Drill Result Service
 * API calls for CTW Foot Drill results management
 */
...
export const ctwFootDrillResultService = {
  /**
   ...
   * Get single result
   */
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwFootDrillResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
   ...
   * Create new result
   */
  async createResult(data: CtwFootDrillResultCreateData): Promise<CtwFootDrillResult | null> {
    try {
      const token = getToken();
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<CtwFootDrillResultCreateData>): Promise<CtwFootDrillResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwFootDrillResultService;
