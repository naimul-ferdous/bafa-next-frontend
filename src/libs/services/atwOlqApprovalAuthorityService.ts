...
export const atwOlqApprovalAuthorityService = {
  getAuthorities: async (params?: AuthorityQueryParams): Promise<AuthorityPaginatedResponse> => {
    try {
      ...
      const endpoint = `/atw-olq-approval-authorities${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  store: async (data: Partial<AtwOlqApprovalAuthority>): Promise<AtwOlqApprovalAuthority | null> => {
    try {
      const token = getToken();
      const result = await apiClient.post<SingleAuthorityApiResponse>('/atw-olq-approval-authorities', data, token);
  ...
  update: async (id: number, data: Partial<AtwOlqApprovalAuthority>): Promise<AtwOlqApprovalAuthority | null> => {
    try {
      const token = getToken();
      const result = await apiClient.put<SingleAuthorityApiResponse>(`/atw-olq-approval-authorities/${id}`, data, token);
      return result?.data || null;
    } catch (error) {
  ...
  destroy: async (id: number): Promise<boolean> => {
    try {
      const token = getToken();
      const result = await apiClient.delete<{ success: boolean }>(`/atw-olq-approval-authorities/${id}`, token);
...
export default atwOlqApprovalAuthorityService;
