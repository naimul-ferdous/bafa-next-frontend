...
/**
 * FTW 12SQN Flying Examination Cadet Approval Status Service
...
export const ftw12sqnFlyingExamCadetApprovalStatusService = {
  /**
  ...
  async getAll(params?: QueryParams): Promise<PaginatedResponse> {
    try {
      ...
      const endpoint = `/ftw-12sqn-flying-exam-cadet-approval-status${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  async getById(id: number): Promise<CadetApprovalStatus | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleApiResponse>(`/ftw-12sqn-flying-exam-cadet-approval-status/${id}`, token);
  ...
  async getByCourseAndSemester(
    ...
    try {
      ...
      const endpoint = `/ftw-12sqn-flying-exam-cadet-approval-status?${query.toString()}`;
      const token = getToken();
      const result = await apiClient.get<ApiResponse>(endpoint, token);
  ...
  async getApprovalSummary(
    ...
    try {
      ...
      const endpoint = `/ftw-12sqn-flying-exam-cadet-approval-status/summary?${query.toString()}`;
      const token = getToken();
  ...
  async create(data: {
    ...
    try {
      ...
      const result = await apiClient.post<ActionApiResponse>('/ftw-12sqn-flying-exam-cadet-approval-status', data, token);
  ...
  async update(
    ...
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.put<ActionApiResponse>(`/ftw-12sqn-flying-exam-cadet-approval-status/${id}`, data, token);
  ...
  async delete(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<ActionApiResponse>(`/ftw-12sqn-flying-exam-cadet-approval-status/${id}`, token);
  ...
  async bulkApprove(data: {
    ...
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.post<ActionApiResponse>('/ftw-12sqn-flying-exam-cadet-approval-status/bulk-approve', data, token);
  ...
  async bulkReject(data: {
    ...
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.post<ActionApiResponse>('/ftw-12sqn-flying-exam-cadet-approval-status/bulk-reject', data, token);
...
export default ftw12sqnFlyingExamCadetApprovalStatusService;
