/**
 * CTW Arms Drill Result Service
 * API calls for CTW Arms Drill results management
 */
...
export const ctwArmsDrillResultService = {
  /**
   ...
   * Get single result
   */
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwArmsDrillResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
   ...
   * Create new result
   */
  async createResult(data: CtwArmsDrillResultCreateData): Promise<CtwArmsDrillResult | null> {
    try {
      const token = getToken();
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<CtwArmsDrillResultCreateData>): Promise<CtwArmsDrillResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwArmsDrillResultService;
