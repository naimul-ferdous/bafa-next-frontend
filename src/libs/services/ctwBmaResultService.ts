/**
 * CTW BMA Result Service
 * API calls for CTW BMA results management
 */

import apiClient from '@/libs/auth/api-client';
import { getToken } from '@/libs/auth/auth-token';
import type { CtwBmaResult, CtwBmaResultCreateData } from '@/libs/types/ctwBma';
...
export const ctwBmaResultService = {
  ...
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwBmaResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
  ...
  async createResult(data: CtwBmaResultCreateData): Promise<CtwBmaResult | null> {
    try {
      const token = getToken();
  ...
  async updateResult(id: number, data: Partial<CtwBmaResultCreateData>): Promise<CtwBmaResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwBmaResultService;
