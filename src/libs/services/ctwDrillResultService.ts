/**
 * CTW  Drill Result Service
 * API calls for CTW  Drill results management
 */
...
import type {
  CtwDrillResult,
  CtwDrillResultCreateData
} from '@/libs/types/ctwDrill';
...
export const ctwDrillResultService = {
  /**
   ...
   * Get single result
   */
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwDrillResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
   ...
   * Create new result
   */
  async createResult(data: CtwDrillResultCreateData): Promise<CtwDrillResult | null> {
    try {
      const token = getToken();
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<CtwDrillResultCreateData>): Promise<CtwDrillResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwDrillResultService;
