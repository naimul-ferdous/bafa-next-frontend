...
import { AtwCounselingCadetApproval } from '@/libs/types/atwCounselingCadetApproval';
...
interface ApprovalPaginatedResponse {
  data: AtwCounselingCadetApproval[];
  current_page: number;
  per_page: number;
  total: number;
  last_page: number;
...
export const atwCounselingCadetApprovalService = {
  getApprovals: async (params?: ApprovalQueryParams): Promise<ApprovalPaginatedResponse> => {
    try {
      ...
      const endpoint = `/atw-counseling-cadet-approvals${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  store: async (data: Partial<AtwCounselingCadetApproval>): Promise<AtwCounselingCadetApproval | null> => {
    try {
      const token = getToken();
      const result = await apiClient.post<SingleApprovalApiResponse>('/atw-counseling-cadet-approvals', data, token);
  ...
  update: async (id: number, data: Partial<AtwCounselingCadetApproval>): Promise<AtwCounselingCadetApproval | null> => {
    try {
      const token = getToken();
      const result = await apiClient.put<SingleApprovalApiResponse>(`/atw-counseling-cadet-approvals/${id}`, data, token);
      return result?.data || null;
    } catch (error) {
  ...
  destroy: async (id: number): Promise<boolean> => {
    try {
      const token = getToken();
      const result = await apiClient.delete<{ success: boolean }>(`/atw-counseling-cadet-approvals/${id}`, token);
      return result?.success || false;
    } catch (error) {
...
export default atwCounselingCadetApprovalService;
