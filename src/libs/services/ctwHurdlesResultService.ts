/**
 * CTW Hurdles Result Service
 * API calls for CTW Hurdles results management
 */
...
import type {
  CtwHurdlesResult,
  CtwHurdlesResultCreateData
} from '@/libs/types/ctwHurdles';
...
export const ctwHurdlesResultService = {
  /**
   ...
   * Get single result
   */
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwHurdlesResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
   ...
   * Create new result
   */
  async createResult(data: CtwHurdlesResultCreateData): Promise<CtwHurdlesResult | null> {
    try {
      const token = getToken();
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<CtwHurdlesResultCreateData>): Promise<CtwHurdlesResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwHurdlesResultService;
