/**
 * CTW Cadet Warning Service
 * API calls for CTW cadet warning management
 */
...
export const ctwCadetWarningService = {
  async getAll(params?: WarningQueryParams): Promise<WarningPaginatedResponse> {
    try {
      ...
      const endpoint = `/ctw-cadet-warnings${query.toString() ? `?${query.toString()}` : ''}`;
      ...
      console.error('Failed to fetch CTW cadet warnings:', error);
      return { data: [], current_page: 1, per_page: 10, total: 0, last_page: 1, from: 0, to: 0 };
    }
  },
  ...
  async getById(id: number): Promise<CadetWarning | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleWarningApiResponse>(`/ctw-cadet-warnings/${id}`, token);
      if (!result || !result.success) return null;
      return result.data || null;
    } catch (error) {
      console.error(`Failed to fetch CTW cadet warning ${id}:`, error);
      return null;
    }
  },
  ...
  async getByCadet(cadetId: number): Promise<CadetWarning[]> {
    try {
      const token = getToken();
      const result = await apiClient.get<WarningApiResponse>(`/ctw-cadet-warnings/cadet/${cadetId}`, token);
      if (!result || !result.success) return [];
      return result.data || [];
    } catch (error) {
      console.error(`Failed to fetch CTW warnings for cadet ${cadetId}:`, error);
      return [];
    }
  },
  ...
  async create(data: WarningCreateData): Promise<CadetWarning | null> {
    try {
      ...
      const result = await apiClient.post<SingleWarningApiResponse>('/ctw-cadet-warnings', data, token);
      if (!result || !result.success) throw new Error(result?.message || 'Failed to create warning');
      return result.data || null;
    } catch (error: unknown) {
      console.error('Failed to create CTW cadet warning:', error);
      throw error;
    }
  },
  ...
  async update(id: number, data: Partial<WarningCreateData>): Promise<CadetWarning | null> {
    try {
      ...
      const result = await apiClient.put<SingleWarningApiResponse>(`/ctw-cadet-warnings/${id}`, data, token);
      if (!result || !result.success) throw new Error(result?.message || 'Failed to update warning');
      return result.data || null;
    } catch (error: unknown) {
      console.error(`Failed to update CTW cadet warning ${id}:`, error);
      throw error;
    }
  },
  ...
  async delete(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<{ success: boolean }>(`/ctw-cadet-warnings/${id}`, token);
      return result?.success || false;
    } catch (error) {
      console.error(`Failed to delete CTW cadet warning ${id}:`, error);
      return false;
    }
  },
  ...
  async getGroupedResults(params?: { course_id?: number; semester_id?: number; search?: string }): Promise<any[]> {
    try {
      ...
      const endpoint = `/ctw-cadet-warnings/grouped${query.toString() ? `?${query.toString()}` : ''}`;
      const token = getToken();
...
export default ctwCadetWarningService;
