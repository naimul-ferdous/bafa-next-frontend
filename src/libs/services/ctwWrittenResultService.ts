/**
 * CTW Written Result Service
 * API calls for CTW Written results management
 */
...
import type {
  CtwWrittenResult,
  CtwWrittenResultCreateData
} from '@/libs/types/ctwWritten';
...
export const ctwWrittenResultService = {
  /**
   ...
   * Get single result
   */
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwWrittenResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
   ...
   * Create new result
   */
  async createResult(data: CtwWrittenResultCreateData): Promise<CtwWrittenResult | null> {
    try {
      const token = getToken();
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<CtwWrittenResultCreateData>): Promise<CtwWrittenResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwWrittenResultService;
