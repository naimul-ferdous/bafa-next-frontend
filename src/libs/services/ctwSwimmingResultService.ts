/**
 * CTW Swimming Result Service
 * API calls for CTW Swimming results management
 */
...
import type {
  CtwSwimmingResult,
  CtwSwimmingResultCreateData
} from '@/libs/types/ctwSwimming';
...
export const ctwSwimmingResultService = {
  /**
  ...
  async getAllResults(ctwResultsModuleId: number, params?: ResultQueryParams): Promise<ResultPaginatedResponse> {
    try {
      ...
      console.error('Failed to fetch swimming results:', error);
      return { data: [], current_page: 1, per_page: 10, total: 0, last_page: 1, from: 0, to: 0 };
    }
  },
   ...
   * Get single result
   */
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwSwimmingResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
      const endpoint = `/ctw-results/${id}${query.toString() ? `?${query.toString()}` : ''}`;

      const token = getToken();
      const result = await apiClient.get<SingleResultApiResponse>(endpoint, token);
      return result?.data || null;
    } catch (error) {
      console.error(`Failed to fetch swimming result ${id}:`, error);
      return null;
    }
  },
   ...
   * Create new result
   */
  async createResult(data: CtwSwimmingResultCreateData): Promise<CtwSwimmingResult | null> {
    try {
      const token = getToken();
      ...
      console.error('Failed to create swimming result:', error);
      throw error;
    }
  },
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<CtwSwimmingResultCreateData>): Promise<CtwSwimmingResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      ...
      console.error(`Failed to update swimming result ${id}:`, error);
      throw error;
    }
  },
  ...
  async deleteResult(id: number): Promise<boolean> {
    try {
      ...
      console.error(`Failed to delete swimming result ${id}:`, error);
      return false;
    }
  },
  ...
  async getGroupedResults(params?: { course_id?: number; semester_id?: number; ctw_results_module_id?: number; search?: string }): Promise<any[]> {
    try {
      ...
      console.error('Failed to fetch grouped swimming results:', error);
      return [];
    }
  },
...
export default ctwSwimmingResultService;
