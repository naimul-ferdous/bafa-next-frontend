...
interface ResultPaginatedResponse {
  data: CtwAssessmentCounselingResult[];
  current_page: number;
  last_page: number;
  per_page: number;
  total: number;
...
export const ctwAssessmentCounselingResultService = {
  /**
  ...
  async getAllResults(params?: ResultQueryParams): Promise<ResultPaginatedResponse> {
    try {
      ...
      const endpoint = `/ctw-assessment-counseling-results${query.toString() ? `?${query.toString()}` : ''}`;
   ...
   * Get single result
   */
  async getResult(id: number): Promise<CtwAssessmentCounselingResult | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleResultApiResponse>(`/ctw-assessment-counseling-results/${id}`, token);
   ...
   * Create new result
   */
  async createResult(data: ResultFormData): Promise<CtwAssessmentCounselingResult | null> {
    try {
      const token = getToken();
      ...
      const result = await apiClient.post<ResultActionApiResponse>('/ctw-assessment-counseling-results', data, token);
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<ResultFormData>): Promise<CtwAssessmentCounselingResult | null> {
    try {
      const token = getToken();
      ...
      const result = await apiClient.put<ResultActionApiResponse>(`/ctw-assessment-counseling-results/${id}`, data, token);
  ...
  async deleteResult(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<ResultActionApiResponse>(`/ctw-assessment-counseling-results/${id}`, token);
  ...
  async getGroupedResults(params?: { instructor_id?: number; authority_id?: number; course_id?: number; semester_id?: number; page?: number; per_page?: number; search?: string; allData?: boolean }): Promise<ResultPaginatedResponse & { data: any[]; semester_approvals: any[]; authorities: any[]; my_authority: any; has_counseling_assign: boolean }> {
    try {
      ...
      const endpoint = `/ctw-assessment-counseling-results/grouped${query.toString() ? `?${query.toString()}` : ''}`;
      const token = getToken();
  ...
  async getConsolidatedResults(params: { course_id: number; semester_id: number }): Promise<{
    ...
    try {
      ...
      const token = getToken();
      const res = await apiClient.get<any>(`/ctw-assessment-counseling-results/consolidated?${query.toString()}`, token);
  ...
  async getFormOptions(params?: { course_id?: number; semester_id?: number }): Promise<{
    ...
    try {
      ...
      const token = getToken();
      const res = await apiClient.get<any>(`/ctw-assessment-counseling-results/form-options?${query.toString()}`, token);
      return res?.data || null;
    } catch (error) {
...
export default ctwAssessmentCounselingResultService;
