...
interface ResultPaginatedResponse {
  data: Ftw12SqnAssessmentPenpictureResult[];
  current_page: number;
  per_page: number;
  total: number;
  last_page: number;
...
export const ftw12sqnAssessmentPenpictureResultService = {
  /**
  ...
  async getAllResults(params?: ResultQueryParams): Promise<ResultPaginatedResponse> {
    try {
      ...
      const endpoint = `/ftw12sqn-assessment-penpicture-results${query.toString() ? `?${query.toString()}` : ''}`;
   ...
   */
  async getResultsByCourse(courseId: number): Promise<Ftw12SqnAssessmentPenpictureResult[]> {
    try {
      const token = getToken();
      const result = await apiClient.get<ResultApiResponse>(`/ftw12sqn-assessment-penpicture-results/course/${courseId}`, token);
   ...
   */
  async getResultsByCadet(cadetId: number): Promise<Ftw12SqnAssessmentPenpictureResult[]> {
    try {
      const token = getToken();
      const result = await apiClient.get<ResultApiResponse>(`/ftw12sqn-assessment-penpicture-results/cadet/${cadetId}`, token);
   ...
   */
  async getResultsByInstructor(instructorId: number): Promise<Ftw12SqnAssessmentPenpictureResult[]> {
    try {
      const token = getToken();
      const result = await apiClient.get<ResultApiResponse>(`/ftw12sqn-assessment-penpicture-results/instructor/${instructorId}`, token);
   ...
   * Get single result
   */
  async getResult(id: number): Promise<Ftw12SqnAssessmentPenpictureResult | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleResultApiResponse>(`/ftw12sqn-assessment-penpicture-results/${id}`, token);
   ...
   * Create new result
   */
  async createResult(data: ResultCreateData): Promise<Ftw12SqnAssessmentPenpictureResult | null> {
    try {
      const token = getToken();
      ...
      const result = await apiClient.post<ResultActionApiResponse>('/ftw12sqn-assessment-penpicture-results', data, token);
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<ResultCreateData>): Promise<Ftw12SqnAssessmentPenpictureResult | null> {
    try {
      const token = getToken();
      ...
      const result = await apiClient.put<ResultActionApiResponse>(`/ftw12sqn-assessment-penpicture-results/${id}`, data, token);
  ...
  async deleteResult(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<ResultActionApiResponse>(`/ftw12sqn-assessment-penpicture-results/${id}`, token);
  ...
  async bulkCreate(results: any[]): Promise<any> {
    ...
    const result = await apiClient.post<any>('/ftw12sqn-assessment-penpicture-results/bulk', { results }, token);
  ...
  async getGroupedResults(params?: { instructor_id?: number; page?: number; per_page?: number; search?: string }): Promise<ResultPaginatedResponse & { data: any[] }> {
    try {
      ...
      const endpoint = `/ftw12sqn-assessment-penpicture-results/grouped${query.toString() ? `?${query.toString()}` : ''}`;
      const token = getToken();
...
export default ftw12sqnAssessmentPenpictureResultService;
