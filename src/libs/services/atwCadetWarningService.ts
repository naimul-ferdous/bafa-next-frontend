...
export const atwCadetWarningService = {
  async getAll(params?: WarningQueryParams): Promise<WarningPaginatedResponse> {
    try {
      ...
      const endpoint = `/atw-cadet-warnings${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  async getById(id: number): Promise<CadetWarning | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleWarningApiResponse>(`/atw-cadet-warnings/${id}`, token);
  ...
  async getByCadet(cadetId: number): Promise<CadetWarning[]> {
    try {
      const token = getToken();
      const result = await apiClient.get<WarningApiResponse>(`/atw-cadet-warnings/cadet/${cadetId}`, token);
  ...
  async create(data: WarningCreateData): Promise<CadetWarning | null> {
    try {
      ...
      const result = await apiClient.post<SingleWarningApiResponse>('/atw-cadet-warnings', data, token);
  ...
  async update(id: number, data: Partial<WarningCreateData>): Promise<CadetWarning | null> {
    try {
      ...
      const result = await apiClient.put<SingleWarningApiResponse>(`/atw-cadet-warnings/${id}`, data, token);
  ...
  async delete(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<{ success: boolean }>(`/atw-cadet-warnings/${id}`, token);
...
export default atwCadetWarningService;
