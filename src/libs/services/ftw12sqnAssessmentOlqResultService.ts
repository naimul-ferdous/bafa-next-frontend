/**
 * FTW 12sqn Assessment OLQ Result Service
 * API calls for FTW 12sqn Assessment OLQ result management
 */
...
export const ftw12sqnAssessmentOlqResultService = {
  ...
  async updateCadetMark(resultId: number, cadetId: number, markId: number, data: { achieved_mark: number; is_active?: boolean }): Promise<Ftw12sqnAssessmentOlqResultMark | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found.');
  ...
  async saveCadetMarks(resultId: number, cadetId: number, marks: Ftw12sqnAssessmentOlqResultMarkCreateData[]): Promise<Ftw12sqnAssessmentOlqResultCadet | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found.');
      const result = await apiClient.post<SingleCadetApiResponse>(`/ftw-12sqn-assessment-olq-results/${resultId}/cadets/${cadetId}/marks/save`, { marks }, token);
