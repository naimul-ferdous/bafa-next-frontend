...
export const ftw12sqnGroundSyllabusService = {
  ...
  async getList(): Promise<Ftw12sqnGroundSyllabus[]> {
    try {
      const endpoint = `/ftw-12sqn-ground-syllabus/list`;
      const token = getToken();
      const result = await apiClient.get<{ success: boolean; data: Ftw12sqnGroundSyllabus[] }>(endpoint, token);
      return result?.data || [];
    } catch (error) {
  ...
  async get(id: number): Promise<Ftw12sqnGroundSyllabus | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleApiResponse>(`/ftw-12sqn-ground-syllabus/${id}`, token);
  ...
  async create(data: Ftw12sqnGroundSyllabusCreateData): Promise<Ftw12sqnGroundSyllabus | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found.');
      const result = await apiClient.post<ActionApiResponse>('/ftw-12sqn-ground-syllabus', data, token);
  ...
  async update(id: number, data: Partial<Ftw12sqnGroundSyllabusCreateData>): Promise<Ftw12sqnGroundSyllabus | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found.');
      const result = await apiClient.put<ActionApiResponse>(`/ftw-12sqn-ground-syllabus/${id}`, data, token);
  ...
  async delete(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<ActionApiResponse>(`/ftw-12sqn-ground-syllabus/${id}`, token);
