/**
 * FTW 11sqn Assessment Counseling Result Service
...
export const ftw11sqnAssessmentCounselingResultService = {
  ...
  async getAllResults(params?: ResultQueryParams): Promise<ResultPaginatedResponse> {
    try {
      ...
      const endpoint = `/ftw-11sqn-assessment-counseling-results${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  async getResult(id: number): Promise<Ftw11sqnAssessmentCounselingResult | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleResultApiResponse>(`/ftw-11sqn-assessment-counseling-results/${id}`, token);
  ...
  async createResult(data: ResultFormData): Promise<Ftw11sqnAssessmentCounselingResult | null> {
    try {
      ...
      const result = await apiClient.post<ResultActionApiResponse>('/ftw-11sqn-assessment-counseling-results', data, token);
  ...
  async updateResult(id: number, data: Partial<ResultFormData>): Promise<Ftw11sqnAssessmentCounselingResult | null> {
    try {
      ...
      const result = await apiClient.put<ResultActionApiResponse>(`/ftw-11sqn-assessment-counseling-results/${id}`, data, token);
  ...
  async deleteResult(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<ResultActionApiResponse>(`/ftw-11sqn-assessment-counseling-results/${id}`, token);
  ...
  async getGroupedResults(params?: {
    ...
    try {
      ...
      const endpoint = `/ftw-11sqn-assessment-counseling-results/grouped${query.toString() ? `?${query.toString()}` : ''}`;
      const token = getToken();
      const result = await apiClient.get<ResultApiResponse>(endpoint, token);
