/**
 * CTW PT Exam Result Service
 * API calls for CTW PT Exam results management
 */
...
import type {
  CtwPtExamResult,
  CtwPtExamResultCreateData
} from '@/libs/types/ctwPtExam';
...
export const ctwPtExamResultService = {
  /**
   ...
   * Get single result
   */
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwPtExamResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
   ...
   * Create new result
   */
  async createResult(data: CtwPtExamResultCreateData): Promise<CtwPtExamResult | null> {
    try {
      const token = getToken();
   ...
   * Update result
   */
  async updateResult(id: number, data: Partial<CtwPtExamResultCreateData>): Promise<CtwPtExamResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwPtExamResultService;
