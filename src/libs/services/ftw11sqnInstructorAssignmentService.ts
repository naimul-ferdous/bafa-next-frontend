/**
 * FTW 11SQN Instructor Assignment Service (Unified)
...
export interface Ftw11sqnAssignment {
  id: number;
...
export const ftw11sqnInstructorAssignmentService = {
  /**
   * Fetch assignments with nested exercises and cadets.
   */
  async getAssignments(params: QueryParams): Promise<Ftw11sqnAssignment[]> {
    try {
      ...
      const result = await apiClient.get<{ success: boolean; data: Ftw11sqnAssignment[] }>(
  ...
  async syncByCadet(payload: CadetSyncPayload): Promise<boolean> {
    ...
    if (!token) throw new Error('Authentication token not found.');
    const result = await apiClient.post<{ success: boolean; message: string }>(
      '/ftw-11sqn-instructor-assign-mission/sync-by-cadet',
      payload,
      token
    );
    return result?.success || false;
...
export default ftw11sqnInstructorAssignmentService;
