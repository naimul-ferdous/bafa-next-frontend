...
/**
 * FTW 12SQN Ground Examination Mark Service
 * API calls for FTW 12SQN Ground Examination Mark management
 */

import apiClient from '@/libs/auth/api-client';
import { getToken } from '@/libs/auth/auth-token';
import type {
  Ftw12sqnGroundExaminationMark,
  Ftw12sqnGroundExaminationMarkCreateData,
  CourseGroupedData
...
interface PaginatedResponse {
  data: Ftw12sqnGroundExaminationMark[];
  current_page: number;
  per_page: number;
  total: number;
  last_page: number;
...
interface ActionApiResponse {
  success: boolean;
  message: string;
  data?: Ftw12sqnGroundExaminationMark | Ftw12sqnGroundExaminationMark[];
}
...
export const ftw12sqnGroundExaminationMarkService = {
  /**
  ...
  async getAllMarks(params?: QueryParams): Promise<PaginatedResponse> {
    try {
      ...
      const endpoint = `/ftw-12sqn-ground-examination-marks${query.toString() ? `?${query.toString()}` : ''}`;
   ...
   */
  async getMark(id: number): Promise<Ftw12sqnGroundExaminationMark | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleApiResponse>(`/ftw-12sqn-ground-examination-marks/${id}`, token);
   ...
   */
  async createMark(data: any): Promise<Ftw12sqnGroundExaminationMark | Ftw12sqnGroundExaminationMark[] | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.post<ActionApiResponse>('/ftw-12sqn-ground-examination-marks', data, token);
   ...
   */
  async createBulkMark(data: any): Promise<Ftw12sqnGroundExaminationMark[] | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.post<ActionApiResponse>('/ftw-12sqn-ground-examination-marks/bulk', data, token);
      if (!result || !result.success) throw new Error(result?.message || 'Failed to create examination marks');
      return (result.data as Ftw12sqnGroundExaminationMark[]) || null;
    } catch (error) {
   ...
   * Update single mark
   */
  async updateMark(id: number, data: Partial<Ftw12sqnGroundExaminationMarkCreateData>): Promise<Ftw12sqnGroundExaminationMark | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.put<ActionApiResponse>(`/ftw-12sqn-ground-examination-marks/${id}`, data, token);
   ...
   * Bulk update marks
   */
  async updateBulkMarks(data: { marks: Array<{ id: number; [key: string]: any }> }): Promise<Ftw12sqnGroundExaminationMark[] | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.put<ActionApiResponse>(`/ftw-12sqn-ground-examination-marks/bulk`, data, token);
  ...
  async deleteMark(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<ActionApiResponse>(`/ftw-12sqn-ground-examination-marks/${id}`, token);
  ...
  async getMarksBySemester(params: {
    ...
    try {
      ...
      if (params.semesterId) {
        ...
        const endpoint = `/ftw-12sqn-ground-examination-marks/semester/${params.semesterId}${query.toString() ? `?${query.toString()}` : ''}`;
        const token = getToken();
        const result = await apiClient.get<ApiResponse>(endpoint, token);
  ...
  async getMarksByCadet(params: {
    ...
    semester_id?: number;
    exam_type_id?: number;
  }): Promise<Ftw12sqnGroundExaminationMark[]> {
    try {
      const query = new URLSearchParams();
      if (params.semester_id) query.append('semester_id', params.semester_id.toString());
      if (params.exam_type_id) query.append('exam_type_id', params.exam_type_id.toString());

      const endpoint = `/ftw-12sqn-ground-examination-marks/cadet/${params.cadetId}${query.toString() ? `?${query.toString()}` : ''}`;
      const token = getToken();
      const result = await apiClient.get<ApiResponse>(endpoint, token);
      return result?.data || [];
    } catch (error) {
  ...
  async getSemesterGrouped(params?: {
    ...
    try {
      ...
      const endpoint = `/ftw-12sqn-ground-examination-marks/grouped/semester${query.toString() ? `?${query.toString()}` : ''}`;
      const token = getToken();
