...
/**
 * FTW 12SQN Results BUP Adjust Mark Grading Service
 */

import apiClient from '@/libs/auth/api-client';
import { getToken } from '@/libs/auth/auth-token';

export interface Ftw12sqnResultsBupAdjustMarkGrading {
  id: number;
...
export interface Ftw12sqnResultsBupAdjustMarkGradingPayload {
...
interface PaginatedResponse {
  data: Ftw12sqnResultsBupAdjustMarkGrading[];
  current_page: number;
  per_page: number;
  total: number;
  last_page: number;
...
const BASE = '/ftw-12sqn-results-bup-adjust-mark-grading';

export const ftw12sqnResultsBupAdjustMarkGradingService = {
  ...
  async getById(id: number): Promise<Ftw12sqnResultsBupAdjustMarkGrading | null> {
    try {
      const token = getToken();
  ...
  async create(data: Ftw12sqnResultsBupAdjustMarkGradingPayload): Promise<Ftw12sqnResultsBupAdjustMarkGrading | null> {
    try {
      const token = getToken();
  ...
  async update(id: number, data: Ftw12sqnResultsBupAdjustMarkGradingPayload): Promise<Ftw12sqnResultsBupAdjustMarkGrading | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ftw12sqnResultsBupAdjustMarkGradingService;
