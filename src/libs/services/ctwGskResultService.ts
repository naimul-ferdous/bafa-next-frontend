/**
 * CTW GSK Result Service
 * API calls for CTW GSK results management
 */
...
import type {
  CtwGskResult,
  CtwGskResultCreateData
} from '@/libs/types/ctwGsk';
...
export const ctwGskResultService = {
  ...
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwGskResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
  ...
  async createResult(data: CtwGskResultCreateData): Promise<CtwGskResult | null> {
    try {
      const token = getToken();
  ...
  async updateResult(id: number, data: Partial<CtwGskResultCreateData>): Promise<CtwGskResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwGskResultService;
