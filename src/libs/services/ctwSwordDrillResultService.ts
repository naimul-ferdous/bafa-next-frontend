/**
 * CTW Sword Drill Result Service
 * API calls for CTW Sword Drill results management
 */
...
export const ctwSwordDrillResultService = {
  ...
  async getResult(ctwResultsModuleId: number, id: number): Promise<CtwSwordDrillResult | null> {
    try {
      const query = new URLSearchParams();
      query.append('ctw_results_module_id', ctwResultsModuleId.toString());
  ...
  async createResult(data: CtwSwordDrillResultCreateData): Promise<CtwSwordDrillResult | null> {
    try {
      const token = getToken();
  ...
  async updateResult(id: number, data: Partial<CtwSwordDrillResultCreateData>): Promise<CtwSwordDrillResult | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
...
export default ctwSwordDrillResultService;
