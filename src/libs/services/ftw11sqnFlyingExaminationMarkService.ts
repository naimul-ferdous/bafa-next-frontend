...
export const ftw11sqnFlyingExaminationMarkService = {
   ...
   */
  async createBulkMark(data: { marks: any[] }): Promise<Ftw11sqnFlyingExaminationMark[]> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.post<any>('/ftw-11sqn-flying-examination-marks/bulk', data, token);
      if (!result || result.status === 422 || result.status === 500) throw new Error(result?.error || result?.message || 'Failed to create bulk marks');
      return (result.data as Ftw11sqnFlyingExaminationMark[]) || [];
    } catch (error) {
   ...
   * Update a mark by ID
   */
  async updateMark(id: number, data: Partial<Ftw11sqnFlyingExaminationMark>): Promise<Ftw11sqnFlyingExaminationMark | null> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.put<any>(`/ftw-11sqn-flying-examination-marks/${id}`, data, token);
  ...
  async deleteMark(id: number): Promise<boolean> {
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.delete<any>(`/ftw-11sqn-flying-examination-marks/${id}`, token);
  ...
  async getPanelData(params: { course_id: number; semester_id: number }): Promise<{
    ...
    try {
      ...
      const result = await apiClient.get<any>(`/ftw-11sqn-flying-examination-marks/panel-data?${query.toString()}`, token);
  ...
  async getReportData(params: { course_id: number; semester_id: number }): Promise<{
    ...
    try {
      ...
      const result = await apiClient.get<any>(`/ftw-11sqn-flying-examination-marks/semester/${params.semester_id}/report-data?${query.toString()}`, token);
      return result?.data;
    } catch (error) {
  ...
  async checkExistingMark(data: {
    ...
    try {
      ...
      const result = await apiClient.get<any>(
        `/ftw-11sqn-flying-examination-marks/check?ftw_11sqn_flying_syllabus_id=${data.ftw_11sqn_flying_syllabus_id}&ftw_11sqn_flying_syllabus_exercise_id=${data.ftw_11sqn_flying_syllabus_exercise_id}&cadet_id=${data.cadet_id}`,
        token
      );
  ...
  async getCadetMarks(params: {
    ...
    semester_id?: number;
    exam_type_id?: number;
  }): Promise<{ marks: Ftw11sqnFlyingExaminationMark[] }> {
    try {
      const query = new URLSearchParams();
      if (params.course_id) query.append('course_id', params.course_id.toString());
      if (params.semester_id) query.append('semester_id', params.semester_id.toString());
      ...
      const endpoint = `/ftw-11sqn-flying-examination-marks/cadet/${params.cadet_id}${query.toString() ? `?${query.toString()}` : ''}`;
      const token = getToken();
      const result = await apiClient.get<ApiResponse>(endpoint, token);
  ...
  async getFormData(params: {
    ...
    try {
      ...
      const endpoint = `/ftw-11sqn-flying-result-form${query.toString() ? `?${query.toString()}` : ''}`;
      const token = getToken();
  ...
  async createAdditionalMark(data: {
    ...
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.post<any>('/ftw-11sqn-flying-examination-marks-additional', data, token);
  ...
  async updateAdditionalMark(id: number, data: Partial<{
    ...
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.put<any>(`/ftw-11sqn-flying-examination-marks-additional/${id}`, data, token);
