...
/**
 * FTW 12SQN Flying Exam Result Approval Process Service
...
export const ftw12sqnFlyingExamApprovalProcessService = {
  /**
  ...
  async getAll(params?: QueryParams): Promise<PaginatedResponse> {
    try {
      ...
      const endpoint = `/ftw-12sqn-flying-exam-approval-process${query.toString() ? `?${query.toString()}` : ''}`;
  ...
  async getActive(): Promise<ApprovalProcess[]> {
    try {
      const token = getToken();
      const result = await apiClient.get<ApiResponse>('/ftw-12sqn-flying-exam-approval-process/active', token);
      return result?.data || [];
    } catch (error) {
  ...
  async getById(id: number): Promise<ApprovalProcess | null> {
    try {
      const token = getToken();
      const result = await apiClient.get<SingleApiResponse>(`/ftw-12sqn-flying-exam-approval-process/${id}`, token);
  ...
  async getByRoleId(roleId: number, params?: QueryParams): Promise<PaginatedResponse> {
    try {
      ...
      const endpoint = `/ftw-12sqn-flying-exam-approval-process?${query.toString()}`;
      const token = getToken();
      const result = await apiClient.get<ApiResponse>(endpoint, token);
  ...
  async create(data: {
    ...
    try {
      ...
      const result = await apiClient.post<ActionApiResponse>('/ftw-12sqn-flying-exam-approval-process', data, token);
  ...
  async update(
    ...
    try {
      const token = getToken();
      if (!token) throw new Error('Authentication token not found. Please login again.');
      const result = await apiClient.put<ActionApiResponse>(`/ftw-12sqn-flying-exam-approval-process/${id}`, data, token);
  ...
  async delete(id: number): Promise<boolean> {
    try {
      const token = getToken();
      const result = await apiClient.delete<ActionApiResponse>(`/ftw-12sqn-flying-exam-approval-process/${id}`, token);
...
export default ftw12sqnFlyingExamApprovalProcessService;
