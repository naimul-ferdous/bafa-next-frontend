...
import type { Ftw11SqnAssessmentOlqResult, Ftw11SqnAssessmentOlqResultCreateData, Ftw11SqnAssessmentOlqType, Ftw11SqnAssessmentOlqTypeEstimatedMark } from "@/libs/types/ftw11sqnAssessmentOlq";
...
import { Ftw11SqnOlqAssign } from "@/libs/types/ftw11sqnAssign";

interface OlqResultFormProps {
  initialData?: Ftw11SqnAssessmentOlqResult | null;
  onSubmit: (data: Ftw11SqnAssessmentOlqResultCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}
...
export default function OlqResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: OlqResultFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    semester_id: 0,
    exam_type_id: 0,
    ftw_11sqn_assessment_olq_type_id: 0,
    remarks: "",
    is_active: true,
  });
  ...
  const [olqAssignments, setOlqAssignments] = useState<Ftw11SqnOlqAssign[]>([]);
  const [allOlqTypes, setAllOlqTypes] = useState<Ftw11SqnAssessmentOlqType[]>([]);
  const [filteredOlqTypes, setFilteredOlqTypes] = useState<Ftw11SqnAssessmentOlqType[]>([]);

  const courses = useMemo(() => {
    if (isInstructor) {
      // Instructors: show only their assigned courses
      const assignedIds = new Set(olqAssignments.map((a: any) => a.course_id));
  ...
  const [selectedOlqType, setSelectedOlqType] = useState<Ftw11SqnAssessmentOlqType | null>(null);
  const [estimatedMarks, setEstimatedMarks] = useState<Ftw11SqnAssessmentOlqTypeEstimatedMark[]>([]);
    ...
    const loadDropdownData = async () => {
      try {
        ...
        const [commonData, olqTypesRes, assignsData] = await Promise.all([
          ...
          ftw11sqnAssessmentOlqTypeService.getAllTypes({ per_page: 1000 }),
  ...
  useEffect(() => {
    ...
    if (formData.course_id) {
      const hasAnyAssignments = allOlqTypes.some(t => (t.assignments?.length ?? 0) > 0);
    ...
    if (filtered.length > 0 && !isEdit) {
      const current = filtered.find(t => t.id === formData.ftw_11sqn_assessment_olq_type_id);
      if (!current) {
        const autoSelect = filtered.find(t => t.is_active) || filtered[0];
        setFormData(prev => ({ ...prev, ftw_11sqn_assessment_olq_type_id: autoSelect.id }));
      }
    } else if (filtered.length === 0) {
      setFormData(prev => ({ ...prev, ftw_11sqn_assessment_olq_type_id: 0 }));
    }
  }, [formData.course_id, formData.semester_id, allOlqTypes, isEdit]);

  // Update selected OLQ type and estimated marks when type changes
  useEffect(() => {
    if (formData.ftw_11sqn_assessment_olq_type_id) {
      const type = allOlqTypes.find(t => t.id === formData.ftw_11sqn_assessment_olq_type_id);
  ...
  }, [formData.ftw_11sqn_assessment_olq_type_id, allOlqTypes]);
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        ftw_11sqn_assessment_olq_type_id: initialData.ftw_11sqn_assessment_olq_type_id,
      ...
      if (initialData.result_cadets && initialData.result_cadets.length > 0) {
        const rows: CadetRow[] = initialData.result_cadets.map(c => {
          ...
          c.marks?.forEach(m => {
            marks[m.ftw_11sqn_assessment_olq_type_estimated_mark_id] = parseFloat(String(m.achieved_mark));
          });
  ...
  const handleChange = (field: string, value: any) => {
    if (field === "course_id") {
      setFormData(prev => ({ ...prev, course_id: value, semester_id: 0, ftw_11sqn_assessment_olq_type_id: 0 }));
      return;
    }
    if (field === "semester_id") {
      setFormData(prev => ({ ...prev, semester_id: value, ftw_11sqn_assessment_olq_type_id: 0 }));
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.ftw_11sqn_assessment_olq_type_id) { setError("Please select an OLQ type"); return; }

    try {
      const submitData: Ftw11SqnAssessmentOlqResultCreateData = {
        course_id: formData.course_id,
        semester_id: formData.semester_id,
        ftw_11sqn_assessment_olq_type_id: formData.ftw_11sqn_assessment_olq_type_id,
        ...
        cadets: cadetRows.filter(c => c.cadet_id > 0).map(c => ({
          ...
          marks: Object.entries(c.marks).map(([estimatedMarkId, achievedMark]) => ({
            ftw_11sqn_assessment_olq_type_estimated_mark_id: parseInt(estimatedMarkId),
  ...
  const filtersSelected = isInstructor
    ...
    : formData.course_id && formData.semester_id;
  const olqTypeSelected = formData.ftw_11sqn_assessment_olq_type_id > 0;
        ...
        <div className="border border-gray-200 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select value={formData.ftw_11sqn_assessment_olq_type_id} onChange={(e) => handleChange("ftw_11sqn_assessment_olq_type_id", parseInt(e.target.value))} className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-blue-500" required>
