...
import type {
  ...
  Ftw11sqnAssessmentOlqType,
  Ftw11sqnAssessmentOlqTypeEstimatedMark
...
interface Ftw11sqnOlqResultFormProps {
...
export default function Ftw11sqnOlqResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: Ftw11sqnOlqResultFormProps) {
    ...
    const loadDropdownData = async () => {
      try {
        ...
        const [options, olqTypesRes] = await Promise.all([
          ...
          ftw11sqnAssessmentOlqTypeService.getAllTypes({ per_page: 100 }),
        ]);
  ...
  // Filter OLQ types by selected semester and auto-select the first one
  useEffect(() => {
    if (formData.semester_id) {
      ...
      // Auto-select the first matching OLQ type when semester changes
      if (filtered.length > 0) {
        setFormData(prev => ({ ...prev, ftw_11sqn_assessment_olq_type_id: filtered[0].id }));
      } else {
  ...
  }, [formData.semester_id, allOlqTypes]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: Ftw11sqnAssessmentOlqResultCreateData = {
        ...
        ftw_11sqn_assessment_olq_type_id: formData.ftw_11sqn_assessment_olq_type_id || undefined,
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <select value={formData.semester_id} onChange={(e) => handleChange("semester_id", parseInt(e.target.value))} className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-blue-500" required>
                ...
                {semesters.filter(s => s.code === "S5" || s.code === "S6").map(semester => (<option key={semester.id} value={semester.id}>{semester.name} ({semester.code})</option>))}
              </select>
            </div>
        ...
        <div className="border border-gray-200 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            ...
            FTW 11SQN OLQ Assessment ({cadetRows.length} Cadets)
            {selectedOlqType && (
