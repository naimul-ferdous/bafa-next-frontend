...
interface OlqTypeFormProps {
  initialData?: Ftw12SqnAssessmentOlqType | null;
  onSubmit: (data: Ftw12SqnAssessmentOlqTypeCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}
...
export default function OlqTypeForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: OlqTypeFormProps) {
  ...
  useEffect(() => {
    const loadMetadata = async () => {
      try {
        ...
        setSemesters(res.data.filter(s => s.is_flying));
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: Ftw12SqnAssessmentOlqTypeCreateData = {
