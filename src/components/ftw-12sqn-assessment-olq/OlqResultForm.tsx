...
import type { Ftw12SqnAssessmentOlqResult, Ftw12SqnAssessmentOlqResultCreateData, Ftw12SqnAssessmentOlqType, Ftw12SqnAssessmentOlqTypeEstimatedMark } from "@/libs/types/ftw12sqnAssessmentOlq";
...
import { Ftw12SqnOlqAssign } from "@/libs/types/ftw12sqnAssign";
...
export default function OlqResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: OlqResultFormProps) {
  ...
  const [olqAssignments, setOlqAssignments] = useState<Ftw12SqnOlqAssign[]>([]);
    ...
    const loadDropdownData = async () => {
      try {
        ...
        const [commonData, olqTypesRes, assignsData] = await Promise.all([
          ...
          ftw12sqnAssessmentOlqTypeService.getAllTypes({ per_page: 1000 }),
  ...
  useEffect(() => {
    ...
    if (filtered.length > 0 && !isEdit) {
      const current = filtered.find(t => t.id === formData.ftw_12sqn_assessment_olq_type_id);
      if (!current) {
        const autoSelect = filtered.find(t => t.is_active) || filtered[0];
        setFormData(prev => ({ ...prev, ftw_12sqn_assessment_olq_type_id: autoSelect.id }));
      }
  ...
  const handleChange = (field: string, value: any) => {
    if (field === "course_id") {
      setFormData(prev => ({ ...prev, course_id: value, semester_id: 0, ftw_12sqn_assessment_olq_type_id: 0 }));
      return;
    }
    if (field === "semester_id") {
      setFormData(prev => ({ ...prev, semester_id: value, ftw_12sqn_assessment_olq_type_id: 0 }));
      return;
    }
