...
import type {
  CtwAssessmentCounselingResult,
  CtwAssessmentCounselingResultCreateData,
  CtwAssessmentCounselingType,
  CtwAssessmentCounselingEvent
} from "@/libs/types/ctwAssessmentCounseling";
...
interface ResultFormProps {
  initialData?: CtwAssessmentCounselingResult | null;
  onSubmit: (data: CtwAssessmentCounselingResultCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}
...
export default function ResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    group_id: 0,
    exam_type_id: 0,
    ctw_assessment_counseling_type_id: 0,
  ...
  const [detectedType, setDetectedType] = useState<CtwAssessmentCounselingType | null>(null);
  const [availableEvents, setAvailableEvents] = useState<CtwAssessmentCounselingEvent[]>([]);
  ...
  useEffect(() => {
    const loadInitialOptions = async () => {
      try {
        ...
        const options = await ctwAssessmentCounselingResultService.getFormOptions();
        if (options) {
      ...
      try {
        ...
        if (options) {
          ...
          if (options.counseling_type) {
            ...
            setFormData(prev => ({ ...prev, ctw_assessment_counseling_type_id: options.counseling_type!.id }));

            const activeEvents = (options.counseling_type.events || [])
              .filter((e: CtwAssessmentCounselingEvent) => e.is_active)
              .sort((a: CtwAssessmentCounselingEvent, b: CtwAssessmentCounselingEvent) => (a.order || 0) - (b.order || 0));
            setAvailableEvents(activeEvents);

            const initialRemarks: { [key: number]: string } = {};
            activeEvents.forEach((e: CtwAssessmentCounselingEvent) => { initialRemarks[e.id] = ""; });
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        ctw_assessment_counseling_type_id: initialData.ctw_assessment_counseling_type_id,
        ...
        if (initialData.counseling_type.events) {
            ...
            if (firstCadet && firstCadet.remarks) {
                firstCadet.remarks.forEach(r => {
                    remarksMap[r.ctw_assessment_counseling_event_id] = r.remark || "";
                });
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.ctw_assessment_counseling_type_id) { setError("No Counseling Type found for selected Course and Semester"); return; }
    ...
    try {
      ...
      const submitData: CtwAssessmentCounselingResultCreateData = {
        course_id: formData.course_id,
        semester_id: formData.semester_id,
        ...
        ctw_assessment_counseling_type_id: formData.ctw_assessment_counseling_type_id,
        ...
        cadets: [
          {
            ...
            remarks: Object.entries(eventRemarks).map(([eventId, remark]) => ({
              ctw_assessment_counseling_event_id: parseInt(eventId),
              remark: remark || "",
              is_active: true,
      ...
      {!showEvents ? (
          ...
          {duplicateWarning ? (
            <div className="space-y-4">
                ...
                <div className="flex justify-center">
                  <button
                    type="button"
                    onClick={() => router.push(`/ctw/assessments/counselings/results/${duplicateWarning.resultId}/edit`)}
        ...
        <div className="border border-black rounded-lg overflow-hidden">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {availableEvents.map((event: CtwAssessmentCounselingEvent, index: number) => (
