...
export default function ResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
    ...
    const fetchCounselingType = async () => {
      ...
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setFormData(prev => ({ ...prev, ctw_assessment_counseling_type_id: type.id }));
          
          const fullType = await ctwAssessmentCounselingTypeService.getType(type.id);
          ...
          setDetectedType(null);
          setAvailableEvents([]);
          setFormData(prev => ({ ...prev, ctw_assessment_counseling_type_id: 0 }));
        }
      } catch (err) {
  ...
  useEffect(() => {
    const loadCadets = async () => {
      ...
      try {
        setLoadingDropdowns(false); // Make sure we don't clear dropdown loading if already false
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        ctw_assessment_counseling_type_id: initialData.ctw_assessment_counseling_type_id || 0,
