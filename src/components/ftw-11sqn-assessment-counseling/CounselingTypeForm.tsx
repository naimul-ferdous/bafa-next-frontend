...
import type { Ftw11SqnAssessmentCounselingType, Ftw11SqnAssessmentCounselingTypeCreateData } from "@/libs/types/ftw11sqnAssessmentCounseling";
import type { SystemSemester } from "@/libs/types/system";
import { commonService } from "@/libs/services/commonService";

interface CounselingTypeFormProps {
  initialData?: Ftw11SqnAssessmentCounselingType | null;
  onSubmit: (data: Ftw11SqnAssessmentCounselingTypeCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}
...
export default function CounselingTypeForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: CounselingTypeFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      const sortedEvents = initialData.events 
        ? [...initialData.events].sort((a, b) => (a.order || 0) - (b.order || 0))
        : [];
      ...
      setFormData({
        ...
        semesters: initialData.semesters?.map(s => s.id || s.pivot?.semester_id).filter(Boolean) || [],
      });
    }
  }, [initialData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: Ftw11SqnAssessmentCounselingTypeCreateData = {
        type_name: formData.type_name,
        type_code: formData.type_code,
        is_active: formData.is_active,
        events: formData.events.map((e, index) => ({
          event_name: e.event_name,
          event_code: e.event_code || undefined,
          event_type: e.event_type,
          order: index + 1,
        })),
        semesters: formData.semesters.map(Number),
      };
      await onSubmit(submitData);
    } catch (error: any) {
      console.error("Form submission error:", error);
      alert(error.response?.data?.message || "Failed to save Counseling type");
    }
  };

  const handleChange = (field: keyof CounselingTypeFormData, value: any) => {
  ...
  const addEvent = () => {
    setFormData(prev => ({
      ...prev,
      events: [...prev.events, { event_name: "", event_code: "", event_type: "Counseling", order: prev.events.length + 1 }],
    }));
  };

  const updateEvent = (index: number, field: keyof EventInput, value: any) => {
    setFormData(prev => ({
      ...prev,
      ...
      <div className="border border-gray-200 rounded-lg p-6">
        ...
        {formData.events.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            {`No assessment events added. Click "Add Event" to add one.`}
          </div>
        ) : (
              ...
              <div key={index} className="flex items-start gap-4 p-4 rounded-lg border border-dashed border-gray-300 bg-gray-50/30">
                <div className="flex flex-col gap-2 no-print self-center">
                  ...
                  <button
                    ...
                    className={`p-1 rounded-md transition-colors ${
                      index === formData.events.length - 1 ? "text-gray-300 cursor-not-allowed" : "text-blue-600 hover:bg-blue-100"
                    }`}
                    title="Move Down"
                  >
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateEvent(index, "event_code", e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                      placeholder="e.g. Counseling, Interview"
                    />
                  </div>
                </div>
                <button
        ...
        <div className="space-y-3">
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <div>
              ...
              <div className="text-sm text-gray-500">This Counseling type will be available for use throughout the system.</div>
            </div>
          </label>
          <label className="flex items-start gap-3 cursor-pointer">
            <input
            ...
            <div>
              ...
              <div className="text-sm text-gray-500">This Counseling type will be hidden from general use.</div>
            </div>
          </label>
        </div>
      </div>
      ...
      <div className="flex items-center justify-end gap-4">
        ...
        >
          {loading ? (
            ...
            <>{isEdit ? "Update Counseling Type" : "Create Counseling Type"}</>
          )}
        </button>
      </div>
    </form>
