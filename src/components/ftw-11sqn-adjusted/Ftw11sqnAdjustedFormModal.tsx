...
import { ftw11sqnResultsBupAdjustMarkGradingService } from "@/libs/services/ftw11sqnResultsBupAdjustMarkGradingService";
import { useFtw11sqnAdjustedModal } from "@/context/Ftw11sqnAdjustedModalContext";
import FullLogo from "@/components/ui/fulllogo";

export default function Ftw11sqnAdjustedFormModal() {
  const { isOpen, editingRecord, closeModal } = useFtw11sqnAdjustedModal();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingRecord) {
        await ftw11sqnResultsBupAdjustMarkGradingService.update(editingRecord.id, payload);
      } else {
        await ftw11sqnResultsBupAdjustMarkGradingService.create(payload);
      }
      closeModal();
      window.dispatchEvent(new CustomEvent('ftw11sqnAdjustedUpdated'));
    } catch (err: any) {
