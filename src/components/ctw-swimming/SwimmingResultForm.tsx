...
interface ResultFormProps {
  initialData?: CtwSwimmingResult | null;
...
export default function SwimmingResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
  const { user } = useAuth();
      ...
      if (initialData.result_marks && initialData.result_marks.length > 0) {
        ...
        const rows: CadetRow[] = initialData.result_marks.map(mark => {
          ...
          return {
            ...
            cadet_rank: currentRank?.short_name || currentRank?.name || "Officer Cadet",
            branch: mark.cadet?.assigned_branchs?.find((ab: any) => ab.is_current)?.branch?.name || "N/A",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.id) { setError("User session error: Instructor ID not found."); return; }

    try {
      const marks: any[] = cadetRows.map(c => ({
        cadet_id: c.cadet_id,
