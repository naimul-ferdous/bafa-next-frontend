...
import type { CtwCampingResult } from "@/libs/types/ctwCamping";

interface ResultFormProps {
  initialData?: CtwCampingResult | null;
...
export default function CampingResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
  const { user } = useAuth();
  ...
  const [campingModuleId, setCampingModuleId] = useState<number>(0);
  const [campingModule, setCampingModule] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const loadDropdownData = async () => {
      try {
        ...
        const options = await ctwCommonService.getCampingFormOptions(user.id);
        ...
        if (options) {
          ...
          if (options.module) {
            ...
            setCampingModule(options.module);
          }
        }
      } catch (err) {
  ...
  useEffect(() => {
    const loadEstimatedMarks = async () => {
      if (!campingModuleId || !formData.course_id || !formData.semester_id) {
        setEstimatedMarks([]);
        return;
      }
      ...
      try {
        ...
        const response = await ctwResultsModuleService.getEstimatedMarks(campingModuleId, {
    ...
    loadEstimatedMarks();
  }, [campingModuleId, formData.course_id, formData.semester_id]);
  ...
  useEffect(() => {
    const loadCadets = async () => {
      ...
      if (!user?.id || !campingModuleId || !formData.course_id || !formData.semester_id || !formData.exam_type_id) {
        setCadetRows([]);
  ...
  }, [user?.id, campingModuleId, formData.course_id, formData.semester_id, formData.program_id, formData.branch_id, formData.group_id, formData.exam_type_id, initialData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submitData = {
        ...
        instructor_id: user.id,
        ctw_results_module_id: campingModuleId,
        ...
        <div className="border border-gray-200 rounded-lg p-6">
          ...
          {!filtersSelected ? (
            ...
            <div className="text-center py-12 text-red-500">
              ...
              <p className="font-medium">You are not assigned to the Camping Drill module for the selected course & semester.</p>
              ...
              <table className="w-full border-collapse border border-black">
                <thead>
                  <tr>
                    ...
                    {campingModule && campingModule.instructor_count > 0 && (
                      <th className="border border-black px-3 py-2 text-center" colSpan={campingModule.instructor_count}>Instructors</th>
                    )}
                  ...
                  {campingModule && campingModule.instructor_count > 0 && (
                    <tr>
                      {Array.from({ length: campingModule.instructor_count }, (_, i) => i + 1).map(instrNum => (
                ...
                <tbody>
                  {cadetRows.map((cadet, index) => (
                    <tr key={cadet.cadet_id}>
                      ...
                      {campingModule && campingModule.instructor_count > 0 && (
                        Array.from({ length: campingModule.instructor_count }, (_, i) => (
