...
import type { Ftw12sqnGroundSyllabus, Ftw12sqnGroundSyllabusExercise } from "@/libs/types/ftw12sqnFlying";
import DatePicker from "@/components/form/input/DatePicker";
import GroundEditMarkModal from "./GroundEditMarkModal";

interface ExerciseWithGroundInfo extends Ftw12sqnGroundSyllabusExercise {
...
export default function Ftw12sqnGroundExaminationMarkForm({
  onSubmit,
  onCancel,
  loading,
  isEdit = false,
  ...
  const [editData, setEditData] = useState({
    ftw_12sqn_ground_syllabus_exercise_id:
      initialData?.ftw_12sqn_ground_syllabus_exercise_id || 0,
  ...
  useEffect(() => {
    const loadFormData = async () => {
      try {
        ...
        const formDataResponse = await ftw12sqnGroundExaminationMarkService.getFormData({
        ...
        selectedGroundIds.forEach((syllabusId) => {
          ...
          selectedSyllabus.exercises?.forEach((exercise) => {
            if (exercise.is_active) {
              allExercises.push({
                ...exercise,
                ground_type_id: selectedSyllabus.ground_type_id || 0,
  ...
  useEffect(() => {
    ...
    if (formData.course_id && formData.semester_id && formData.exam_type_id) {
      const fetchExistingMarks = async () => {
        ...
        try {
          const result = await ftw12sqnGroundExaminationMarkService.getCadetMarks({
          ...
          if (result.marks && result.marks.length > 0) {
            ...
            result.marks.forEach((m) => {
              marksMap.set(m.ftw_12sqn_ground_syllabus_exercise_id, m);
            });
  ...
  const handleUpdateExistingMark = async (row: GroundRow) => {
    ...
    try {
      const submitData = {
        ftw_12sqn_ground_syllabus_id: row.syllabus_id,
        ftw_12sqn_ground_syllabus_exercise_id: row.exercise_id,
        course_id: formData.course_id,
        semester_id: formData.semester_id,
      ...
      await ftw12sqnGroundExaminationMarkService.updateMark(markInfo.id as number, submitData);
    ...
    try {
      if (isEdit) {
        ...
        const submitData = {
          ...
          ftw_12sqn_ground_syllabus_exercise_id:
            editData.ftw_12sqn_ground_syllabus_exercise_id,
          course_id: formData.course_id,
          semester_id: formData.semester_id,
        ...
        const marks: any[] = groundRows
          ...
          .map((r) => ({
            cadet_id: formData.selected_cadet_id,
            ftw_12sqn_ground_syllabus_id: r.syllabus_id,
            ftw_12sqn_ground_syllabus_exercise_id: r.exercise_id,
