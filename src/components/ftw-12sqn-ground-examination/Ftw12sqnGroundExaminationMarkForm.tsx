...
import { Ftw12sqnGroundSyllabus, Ftw12sqnGroundSyllabusExercise } from "@/libs/types/ftw12sqnFlying";
...
export default function Ftw12sqnGroundExaminationMarkForm({ onSubmit, onCancel, loading, isEdit = false, initialData }: BulkFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    syllabus_id: isEdit ? (initialData?.ftw_12sqn_ground_syllabus_id || 0) : 0,
  });

  // Single record edit state
  const [editData, setEditData] = useState({
    ftw_12sqn_ground_syllabus_exercise_id: initialData?.ftw_12sqn_ground_syllabus_exercise_id || 0,
  ...
  const [syllabuses, setSyllabuses] = useState<Ftw12sqnGroundSyllabus[]>([]);
  const [allCadets, setAllCadets] = useState<CadetProfile[]>([]);
  const [exercises, setExercises] = useState<Ftw12sqnGroundSyllabusExercise[]>([]);
    ...
    const loadInitialData = async () => {
      try {
        ...
        if (options) {
          ...
          setSyllabuses(options.ftw12sqn_ground_syllabuses);
        }
  ...
  useEffect(() => {
    const loadExercises = async () => {
      ...
      try {
        ...
        const fullSyllabus = await ftw12sqnGroundSyllabusService.get(formData.syllabus_id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        ...
        if (!editData.ftw_12sqn_ground_syllabus_exercise_id) {
        ...
        const submitData = {
          ftw_12sqn_ground_syllabus_id: formData.syllabus_id,
          ftw_12sqn_ground_syllabus_exercise_id: editData.ftw_12sqn_ground_syllabus_exercise_id,
          course_id: formData.course_id,
          semester_id: formData.semester_id,
        ...
        cadetRows.filter(c => c.is_active && c.cadet_id > 0).forEach(c => {
          marks.push({
            cadet_id: c.cadet_id,
            ftw_12sqn_ground_syllabus_id: formData.syllabus_id,
            ftw_12sqn_ground_syllabus_exercise_id: c.exercise_id,
            ...
            <div className="border border-gray-200 rounded-lg p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={editData.ftw_12sqn_ground_syllabus_exercise_id}
                    onChange={(e) => handleEditChange("ftw_12sqn_ground_syllabus_exercise_id", parseInt(e.target.value))}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-blue-500"
                    required
                      ...
                      <tr key={cadet.cadet_id} className={!cadet.is_active ? "bg-gray-100 opacity-50" : ""}>
                        ...
                        <td className="border border-black px-2 py-1">
                          <select
                            ...
                            className="w-full px-2 py-1 border border-gray-200 rounded focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
                          >
                            <option value={0}>
                        ...
                        <td className="border border-black px-2 py-1">
                          <input
                            ...
                            disabled={!cadet.is_active}
                            placeholder="0"
                            className="w-24 px-2 py-1 border border-gray-200 rounded text-center focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100"
                          />
                        </td>
                        <td className="border border-black px-2 py-1">
                          <input
