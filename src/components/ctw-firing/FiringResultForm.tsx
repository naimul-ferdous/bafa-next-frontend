...
import type { CtwFiringResult } from "@/libs/types/ctwFiring";

interface ResultFormProps {
  initialData?: CtwFiringResult | null;
...
export default function FiringResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
  const { user } = useAuth();
  ...
  const [firingModuleId, setFiringModuleId] = useState<number>(0);
  const [firingModule, setFiringModule] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const loadDropdownData = async () => {
      try {
        ...
        const options = await ctwCommonService.getFiringFormOptions(user.id);
        ...
        if (options) {
          ...
          if (options.module) {
            ...
            setFiringModule(options.module);
          }
        }
      } catch (err) {
  ...
  useEffect(() => {
    const loadEstimatedMarks = async () => {
      if (!firingModuleId || !formData.course_id || !formData.semester_id) {
        setEstimatedMarks([]);
        return;
      }
      ...
      try {
        ...
        const response = await ctwResultsModuleService.getEstimatedMarks(firingModuleId, {
    ...
    loadEstimatedMarks();
  }, [firingModuleId, formData.course_id, formData.semester_id]);
  ...
  useEffect(() => {
    const loadCadets = async () => {
      ...
      if (!user?.id || !firingModuleId || !formData.course_id || !formData.semester_id || !formData.exam_type_id) {
        setCadetRows([]);
      ...
      try {
        ...
        const moduleAssignRes = await ctwInstructorAssignModuleService.getAll({
          ...
          module_code: FIRING_MODULE_CODE,
  ...
  }, [user?.id, firingModuleId, formData.course_id, formData.semester_id, formData.program_id, formData.branch_id, formData.group_id, formData.exam_type_id, initialData]);
        ...
        <div className="border border-gray-200 rounded-lg p-6">
          ...
          {!filtersSelected ? (
            ...
            <div className="text-center py-12 text-red-500">
              ...
              <p className="font-medium">You are not assigned to the Firing Drill module for the selected course & semester.</p>
              ...
              <table className="w-full border-collapse border border-black">
                <thead>
                  <tr>
                    ...
                    {firingModule && firingModule.instructor_count > 0 && (
                      <th className="border border-black px-3 py-2 text-center" colSpan={firingModule.instructor_count}>Instructors</th>
                    )}
                  ...
                  {firingModule && firingModule.instructor_count > 0 && (
                    <tr>
                      {Array.from({ length: firingModule.instructor_count }, (_, i) => i + 1).map(instrNum => (
                ...
                <tbody>
                  {cadetRows.map((cadet, index) => (
                    <tr key={cadet.cadet_id}>
                      ...
                      {firingModule && firingModule.instructor_count > 0 && (
                        Array.from({ length: firingModule.instructor_count }, (_, i) => (
