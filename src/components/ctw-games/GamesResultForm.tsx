...
interface ResultFormProps {
  initialData?: CtwGamesResult | null;
...
export default function GamesResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (initialData.achieved_marks && initialData.achieved_marks.length > 0) {
      ...
      const rows: CadetRow[] = uniqueCadetIds.map(cadetId => {
        ...
        const total_mark = Object.keys(detail_entries).length > 0
          ? Object.values(detail_entries).reduce((sum, e) => sum + e.mark, 0)
          : parseFloat(String(mark?.achieved_mark || 0));
  ...
  const handleSimpleMarkChange = (cadetIndex: number, value: number) => {
    setCadetRows(prev => {
      const updated = [...prev];
      const clampedMark = simpleMaxMark > 0 ? Math.min(value, simpleMaxMark) : value;
      updated[cadetIndex] = { ...updated[cadetIndex], total_mark: clampedMark };
      return updated;
    });
  };
        ...
        <div className="border border-gray-200 rounded-lg p-6">
          ...
          {!filtersSelected ? (
            ...
            <div className="text-center py-12 text-red-500">
              ...
              <p className="font-medium">You are not assigned to the Games module for the selected course & semester.</p>
              ...
              <table className="w-full border-collapse border border-black text-xs">
                <thead>
                  <tr>
                    ...
                    <th className="border border-black px-2 py-2 text-center whitespace-nowrap">BD No.</th>
                    <th className="border border-black px-2 py-2 text-center whitespace-nowrap">Rank</th>
                    <th className="border border-black px-2 py-2 text-left">Name</th>
                    <th className="border border-black px-2 py-2 text-left">Branch</th>
                    <th className="border border-black px-2 py-2 text-center whitespace-nowrap">
                      Mark{simpleMaxMark > 0 ? ` (Max: ${simpleMaxMark})` : ""}
                    </th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={cadet.cadet_id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      ...
                      <td className="border border-black px-1 py-1 text-center">
                        <input
                          type="number"
                          min={0}
                          max={simpleMaxMark > 0 ? simpleMaxMark : undefined}
                          step={0.01}
