...
import type { CtwSwordDrillAssessmentEstimatedMark, CtwSwordDrillAssessmentEstimatedMarkCreateData } from "@/libs/types/ctwSwordDrill";
...
interface SwordDrillEstimatedMarkFormProps {
  initialData?: CtwSwordDrillAssessmentEstimatedMark | null;
  onSubmit: (data: CtwSwordDrillAssessmentEstimatedMarkCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}

export interface SwordDrillEstimatedMarkFormData {
  semester_id: number;
  exam_type_id: number;
...
export default function SwordDrillEstimatedMarkForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: SwordDrillEstimatedMarkFormProps) {
  const [formData, setFormData] = useState<SwordDrillEstimatedMarkFormData>({
    semester_id: 0,
    exam_type_id: 0,
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof SwordDrillEstimatedMarkFormData, string>>>({});
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof SwordDrillEstimatedMarkFormData, string>> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: CtwSwordDrillAssessmentEstimatedMarkCreateData = {
  ...
  const handleChange = (field: keyof SwordDrillEstimatedMarkFormData, value: any) => {
