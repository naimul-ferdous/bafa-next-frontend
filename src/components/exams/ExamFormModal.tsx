...
export default function ExamFormModal() {
  const { isOpen, editingExam, closeModal } = useExamModal();
      ...
      <form onSubmit={handleSubmit} className="p-8">
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            {editingExam ? "Edit Exam Type" : "Add a New Exam Type"}
          </h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {editingExam ? "Update exam type details" : "Configure your new exam type details"}
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label>Exam Type Name <span className="text-red-500">*</span></Label>
              <Input value={formData.name} onChange={(e) => handleChange("name", e.target.value)} placeholder="e.g. Midterm, Final" required />
            </div>
            <div>
            ...
            <div className="space-y-3">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400">This exam type will be available for use throughout the system.</div>
                </div>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-400">This exam type will be hidden from general use.</div>
                </div>
              </label>
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-3 mt-2 pt-2">
          ...
          <button type="submit" className="px-6 py-2 bg-blue-500 text-white rounded-xl" disabled={loading}>{loading ? "Saving..." : editingExam ? "Update Exam Type" : "Save Exam Type"}</button>
        </div>
      </form>
    </Modal>
  );
