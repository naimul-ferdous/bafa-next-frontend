...
import { useFtw11sqnFlyingTypeModal } from "@/context/Ftw11sqnFlyingTypeModalContext";
import FullLogo from "@/components/ui/fulllogo";

export default function FlyingTypeFormModal() {
  const { isOpen, editingType, viewingType, isViewMode, closeModal } = useFtw11sqnFlyingTypeModal();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingType) {
        await ftw11sqnFlyingTypeService.update(editingType.id, formData);
      } else {
        await ftw11sqnFlyingTypeService.create(formData);
      }
      closeModal();
