...
export default function ResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
  const [formData, setFormData] = useState({
    ...
    group_id: 0,
    exam_type_id: 0,
    ftw_12sqn_assessment_counseling_type_id: 0,
    ...
    const fetchCounselingType = async () => {
      ...
      try {
        ...
        if (response.data && response.data.length > 0) {
          ...
          setFormData(prev => ({ ...prev, ftw_12sqn_assessment_counseling_type_id: type.id }));
          
          const fullType = await ftw12sqnAssessmentCounselingTypeService.getType(type.id);
          ...
          setDetectedType(null);
          setAvailableEvents([]);
          setFormData(prev => ({ ...prev, ftw_12sqn_assessment_counseling_type_id: 0 }));
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        ftw_12sqn_assessment_counseling_type_id: initialData.ftw_12sqn_assessment_counseling_type_id || 0,
  ...
  const getSelectedInstructor = () => {
    ...
    return instructor?.name || (instructor as any)?.instructor_biodata?.name || "—";
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.ftw_12sqn_assessment_counseling_type_id) { setError("No Counseling Type found for selected Course and Semester"); return; }
    ...
    try {
      ...
      const submitData = {
        ...
        ftw_12sqn_assessment_counseling_type_id: formData.ftw_12sqn_assessment_counseling_type_id,
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <select value={formData.instructor_id} onChange={(e) => handleChange("instructor_id", parseInt(e.target.value))} className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-blue-500 focus:outline-none" required>
            ...
            {instructors.map(ins => (<option key={ins.id} value={ins.id}>{ins.name || (ins as any).instructor_biodata?.name} ({ins.service_number || (ins as any).instructor_biodata?.service_number})</option>))}
          </select>
        </div>
