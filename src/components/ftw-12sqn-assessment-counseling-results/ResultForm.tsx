...
import type {
  Ftw12SqnAssessmentCounselingResult,
  Ftw12SqnAssessmentCounselingResultCreateData,
  Ftw12SqnAssessmentCounselingType,
  Ftw12SqnAssessmentCounselingEvent
} from "@/libs/types/ftw12sqnAssessmentCounseling";
...
interface ResultFormProps {
  initialData?: Ftw12SqnAssessmentCounselingResult | null;
  onSubmit: (data: Ftw12SqnAssessmentCounselingResultCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}
...
export default function ResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    group_id: 0,
    exam_type_id: 0,
    ftw12sqn_assessment_counseling_type_id: 0,
  ...
  const [detectedType, setDetectedType] = useState<Ftw12SqnAssessmentCounselingType | null>(null);
  const [availableEvents, setAvailableEvents] = useState<Ftw12SqnAssessmentCounselingEvent[]>([]);
  ...
  useEffect(() => {
    const loadInitialOptions = async () => {
      try {
        ...
        const options = await ftw12sqnAssessmentCounselingResultService.getFormOptions();
        if (options) {
      ...
      try {
        ...
        if (options) {
          ...
          if (options.counseling_type) {
            ...
            setFormData(prev => ({ ...prev, ftw12sqn_assessment_counseling_type_id: options.counseling_type!.id }));

            const activeEvents = (options.counseling_type.events || [])
              .filter((e: Ftw12SqnAssessmentCounselingEvent) => e.is_active)
              .sort((a: Ftw12SqnAssessmentCounselingEvent, b: Ftw12SqnAssessmentCounselingEvent) => (a.order || 0) - (b.order || 0));
            setAvailableEvents(activeEvents);

            const initialRemarks: { [key: number]: string } = {};
            activeEvents.forEach((e: Ftw12SqnAssessmentCounselingEvent) => { initialRemarks[e.id] = ""; });
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        ftw12sqn_assessment_counseling_type_id: initialData.ftw12sqn_assessment_counseling_type_id,
        ...
        if (initialData.counseling_type.events) {
            ...
            if (firstCadet && firstCadet.remarks) {
                firstCadet.remarks.forEach(r => {
                    remarksMap[r.ftw_12sqn_assessment_counseling_event_id] = r.remark || "";
                });
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.ftw12sqn_assessment_counseling_type_id) { setError("No Counseling Type found for selected Course and Semester"); return; }
    ...
    try {
      ...
      const submitData: Ftw12SqnAssessmentCounselingResultCreateData = {
        course_id: formData.course_id,
        semester_id: formData.semester_id,
        ...
        ftw12sqn_assessment_counseling_type_id: formData.ftw12sqn_assessment_counseling_type_id,
        ...
        cadets: [
          {
            ...
            remarks: Object.entries(eventRemarks).map(([eventId, remark]) => ({
              ftw_12sqn_assessment_counseling_event_id: parseInt(eventId),
              remark: remark || "",
              is_active: true,
      ...
      {!showEvents ? (
          ...
          {duplicateWarning ? (
            <div className="space-y-4">
                ...
                <div className="flex justify-center">
                  <button
                    type="button"
                    onClick={() => router.push(`/ftw12sqn/assessments/counselings/results/${duplicateWarning.resultId}/edit`)}
        ...
        <div className="border border-black rounded-lg overflow-hidden">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {availableEvents.map((event: Ftw12SqnAssessmentCounselingEvent, index: number) => (
