...
import type { Ftw12SqnAssessmentCommandType, Ftw12SqnAssessmentCommandTypeCreateData } from "@/libs/types/ftw12sqnAssessmentCommand";
import semesterService from "@/libs/services/semesterService";
import { SystemSemester } from "@/libs/types/system";

interface CommandTypeFormProps {
  initialData?: Ftw12SqnAssessmentCommandType | null;
  onSubmit: (data: Ftw12SqnAssessmentCommandTypeCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}
...
export interface CommandTypeFormData {
  type_name: string;
  type_code: string;
...
export default function CommandTypeForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: CommandTypeFormProps) {
  const [formData, setFormData] = useState<CommandTypeFormData>({
    type_name: "",
    type_code: "",
    is_multiplier: false,
    multiplier: "1",
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof CommandTypeFormData, string>>>({});
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof CommandTypeFormData, string>> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: Ftw12SqnAssessmentCommandTypeCreateData = {
      ...
      alert(error.response?.data?.message || "Failed to save Command type");
    }
  };

  const handleChange = (field: keyof CommandTypeFormData, value: any) => {
        ...
        <div className="space-y-3">
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <div>
              ...
              <div className="text-sm text-gray-500">This Command type will be available for assignment.</div>
            </div>
          </label>
          <label className="flex items-start gap-3 cursor-pointer">
            <input
            ...
            <div>
              ...
              <div className="text-sm text-gray-500">This Command type will be hidden.</div>
            </div>
          </label>
        </div>
      </div>
      ...
      <div className="flex items-center justify-end gap-4">
        ...
        >
          {loading ? (
            ...
            <>{isEdit ? "Update Command Type" : "Create Command Type"}</>
          )}
        </button>
      </div>
    </form>
