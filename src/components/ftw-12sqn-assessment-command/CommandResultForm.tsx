...
import type { Ftw12SqnAssessmentCommandResult, Ftw12SqnAssessmentCommandResultCreateData, Ftw12SqnAssessmentCommandType, Ftw12SqnAssessmentCommandTypeEstimatedMark } from "@/libs/types/ftw12sqnAssessmentCommand";
...
import { Ftw12SqnCommandAssign } from "@/libs/types/ftw12sqnAssign";

interface CommandResultFormProps {
  initialData?: Ftw12SqnAssessmentCommandResult | null;
  onSubmit: (data: Ftw12SqnAssessmentCommandResultCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}
...
export default function CommandResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: CommandResultFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    semester_id: 0,
    exam_type_id: 0,
    ftw_12sqn_assessment_command_type_id: 0,
    remarks: "",
  });
  ...
  const [CommandAssignments, setCommandAssignments] = useState<Ftw12SqnCommandAssign[]>([]);
  const [allCommandTypes, setAllCommandTypes] = useState<Ftw12SqnAssessmentCommandType[]>([]);
  const [filteredCommandTypes, setFilteredCommandTypes] = useState<Ftw12SqnAssessmentCommandType[]>([]);
  ...
  const [selectedCommandType, setSelectedCommandType] = useState<Ftw12SqnAssessmentCommandType | null>(null);
  const [estimatedMarks, setEstimatedMarks] = useState<Ftw12SqnAssessmentCommandTypeEstimatedMark[]>([]);
    ...
    const loadDropdownData = async () => {
      try {
        ...
        const [commonData, CommandTypesRes, assignsData] = await Promise.all([
          ...
          Ftw12SqnAssessmentCommandTypeService.getAllTypes({ per_page: 1000 }),
          user?.id ? ftw12sqnUserAssignService.getAll({ user_id: user.id }) : Promise.resolve(null),
        ]);
  ...
  useEffect(() => {
    ...
    if (filtered.length > 0 && !isEdit) {
      const current = filtered.find(t => t.id === formData.ftw_12sqn_assessment_command_type_id);
      if (!current) {
        const autoSelect = filtered.find(t => t.is_active) || filtered[0];
        setFormData(prev => ({ ...prev, ftw_12sqn_assessment_command_type_id: autoSelect.id }));
      }
    } else if (filtered.length === 0) {
      setFormData(prev => ({ ...prev, ftw_12sqn_assessment_command_type_id: 0 }));
    }
  ...
  useEffect(() => {
    if (formData.ftw_12sqn_assessment_command_type_id) {
      const type = allCommandTypes.find(t => t.id === formData.ftw_12sqn_assessment_command_type_id);
  ...
  }, [formData.ftw_12sqn_assessment_command_type_id, allCommandTypes]);
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        ftw_12sqn_assessment_command_type_id: initialData.ftw_12sqn_assessment_command_type_id,
      ...
      if (initialData.result_cadets && initialData.result_cadets.length > 0) {
        const rows: CadetRow[] = initialData.result_cadets.map(c => {
          ...
          c.marks?.forEach(m => {
            marks[m.ftw_12sqn_assessment_command_type_estimated_mark_id] = parseFloat(String(m.achieved_mark));
          });
  ...
  const handleChange = (field: string, value: any) => {
    if (field === "course_id") {
      setFormData(prev => ({ ...prev, course_id: value, semester_id: 0, ftw_12sqn_assessment_command_type_id: 0 }));
      return;
    }
    if (field === "semester_id") {
      setFormData(prev => ({ ...prev, semester_id: value, ftw_12sqn_assessment_command_type_id: 0 }));
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.ftw_12sqn_assessment_command_type_id) { setError("Please select an Command type"); return; }

    try {
      const submitData: Ftw12SqnAssessmentCommandResultCreateData = {
        course_id: formData.course_id,
        semester_id: formData.semester_id,
        ftw_12sqn_assessment_command_type_id: formData.ftw_12sqn_assessment_command_type_id,
        ...
        cadets: cadetRows.filter(c => c.cadet_id > 0).map(c => ({
          ...
          marks: Object.entries(c.marks).map(([estimatedMarkId, achievedMark]) => ({
            ftw_12sqn_assessment_command_type_estimated_mark_id: parseInt(estimatedMarkId),
  ...
  const filtersSelected = isInstructor
    ...
    : formData.course_id && formData.semester_id;
  const CommandTypeSelected = formData.ftw_12sqn_assessment_command_type_id > 0;
        ...
        <div className="border border-gray-200 rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select value={formData.ftw_12sqn_assessment_command_type_id} onChange={(e) => handleChange("ftw_12sqn_assessment_command_type_id", parseInt(e.target.value))} className="w-full px-4 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 focus:ring-2 focus:ring-blue-500" required>
