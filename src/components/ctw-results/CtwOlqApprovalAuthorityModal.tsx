...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  authority: CtwOlqApprovalAuthority | null;
}

export default function CtwOlqApprovalAuthorityModal({ isOpen, onClose, onSuccess, authority }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authority) {
        await ctwOlqApprovalAuthorityService.update(authority.id, data);
      } else {
        await ctwOlqApprovalAuthorityService.store(data);
      }
      onSuccess();
      onClose();
    } catch {
      ...
      <div className="p-6">
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-sm text-gray-500 text-center">
            Define roles and permissions for CTW OLQ assessment approval processes.
          </p>
        </div>
