...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  authority: CtwPenpictureAuthority | null;
}

export default function CtwPenpictureApprovalAuthorityModal({ isOpen, onClose, onSuccess, authority }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authority) {
        await ctwPenpictureAuthorityService.update(authority.id, data);
      } else {
        await ctwPenpictureAuthorityService.store(data);
      }
      onSuccess();
      onClose();
    } catch {
      ...
      <div className="p-6">
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-900 uppercase text-center">
            {authority ? "Edit Pen Picture Approval Authority" : "Add Pen Picture Approval Authority"}
          </h2>
          <p className="text-sm text-gray-500 text-center">
            Define roles and permissions for CTW Pen Picture assessment approval processes.
          </p>
        </div>
