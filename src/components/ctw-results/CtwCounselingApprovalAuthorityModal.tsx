...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  authority: CtwCounselingApprovalAuthority | null;
}

export default function CtwCounselingApprovalAuthorityModal({ isOpen, onClose, onSuccess, authority }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authority) {
        await ctwCounselingApprovalAuthorityService.update(authority.id, data);
      } else {
        await ctwCounselingApprovalAuthorityService.store(data);
      }
      onSuccess();
      onClose();
    } catch {
      ...
      <div className="p-6">
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-sm text-gray-500 text-center">
            Define roles and permissions for CTW Counseling assessment approval processes.
          </p>
        </div>
