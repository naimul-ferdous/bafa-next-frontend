...
export default function LeadershipSkillResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
  ...
  const [footDrillModuleId, setLeadershipDrillModuleId] = useState<number>(0);
  const [footDrillModule, setLeadershipDrillModule] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const loadDropdownData = async () => {
      try {
        ...
        const options = await ctwCommonService.getLeadershipSkillFormOptions(user.id);
        ...
        if (options) {
          ...
          if (options.module) {
            setLeadershipDrillModuleId(options.module.id);
            setLeadershipDrillModule(options.module);
          }
        }
      } catch (err) {
        ...
        <div className="border border-gray-200 rounded-lg p-6">
          ...
          {!filtersSelected ? (
            ...
            <div className="text-center py-12 text-red-500">
              ...
              <p className="font-medium">You are not assigned to the Leadership Drill module for the selected course & semester.</p>
