...
interface ResultFormProps {
  initialData?: CtwLeadershipSkillResult | null;
...
export default function LeadershipSkillResultForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: ResultFormProps) {
  const { user } = useAuth();
  ...
  const [leadershipSkillModuleId, setLeadershipSkillModuleId] = useState<number>(0);
  ...
  // Load dropdown data + find leadership skill module
  useEffect(() => {
    const loadDropdownData = async () => {
      try {
        ...
        // Find leadership skill module id
  ...
  useEffect(() => {
    const loadEstimatedMarks = async () => {
      if (!leadershipSkillModuleId || !formData.course_id || !formData.semester_id) {
        setEstimatedMarks([]);
        return;
      }
      ...
      try {
        ...
        const response = await ctwResultsModuleService.getEstimatedMarks(leadershipSkillModuleId, {
    ...
    loadEstimatedMarks();
  }, [leadershipSkillModuleId, formData.course_id, formData.semester_id]);
  ...
  useEffect(() => {
    const loadCadets = async () => {
      ...
      if (!user?.id || !leadershipSkillModuleId || !formData.course_id || !formData.semester_id || !formData.exam_type_id) {
        setCadetRows([]);
      ...
      try {
        ...
        // First check if instructor is assigned to this leadership_skill module
  ...
  }, [user?.id, leadershipSkillModuleId, formData.course_id, formData.semester_id, formData.program_id, formData.branch_id, formData.group_id, formData.exam_type_id, initialData]);
        ...
        <div className="border border-gray-200 rounded-lg p-6">
          ...
          {!filtersSelected ? (
            ...
            <div className="text-center py-12 text-red-500">
              ...
              <p className="font-medium">You are not assigned to the Leadership Skill module for the selected course & semester.</p>
