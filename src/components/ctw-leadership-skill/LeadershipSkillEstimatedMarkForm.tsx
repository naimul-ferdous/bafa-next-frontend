...
import type { CtwLeadershipSkillAssessmentEstimatedMark, CtwLeadershipSkillAssessmentEstimatedMarkCreateData } from "@/libs/types/ctwLeadershipSkill";
...
interface LeadershipSkillEstimatedMarkFormProps {
  initialData?: CtwLeadershipSkillAssessmentEstimatedMark | null;
  onSubmit: (data: CtwLeadershipSkillAssessmentEstimatedMarkCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}

export interface LeadershipSkillEstimatedMarkFormData {
  semester_id: number;
  exam_type_id: number;
...
export default function LeadershipSkillEstimatedMarkForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: LeadershipSkillEstimatedMarkFormProps) {
  const [formData, setFormData] = useState<LeadershipSkillEstimatedMarkFormData>({
    semester_id: 0,
    exam_type_id: 0,
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof LeadershipSkillEstimatedMarkFormData, string>>>({});
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof LeadershipSkillEstimatedMarkFormData, string>> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: CtwLeadershipSkillAssessmentEstimatedMarkCreateData = {
  ...
  const handleChange = (field: keyof LeadershipSkillEstimatedMarkFormData, value: any) => {
