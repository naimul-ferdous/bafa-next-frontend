...
import { Role, User } from "@/libs/types/user";
...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  authority: Ftw11SqnCounselingApprovalAuthority | null;
}

export default function Ftw11SqnCounselingApprovalAuthorityModal({ isOpen, onClose, onSuccess, authority }: Props) {
    ...
    if (isOpen) {
      ...
      if (authority) {
        setFormData({
          ...
          is_signature: authority.is_signature ?? false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authority) {
        await ftw11sqnCounselingApprovalAuthorityService.update(authority.id, data);
      } else {
        await ftw11sqnCounselingApprovalAuthorityService.store(data);
      }
      onSuccess();
      onClose();
    } catch {
      ...
      <div className="p-6">
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-900 uppercase text-center">
            {authority ? "Edit Counseling Approval Authority" : "Add Counseling Approval Authority"}
          </h2>
          <p className="text-sm text-gray-500 text-center">
            Define roles and permissions for Counseling assessment approval processes.
          </p>
        </div>
