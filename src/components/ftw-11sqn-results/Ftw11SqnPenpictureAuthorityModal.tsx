...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  authority: Ftw11SqnPenpictureAuthority | null;
}

export default function Ftw11SqnPenpictureAuthorityModal({ isOpen, onClose, onSuccess, authority }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authority) {
        await ftw11sqnPenpictureAuthorityService.update(authority.id, data);
      } else {
        await ftw11sqnPenpictureAuthorityService.store(data);
      }
      onSuccess();
      onClose();
    } catch {
      ...
      <div className="p-6">
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-900 uppercase text-center">
            {authority ? "Edit Penpicture Authority" : "Add Penpicture Authority"}
          </h2>
          <p className="text-sm text-gray-500 text-center">
            Define roles and permissions for pen picture approval processes.
          </p>
        </div>
