...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  authority: Ftw11SqnOlqApprovalAuthority | null;
}

export default function Ftw11SqnOlqApprovalAuthorityModal({ isOpen, onClose, onSuccess, authority }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authority) {
        await ftw11sqnOlqApprovalAuthorityService.update(authority.id, data);
      } else {
        await ftw11sqnOlqApprovalAuthorityService.store(data);
      }
      onSuccess();
      onClose();
    } catch {
