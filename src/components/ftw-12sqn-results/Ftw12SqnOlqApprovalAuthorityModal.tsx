...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  authority: Ftw12SqnOlqApprovalAuthority | null;
}

export default function Ftw12SqnOlqApprovalAuthorityModal({ isOpen, onClose, onSuccess, authority }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authority) {
        await ftw12sqnOlqApprovalAuthorityService.update(authority.id, data);
      } else {
        await ftw12sqnOlqApprovalAuthorityService.store(data);
      }
      onSuccess();
      onClose();
    } catch {
      ...
      <div className="p-6">
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-900 uppercase text-center">
            {authority ? "Edit OLQ Approval Authority" : "Add OLQ Approval Authority"}
          </h2>
          <p className="text-sm text-gray-500 text-center">
            Define roles and permissions for OLQ assessment approval processes.
          </p>
        </div>
