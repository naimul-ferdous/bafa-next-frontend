...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  authority: Ftw12SqnPenpictureAuthority | null;
}

export default function Ftw12SqnPenpictureAuthorityModal({ isOpen, onClose, onSuccess, authority }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authority) {
        await ftw12sqnPenpictureAuthorityService.update(authority.id, data);
      } else {
        await ftw12sqnPenpictureAuthorityService.store(data);
      }
      onSuccess();
      onClose();
    } catch {
