...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  authority: Ftw12SqnCounselingApprovalAuthority | null;
}

export default function Ftw12SqnCounselingApprovalAuthorityModal({ isOpen, onClose, onSuccess, authority }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authority) {
        await ftw12sqnCounselingApprovalAuthorityService.update(authority.id, data);
      } else {
        await ftw12sqnCounselingApprovalAuthorityService.store(data);
      }
      onSuccess();
      onClose();
    } catch {
