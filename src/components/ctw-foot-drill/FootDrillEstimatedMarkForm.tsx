...
import type { CtwFootDrillAssessmentEstimatedMark, CtwFootDrillAssessmentEstimatedMarkCreateData } from "@/libs/types/ctwFootDrill";
...
interface FootDrillEstimatedMarkFormProps {
  initialData?: CtwFootDrillAssessmentEstimatedMark | null;
  onSubmit: (data: CtwFootDrillAssessmentEstimatedMarkCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}

export interface FootDrillEstimatedMarkFormData {
  semester_id: number;
  exam_type_id: number;
...
export default function FootDrillEstimatedMarkForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: FootDrillEstimatedMarkFormProps) {
  const [formData, setFormData] = useState<FootDrillEstimatedMarkFormData>({
    semester_id: 0,
    exam_type_id: 0,
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof FootDrillEstimatedMarkFormData, string>>>({});
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof FootDrillEstimatedMarkFormData, string>> = {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: CtwFootDrillAssessmentEstimatedMarkCreateData = {
  ...
  const handleChange = (field: keyof FootDrillEstimatedMarkFormData, value: any) => {
