...
interface CounselingTypeFormProps {
  initialData?: Ftw12SqnAssessmentCounselingType | null;
  onSubmit: (data: Ftw12SqnAssessmentCounselingTypeCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}
...
export default function CounselingTypeForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: CounselingTypeFormProps) {
  ...
  useEffect(() => {
    const loadMetadata = async () => {
      try {
        ...
        if (options) {
          setSemesters((options.semesters || []).filter(s => s.is_flying));
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData: Ftw12SqnAssessmentCounselingTypeCreateData = {
