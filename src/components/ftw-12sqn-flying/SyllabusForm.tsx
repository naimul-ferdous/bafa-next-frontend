...
import type {
  ...
  Ftw12sqnFlyingSyllabusCreateData,
  Ftw12sqnFlyingPhaseType,
  Ftw12sqnFlyingSyllabusTypeCreateData,
  Ftw12sqnFlyingSyllabusExerciseCreateData,
  Ftw12sqnFlyingType
...
interface SyllabusFormProps {
  ...
  onSubmit: (data: Ftw12sqnFlyingSyllabusCreateData) => Promise<void>;
  onCancel: () => void;
  loading: boolean;
  isEdit?: boolean;
}
...
export default function SyllabusForm({ initialData, onSubmit, onCancel, loading, isEdit = false }: SyllabusFormProps) {
      ...
      initialData.syllabus_types?.forEach(st => {
        st.exercises?.forEach(ex => {
          allExercises.push({
            ...
            phase_type_id: st.ftw_12sqn_flying_phase_type_id,
          });
        });
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const syllabusTypes: Ftw12sqnFlyingSyllabusTypeCreateData[] = [];
      exercisesByType.forEach((exercises, phaseTypeId) => {
        ...
        syllabusTypes.push({
          ...
          exercises: exercises.map((ex, index) => {
            const exercise: Ftw12sqnFlyingSyllabusExerciseCreateData = {
              id: ex.id,
