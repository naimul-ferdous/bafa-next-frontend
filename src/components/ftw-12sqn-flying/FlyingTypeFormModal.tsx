...
import { useFtw12sqnFlyingTypeModal } from "@/context/Ftw12sqnFlyingTypeModalContext";
import FullLogo from "@/components/ui/fulllogo";

export default function FlyingTypeFormModal() {
  const { isOpen, editingType, viewingType, isViewMode, closeModal } = useFtw12sqnFlyingTypeModal();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingType) {
        await ftw12sqnFlyingTypeService.update(editingType.id, formData);
      } else {
        await ftw12sqnFlyingTypeService.create(formData);
      }
      closeModal();
      window.dispatchEvent(new CustomEvent('flyingTypeUpdated'));
    } catch (err: any) {
      setError(err.message || "Failed to save flying type");
    } finally {
      setLoading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="p-8">
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            {isViewMode ? "View Flying Type" : editingType ? "Edit Flying Type" : "Add a New Flying Type"}
          </h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {isViewMode ? "Flying type details" : editingType ? "Update flying type details" : "Configure your new flying type details"}
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              {isViewMode ? (
                ...
                <Input value={formData.type_name} onChange={(e) => handleChange("type_name", e.target.value)} placeholder="e.g. Mission, Exam" required />
              )}
            </div>
            <div>
              ...
              {isViewMode ? (
                ...
                <Input value={formData.type_code} onChange={(e) => handleChange("type_code", e.target.value)} placeholder="e.g. mission, exam" required />
              )}
            </div>
          </div>
              ...
              <div className="space-y-3">
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">This flying type will be available for use throughout the system.</div>
                  </div>
                </label>
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 dark:text-gray-400">This flying type will be hidden from general use.</div>
                  </div>
                </label>
              </div>
            )}
        ...
        <div className="flex items-center justify-end gap-3 mt-2 pt-2">
          ...
          {!isViewMode && (
            <button type="submit" className="px-6 py-2 bg-blue-500 text-white rounded-xl" disabled={loading}>
              {loading ? "Saving..." : editingType ? "Update Flying Type" : "Save Flying Type"}
            </button>
          )}
        </div>
      </form>
