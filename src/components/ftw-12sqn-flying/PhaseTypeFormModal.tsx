...
import { useFtw12sqnFlyingPhaseTypeModal } from "@/context/Ftw12sqnFlyingPhaseTypeModalContext";
import FullLogo from "@/components/ui/fulllogo";

export default function PhaseTypeFormModal() {
  const { isOpen, editingType, viewingType, isViewMode, closeModal } = useFtw12sqnFlyingPhaseTypeModal();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingType) {
        await ftw12sqnFlyingPhaseTypeService.update(editingType.id, formData);
      } else {
        await ftw12sqnFlyingPhaseTypeService.create(formData);
      }
      closeModal();
