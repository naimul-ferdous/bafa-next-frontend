...
import { useFtw12sqnExerciseModal } from "@/context/Ftw12sqnExerciseModalContext";
...
export default function ExerciseFormModal() {
  ...
  useEffect(() => {
    ...
    ftw12sqnFlyingPhaseTypeService
      .getList()
      .then(setPhaseTypes)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const syllabus = await ftw12sqnFlyingSyllabusService.get(syllabusId);
      ...
      if (editingExercise) {
        ...
        if (phaseTypeChanged && oldPhaseTypeId) {
          ...
          syllabusTypes = syllabusTypes.map(st => {
            if (st.ftw_12sqn_flying_phase_type_id === oldPhaseTypeId) {
          ...
          const targetIdx = syllabusTypes.findIndex(
            st => st.ftw_12sqn_flying_phase_type_id === selectedPhaseTypeId
          );
          if (targetIdx >= 0) {
            ...
            syllabusTypes.push({
              ftw_12sqn_flying_phase_type_id: selectedPhaseTypeId,
              is_active: true,
          ...
          syllabusTypes = syllabusTypes.map(st => {
            if (st.ftw_12sqn_flying_phase_type_id === selectedPhaseTypeId) {
      ...
      // 3. Send full update
      await ftw12sqnFlyingSyllabusService.update(syllabusId, {
