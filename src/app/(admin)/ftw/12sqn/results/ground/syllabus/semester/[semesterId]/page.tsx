...
export default function Ftw12sqnDetailedGroundSyllabusPage() {
    ...
    const [syllabusData, setSyllabusData] = useState<Ftw12sqnGroundSyllabus[]>([]);
    ...
    const handleMoveUp = async (index: number) => {
        ...
        try {
            await Promise.all([
                ftw12sqnGroundSyllabusService.update(currentRow.id, { ground_sort: newCurrentSort }),
                ftw12sqnGroundSyllabusService.update(prevRow.id, { ground_sort: newPrevSort }),
            ]);
            await loadSyllabus();
        } catch (error) {
    ...
    const handleMoveDown = async (index: number) => {
        ...
        try {
            await Promise.all([
                ftw12sqnGroundSyllabusService.update(currentRow.id, { ground_sort: newCurrentSort }),
                ftw12sqnGroundSyllabusService.update(nextRow.id, { ground_sort: newNextSort }),
            ]);
            await loadSyllabus();
        } catch (error) {
            console.error("Failed to reorder:", error);
                ...
                <div className="flex items-center gap-4 mb-6">
                    <div className="flex-1 text-center">
                        ...
                        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">
                            FTW 12 SQN Ground Syllabus - {semesterName}
                        </h2>
                    </div>
                </div>
