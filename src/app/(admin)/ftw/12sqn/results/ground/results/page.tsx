...
import type { Ftw12sqnGroundExaminationMark } from "@/libs/types/ftw12sqnExamination";
...
export default function Ftw12sqnGroundExaminationMarksPage() {
  ...
  // ── Instructor view ────────────────────────────────────────────────────
  const [marks, setMarks] = useState<Ftw12sqnGroundExaminationMark[]>([]);
  ...
  const [deletingMark, setDeletingMark] = useState<Ftw12sqnGroundExaminationMark | null>(null);
  ...
  const loadMarks = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ftw12sqnGroundExaminationMarkService.getAllMarks({
        page: currentPage,
        per_page: perPage,
  ...
  // ── Delete ─────────────────────────────────────────────────────────────
  const handleDeleteMark = (mark: Ftw12sqnGroundExaminationMark) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ftw12sqnGroundExaminationMarkService.deleteMark(deletingMark.id);
      await loadMarks();
  ...
  // ── Instructor columns ─────────────────────────────────────────────────
  const markColumns: Column<Ftw12sqnGroundExaminationMark>[] = [
    {
    ...
    },
    {
      ...
      render: (m) => (
        <div>
          <div className="font-medium text-gray-900">{m.syllabus?.ground_shortname || "N/A"}</div>
    ...
    },
    {
      ...
      render: (m) => (
        <div className="flex items-center justify-center gap-1" onClick={(e) => e.stopPropagation()}>
          <button onClick={() => router.push(`/ftw/12sqn/results/ground/results/${m.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded" title="View">
            <Icon icon="hugeicons:view" className="w-4 h-4" />
          </button>
          <button onClick={() => router.push(`/ftw/12sqn/results/ground/results/${m.id}/edit`)} className="p-1 text-yellow-600 hover:bg-yellow-50 rounded" title="Edit">
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-center">
          <button
            onClick={() => router.push(`/ftw/12sqn/results/ground/results/course/${row.course_id}/semester/${row.semester_id}`)}
    ...
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          {isInstructor && (
            <button
              onClick={() => router.push("/ftw/12sqn/results/ground/results/create")}
      ...
      {isInstructor ? (
        ...
        groupedLoading ? <TableLoading /> : (
          <DataTable
            ...
            onRowClick={(row) => router.push(`/ftw/12sqn/results/ground/results/course/${row.course_id}/semester/${row.semester_id}`)}
          />
        )
      )}
