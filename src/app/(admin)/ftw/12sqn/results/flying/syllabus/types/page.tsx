...
import { Ftw12sqnFlyingType, Ftw12sqnFlyingPhaseType } from "@/libs/types/ftw12sqnFlying";
...
function FlyingTypesPageContent() {
    ...
    const { openModal: openPhaseTypeModal, openViewModal: openPhaseTypeViewModal } = useFtw12sqnFlyingPhaseTypeModal();
    ...
    const [deletingFlyingType, setDeletingFlyingType] = useState<Ftw12sqnFlyingType | null>(null);
    ...
    const [deletingPhaseType, setDeletingPhaseType] = useState<Ftw12sqnFlyingPhaseType | null>(null);
    ...
    const loadFlyingTypes = useCallback(async () => {
        try {
            ...
            const response = await ftw12sqnFlyingTypeService.getAll({ per_page: 10000 });
    ...
    const loadPhaseTypes = useCallback(async () => {
        try {
            ...
            const response = await ftw12sqnFlyingPhaseTypeService.getAll({ per_page: 10000 });
    ...
    // Flying Type Actions
    const handleDeleteFlyingType = (type: Ftw12sqnFlyingType) => { setDeletingFlyingType(type); setFlyingTypeDeleteModalOpen(true); };
    const confirmDeleteFlyingType = async () => {
        if (!deletingFlyingType) return;
        try {
            setFlyingTypeDeleteLoading(true);
            await ftw12sqnFlyingTypeService.delete(deletingFlyingType.id);
    ...
    // Phase Type Actions
    const handleDeletePhaseType = (type: Ftw12sqnFlyingPhaseType) => { setDeletingPhaseType(type); setPhaseTypeDeleteModalOpen(true); };
    const confirmDeletePhaseType = async () => {
        if (!deletingPhaseType) return;
        try {
            setPhaseTypeDeleteLoading(true);
            await ftw12sqnFlyingPhaseTypeService.delete(deletingPhaseType.id);
...
export default function FlyingTypesPage() {
    return (
        <Ftw12sqnFlyingTypeModalProvider>
            <Ftw12sqnFlyingPhaseTypeModalProvider>
