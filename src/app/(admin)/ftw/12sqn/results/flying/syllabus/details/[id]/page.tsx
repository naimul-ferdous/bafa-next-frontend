...
export default function Ftw12sqnCourseDetailedSyllabusPage() {
    ...
    const loadCourseData = useCallback(async () => {
        try {
            setLoading(true);
            const data = await ftw12sqnFlyingSyllabusService.getCourseGrouped({ course_id: courseId });
            ...
            <div className="p-8 cv-content">
                <div className="mb-8">
                    ...
                    <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
                        Official Flying Training Syllabus - FTW 12 SQN
                    </p>
                    <div className="text-center">
                        <span className="inline-block px-6 py-1 bg-gray-100 rounded-full font-black text-gray-700 uppercase text-sm border border-gray-200">
                    ...
                    {courseData.semesters.map((semesterGroup: any, index: number) => (
                        <div key={index} className="page-break-before-always space-y-8">
                            <div className="space-y-12">
                                {semesterGroup.syllabus.sort((a: any, b: any) => a.phase_sort - b.phase_sort).map((phase: any) => {
                                    // Flatten exercises from all types (Dual, Solo) for this phase
                                    const allExercises: any[] = [];
                                    phase.syllabus_types?.forEach((type: any) => {
                                        type.exercises?.forEach((ex: any) => {
                                            allExercises.push({
                                                ...ex,
                ...
                <div className="pt-20 text-center border-t border-gray-100">
                    <div className="grid grid-cols-3 text-center mb-16 no-print">
                        ...
                        <div className="space-y-12">
                            <div className="h-0.5 w-40 bg-black mx-auto"></div>
                            <div className="font-bold text-black uppercase text-xs">Chief Flying Instructor</div>
                        </div>
                    </div>
