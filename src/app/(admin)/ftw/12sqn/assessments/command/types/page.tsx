...
import { Ftw12SqnAssessmentCommandType } from "@/libs/types/ftw12sqnAssessmentCommand";
...
export default function Ftw12SqnAssessmentCommandTypesPage() {
  const router = useRouter();
  const can = useCan();

  const [types, setTypes] = useState<Ftw12SqnAssessmentCommandType[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deletingType, setDeletingType] = useState<Ftw12SqnAssessmentCommandType | null>(null);
  ...
  const loadTypes = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw12sqnAssessmentCommandTypeService.getAllTypes({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddType = () => router.push("/ftw/12sqn/assessments/command/types/create");
  const handleEditType = (type: Ftw12SqnAssessmentCommandType) => router.push(`/ftw/12sqn/assessments/command/types/${type.id}/edit`);
  const handleViewType = (type: Ftw12SqnAssessmentCommandType) => router.push(`/ftw/12sqn/assessments/command/types/${type.id}`);
  const handleDeleteType = (type: Ftw12SqnAssessmentCommandType) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ftw12sqnAssessmentCommandTypeService.deleteType(deletingType.id);
      await loadTypes();
  ...
  const columns: Column<Ftw12SqnAssessmentCommandType>[] = [
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">12Sqn Assessment Command Types</h2>
      </div>
