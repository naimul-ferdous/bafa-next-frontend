...
import type { Ftw12SqnAssessmentCommandType, Ftw12SqnAssessmentCommandTypeEstimatedMark } from "@/libs/types/ftw12sqnAssessmentCommand";

export default function CommandTypeDetailsPage() {
  const router = useRouter();
  const params = useParams();
  const typeId = params?.id as string;

  const [type, setType] = useState<Ftw12SqnAssessmentCommandType | null>(null);
  ...
  const loadType = async () => {
    try {
      setLoading(true);
      const data = await ftw12sqnAssessmentCommandTypeService.getType(parseInt(typeId));
      if (data) {
        // Sort estimated marks by order
        if (data.estimated_marks) {
          data.estimated_marks.sort((a: Ftw12SqnAssessmentCommandTypeEstimatedMark, b: Ftw12SqnAssessmentCommandTypeEstimatedMark) => (a.order || 0) - (b.order || 0));
        }
        setType(data);
      } else {
      ...
      setError("Failed to load Command type data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      ...
      const success = await ftw12sqnAssessmentCommandTypeService.reorderEstimatedMarks(type.id, orders);
      if (!success) {
        loadType(); // Revert
      }
    } catch (err) {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <p className="text-red-600">{error || "Command Type not found"}</p>
          <button
            onClick={() => router.push("/ftw/12sqn/assessments/command/types")}
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
            Command Type Details - {type.type_name}
          </p>
        </div>
