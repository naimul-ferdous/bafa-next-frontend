...
export default function EditCommandTypePage() {
  ...
  useEffect(() => {
    const loadType = async () => {
      try {
        setLoadingType(true);
        const typeData = await ftw12sqnAssessmentCommandTypeService.getType(parseInt(typeId));
        if (typeData) {
          setType(typeData);
        } else {
  ...
  const handleSubmit = async (data: Ftw12SqnAssessmentCommandTypeCreateData) => {
    setLoading(true);
    try {
      await ftw12sqnAssessmentCommandTypeService.updateType(parseInt(typeId), data);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ftw/12sqn/assessments/command/types")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
