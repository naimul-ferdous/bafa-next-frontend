...
import { Ftw12SqnAssessmentCommandResult } from "@/libs/types/ftw12sqnAssessmentCommand";
...
export default function Ftw12SqnViewAssessmentCommandResultsPage() {
  const router = useRouter();
  const { user } = useAuth();
  const can = useCan();

  const [hasCommandAssign, setHasCommandAssign] = useState(false);
  const [results, setResults] = useState<Ftw12SqnAssessmentCommandResult[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deletingResult, setDeletingResult] = useState<Ftw12SqnAssessmentCommandResult | null>(null);
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await Ftw12SqnAssessmentCommandResultService.getAllResults({
        page: currentPage,
        per_page: perPage,
      });
      ...
      console.error("Failed to load Command results:", error);
    } finally {
      setLoading(false);
    }
  }, [currentPage, perPage]);
  ...
  // Load Command assigns for current user
  useEffect(() => {
    if (!user?.id) return;
    ftw12sqnUserAssignService.getAll({ user_id: user.id }).then((data) => {
      console.log("Command assigns for user", user.id, data.command);
      setHasCommandAssign((data.command?.length ?? 0) > 0);
    });
  }, [user?.id]);

  const handleAddResult = () => router.push("/ftw/12sqn/assessments/command/results/create");
  const handleEditResult = (result: Ftw12SqnAssessmentCommandResult) => router.push(`/ftw/12sqn/assessments/command/results/${result.id}/edit`);
  const handleViewResult = (result: Ftw12SqnAssessmentCommandResult) => router.push(`/ftw/12sqn/assessments/command/results/${result.id}`);
  const handleDeleteResult = (result: Ftw12SqnAssessmentCommandResult) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await Ftw12SqnAssessmentCommandResultService.deleteResult(deletingResult.id);
      await loadResults();
      setDeleteModalOpen(false);
      setDeletingResult(null);
    } catch (error) {
      console.error("Failed to delete Command result:", error);
      alert("Failed to delete Command result");
    } finally {
      setDeleteLoading(false);
    }
  };

  const handleExport = () => console.log("Export Command results");
  ...
  const columns: Column<Ftw12SqnAssessmentCommandResult>[] = [
    { key: "id", header: "SL.", headerAlign:"center", className: "text-center text-gray-900", render: (result, index) => (pagination.from || 0) + (index) },
    {
      ...
      render: (result) => result.course?.name || "â€”"
    },
    {
      key: "semester",
      header: "Semester",
      className: "text-gray-700",
      render: (result) => result.semester?.name || "â€”"
    },
    {
      key: "Command_type",
      header: "Command Type",
      className: "text-gray-700",
      render: (result) => result.command_type?.type_name || "â€”"
    },
    {
      key: "result_cadets",
      header: "Cadets",
    ...
    },
    {
      ...
      render: (result) => result.created_at ? new Date(result.created_at).toLocaleDateString("en-GB") : "â€”"
    },
    {
      key: "actions",
      header: "Actions",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">12Sqn Assessment Command Results</h2>
      </div>

      <div className="flex items-center justify-end gap-4 mb-6">
        <div className="flex items-center gap-3">
          {can('add') && hasCommandAssign && (
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No Command results found"
      ...
      <ConfirmationModal
        ...
        title="Delete Command Result"
        message={`Are you sure you want to delete this Command result? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
