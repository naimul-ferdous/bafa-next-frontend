...
import { Ftw12SqnAssessmentCommandResultService as ftw12sqnAssessmentCommandResultService } from "@/libs/services/ftw12sqnAssessmentCommandResultService";
import { ftw12sqnCommandSemesterApprovalService } from "@/libs/services/ftw12sqnCommandSemesterApprovalService";
import { ftw12sqnCommandApprovalAuthorityService } from "@/libs/services/ftw12sqnCommandApprovalAuthorityService";
import { useAuth } from "@/context/AuthContext";
import FullLogo from "@/components/ui/fulllogo";
import { getOrdinal } from "@/libs/utils/formatter";
import type { Ftw12SqnAssessmentCommandResult } from "@/libs/types/ftw12sqnAssessmentCommand";
import type { Ftw12SqnCommandCadetApproval } from "@/libs/types/ftw12sqnCommandCadetApproval";
...
import { Ftw12SqnCommandSemesterApproval as Ftw12SqnCommandSemesterApprovalType } from "@/libs/types/ftw12sqnCommandSemesterApproval";
...
export default function CommandResultDetailsPage() {
  ...
  const [approvals, setApprovals] = useState<Ftw12SqnCommandCadetApproval[]>([]);
  const [semesterApprovals, setSemesterApprovals] = useState<Ftw12SqnCommandSemesterApprovalType[]>([]);
  const [semesterApproval, setSemesterApproval] = useState<Ftw12SqnCommandSemesterApprovalType | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [data, authRes] = await Promise.all([
        ftw12sqnAssessmentCommandResultService.getResult(parseInt(resultId)),
        ftw12sqnCommandApprovalAuthorityService.getAuthorities({ allData: true, is_active: true }),
      ]);
  ...
  const getCadetApproval = (cadetId: number, authId: number): Ftw12SqnCommandCadetApproval | undefined =>
    approvals.find((a) => a.cadet_id === cadetId && a.authority_id === authId);

  const getCurrentCadetApproval = (cadetId: number): Ftw12SqnCommandCadetApproval | undefined =>
  ...
  const confirmBulkAction = async () => {
    ...
    try {
      if (approvalModal.status === "approved") {
        await ftw12sqnAssessmentCommandResultService.bulkApprove({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
        });
      } else {
        await ftw12sqnAssessmentCommandResultService.bulkReject({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
  ...
  const confirmForward = async () => {
    ...
    try {
      await ftw12sqnCommandSemesterApprovalService.store({
  ...
  const calculateTotal = useCallback((cadetMarks: any[]) => {
    ...
    estimatedMarks.forEach((em) => {
      const mark = cadetMarks.find((m: any) => m.ftw_12sqn_assessment_command_type_estimated_mark_id === em.id);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/Ftw12Sqn/assessments/Command/results")}
                    ...
                    return (
                      <tr key={cadet.cadet_id}>
                        ...
                        {estimatedMarks.map((em, i) => {
                          const mark = cadet.marks?.find((m: any) => m.ftw_12sqn_assessment_command_type_estimated_mark_id === em.id);
