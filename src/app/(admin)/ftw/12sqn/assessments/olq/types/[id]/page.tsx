...
import type { Ftw12SqnAssessmentOlqType, Ftw12SqnAssessmentOlqTypeEstimatedMark } from "@/libs/types/ftw12sqnAssessmentOlq";
...
export default function OlqTypeDetailsPage() {
  ...
  const loadType = async () => {
    try {
      setLoading(true);
      const data = await ftw12sqnAssessmentOlqTypeService.getType(parseInt(typeId));
      if (data) {
        // Sort estimated marks by order
        if (data.estimated_marks) {
          data.estimated_marks.sort((a: Ftw12SqnAssessmentOlqTypeEstimatedMark, b: Ftw12SqnAssessmentOlqTypeEstimatedMark) => (a.order || 0) - (b.order || 0));
        }
        setType(data);
      } else {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      ...
      const success = await ftw12sqnAssessmentOlqTypeService.reorderEstimatedMarks(type.id, orders);
      if (!success) {
        loadType(); // Revert
      }
    } catch (err) {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw/12sqn/assessments/olq/types")}
