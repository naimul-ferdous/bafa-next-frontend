...
export default function Ftw12sqnOlqResultDetailsPage() {
  ...
  useEffect(() => {
    const loadResult = async () => {
      try {
        setLoading(true);
        const data = await ftw12sqnAssessmentOlqResultService.getResult(parseInt(resultId));
        if (data) {
          setResult(data);
        } else {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw/12sqn/assessments/olq/results")}
  ...
  const calculateTotal = (cadetMarks: { achieved_mark: number | string; ftw_12sqn_assessment_olq_type_estimated_mark_id: number }[]) => {
    // Calculate: sum of (estimated_mark * achieved_mark)
    let total = 0;
    estimatedMarks.forEach(em => {
      const mark = cadetMarks.find(m => m.ftw_12sqn_assessment_olq_type_estimated_mark_id === em.id);
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push(`/ftw/12sqn/assessments/olq/results/${result.id}/edit`)}
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
            FTW 12sqn OLQ Assessment Result - {result.olq_type?.type_name}
          </p>
        </div>
                    ...
                    return (
                      <tr key={cadet.cadet_id}>
                        ...
                        {estimatedMarks.map((em) => {
                          const mark = cadet.marks?.find(m => m.ftw_12sqn_assessment_olq_type_estimated_mark_id === em.id);
