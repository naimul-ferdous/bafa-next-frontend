...
import { ftw12sqnOlqSemesterApprovalService } from "@/libs/services/ftw12sqnOlqSemesterApprovalService";
import { ftw12sqnOlqApprovalAuthorityService } from "@/libs/services/ftw12sqnOlqApprovalAuthorityService";
...
import type { Ftw12SqnOlqCadetApproval } from "@/libs/types/ftw12sqnOlqCadetApproval";
...
export default function OlqResultDetailsPage() {
  ...
  const [approvals, setApprovals] = useState<Ftw12SqnOlqCadetApproval[]>([]);
  const [semesterApprovals, setSemesterApprovals] = useState<Ftw12SqnOlqSemesterApproval[]>([]);
  const [semesterApproval, setSemesterApproval] = useState<Ftw12SqnOlqSemesterApproval | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [data, authRes] = await Promise.all([
        ftw12sqnAssessmentOlqResultService.getResult(parseInt(resultId)),
        ftw12sqnOlqApprovalAuthorityService.getAuthorities({ allData: true, is_active: true }),
      ]);
  ...
  const getCadetApproval = (cadetId: number, authId: number): Ftw12SqnOlqCadetApproval | undefined =>
    approvals.find((a) => a.cadet_id === cadetId && a.authority_id === authId);

  const getCurrentCadetApproval = (cadetId: number): Ftw12SqnOlqCadetApproval | undefined =>
  ...
  const confirmBulkAction = async () => {
    ...
    try {
      if (approvalModal.status === "approved") {
        await ftw12sqnAssessmentOlqResultService.bulkApprove({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
        });
      } else {
        await ftw12sqnAssessmentOlqResultService.bulkReject({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
  ...
  const confirmForward = async () => {
    ...
    try {
      await ftw12sqnOlqSemesterApprovalService.store({
  ...
  const calculateTotal = useCallback((cadetMarks: any[]) => {
    ...
    estimatedMarks.forEach((em) => {
      const mark = cadetMarks.find((m: any) => m.ftw_12sqn_assessment_olq_type_estimated_mark_id === em.id);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw12sqn/assessments/olq/results")}
                    ...
                    return (
                      <tr key={cadet.cadet_id}>
                        ...
                        {estimatedMarks.map((em, i) => {
                          const mark = cadet.marks?.find((m: any) => m.ftw_12sqn_assessment_olq_type_estimated_mark_id === em.id);
