...
export default function EditResultPage() {
  ...
  useEffect(() => {
    const loadResult = async () => {
      try {
        ...
        const resultData = await ftw12sqnAssessmentCounselingResultService.getResult(parseInt(resultId));
        if (resultData) {
  ...
  const handleSubmit = async (data: any) => {
    setLoading(true);
    try {
      await ftw12sqnAssessmentCounselingResultService.updateResult(parseInt(resultId), data);
      router.push("/ftw/12sqn/assessments/counselings/results");
    } catch (err: any) {
      throw err;
    } finally {
      setLoading(false);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ftw/12sqn/assessments/counselings/results")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
            Back to Results
          </button>
        </div>
      </div>
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">Edit FTW 12sqn Assessment Counseling Results</h2>
      </div>
