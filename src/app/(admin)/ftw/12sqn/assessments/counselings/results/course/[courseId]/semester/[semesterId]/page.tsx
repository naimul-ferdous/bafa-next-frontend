...
export default function CounselingCourseSemesterResultPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  const handleEdit = (result: Ftw12sqnAssessmentCounselingResult) => {
    router.push(`/ftw/12sqn/assessments/counselings/results/${result.id}/edit`);
  };

  const handleView = (result: Ftw12sqnAssessmentCounselingResult) => {
    router.push(`/ftw/12sqn/assessments/counselings/results/${result.id}`);
  };

  const handleDelete = (result: Ftw12sqnAssessmentCounselingResult) => {
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2 underline decoration-2 underline-offset-4">
            FTW 12SQN Counseling Assessment Summary Sheet
          </p>
        </div>
