...
export default function ResultDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const consolidated = await ftw12sqnAssessmentCounselingResultService.getConsolidatedResults({
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw12sqn/assessments/counselings/results")}
                ...
                <table className="w-full border-collapse border border-black text-sm">
                  ...
                  <tbody>
                    {availableEvents.map((event: any, eventIdx: number) => {
                      const cadetRemark = (rc.remarks || []).find((r: any) => r.ftw_12sqn_assessment_counseling_event_id === event.id)
                        ?? (result.remarks || []).find((r: any) => r.ftw_12sqn_assessment_counseling_event_id === event.id);
                      return (
                          ...
                          {eventIdx === 0 && (
                            <>
                              ...
                              <td rowSpan={availableEvents.length || 1} className="px-2 py-4 border-r border-black align-middle text-center bg-white font-mono">
                                ...
                                <p className="text-[12px]">{`Counselling has been shown and understood by the Officer Cadet.`}</p>
                              </td>
                              {/* Counseling Officer */}
                              <td rowSpan={availableEvents.length || 1} className="px-2 py-4 text-gray-900 border-r border-black align-middle bg-white">
                                <SignatureBlock
                                  ...
                                  designation={instructorAuthority?.role?.name || (Array.isArray((result.instructor as any)?.roles) ? (result.instructor as any)?.roles?.find((r: any) => r.pivot?.is_primary)?.name : undefined)}
