...
export default function Ftw12SqnViewCadetWarningsPage() {
  const router = useRouter();
  const can = useCan("/ftw12sqn/assessments/warnings");
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw12sqnCadetWarningService.getGroupedResults({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddWarning = () => router.push("/ftw12sqn/assessments/warnings/create");
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          {can("view") && (
            <button
              onClick={() => router.push(`/ftw12sqn/assessments/warnings/course/${row.course_id}/semester/${row.semester_id}`)}
