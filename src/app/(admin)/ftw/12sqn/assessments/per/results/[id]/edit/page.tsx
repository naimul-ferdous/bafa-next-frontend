...
export default function EditResultPage() {
  ...
  useEffect(() => {
    const loadResult = async () => {
      try {
        ...
        const resultData = await ftw12sqnAssessmentPenpictureResultService.getResult(parseInt(resultId));
        if (resultData) {
  ...
  const handleSubmit = async (data: any) => {
    setLoading(true);
    try {
      await ftw12sqnAssessmentPenpictureResultService.updateResult(parseInt(resultId), data);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ftw12sqn/assessments/penpicture/results")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
            Back to Results
          </button>
        </div>
      </div>
