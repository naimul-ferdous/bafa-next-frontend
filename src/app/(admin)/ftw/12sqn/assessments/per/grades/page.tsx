...
import { Ftw12SqnAssessmentPenpictureGrade, SystemCourse, SystemSemester } from "@/libs/types/system";
...
export default function Ftw12SqnAssessmentPenpictureGradesPage() {
  ...
  const [deletingGrade, setDeletingGrade] = useState<Ftw12SqnAssessmentPenpictureGrade | null>(null);
  ...
  const loadGrades = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw12sqnAssessmentPenpictureGradeService.getAllGrades({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddGrade = () => router.push("/ftw12sqn/assessments/penpicture/grades/create");
  const handleEditGrade = (grade: Ftw12SqnAssessmentPenpictureGrade) => router.push(`/ftw12sqn/assessments/penpicture/grades/${grade.id}/edit`);
  const handleViewGrade = (grade: Ftw12SqnAssessmentPenpictureGrade) => router.push(`/ftw12sqn/assessments/penpicture/grades/${grade.id}`);
  const handleDeleteGrade = (grade: Ftw12SqnAssessmentPenpictureGrade) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ftw12sqnAssessmentPenpictureGradeService.deleteGrade(deletingGrade.id);
      await loadGrades();
  ...
  const columns: Column<Ftw12SqnAssessmentPenpictureGrade>[] = [
