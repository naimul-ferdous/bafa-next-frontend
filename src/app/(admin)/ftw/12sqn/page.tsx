...
const OrbitCircle = ({ title, href, icon, style, className }: OrbitCircleProps) => {
  return (
    <Link
      href={href}
      style={style}
      className={`absolute group w-28 h-28 sm:w-36 sm:h-36 md:w-40 md:h-40 rounded-full bg-white border-2 border-slate-200 shadow-xl flex items-center justify-center transition-all duration-500 hover:shadow-2xl hover:scale-120 hover:border-orange-400 overflow-hidden z-20 ${className || ""}`}
    >
      <div className="absolute inset-0 transition-transform duration-700 group-hover:scale-120">
      ...
      <div className="relative z-10 flex flex-col items-center gap-1.5 text-center">
        <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-xl bg-white/90 backdrop-blur-sm flex items-center justify-center group-hover:bg-orange-600 transition-all duration-300 shadow-md border border-slate-100">
...
const StatusCard = ({ title, subtitle, href, icon, accentColor, bgImage }: StatusCardProps) => {
  return (
    ...
    >
      <div className="absolute inset-y-0 right-0 w-1/2 pointer-events-none transition-transform duration-700 group-hover:scale-120">
        <Image src={`/images/bg/${bgImage}`} alt="" fill className="object-cover object-right" priority />
        <div className="absolute inset-0 bg-gradient-to-r from-white via-white/20 to-transparent" />
      </div>
      <div className="relative z-10">
        <div className={`w-12 h-12 sm:w-12 sm:h-12 lg:w-14 lg:h-14 rounded-2xl bg-gradient-to-br ${accentColor} flex items-center justify-center shadow-lg transform transition-transform duration-500 group-hover:rotate-[10deg] group-hover:scale-120`}>
  ...
  return (
    <div className="relative bg-white rounded-[1.5rem] sm:rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full">
      <div className="relative px-4 sm:px-6 pt-4 sm:pt-6 pb-3 sm:pb-4 border-b border-slate-100 overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center shadow-md">
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-slate-50 px-1.5 sm:px-2 py-1.5 sm:py-2">
        {demoNotices.map((notice) => (
          <div key={notice.id} className="group relative flex gap-2.5 sm:gap-3 px-3 sm:px-4 py-3 sm:py-4 rounded-xl sm:rounded-2xl hover:bg-slate-50 transition-colors duration-300 cursor-pointer">
            {notice.isNew && <span className="absolute top-3 sm:top-4 right-3 sm:right-4 w-2 h-2 rounded-full bg-orange-500" />}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[12px] sm:text-xs font-black text-slate-800 leading-snug group-hover:text-orange-700 transition-colors">{notice.title}</p>
      ...
      <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-slate-100">
        <Link href="/notices" className="group flex items-center justify-center gap-2 w-full py-2 sm:py-2.5 rounded-xl bg-slate-50 hover:bg-orange-50 border border-slate-100 hover:border-orange-200 transition-all duration-300">
          <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-slate-500 group-hover:text-orange-600 transition-colors">View All Notices</span>
          <Icon icon="hugeicons:arrow-right-02" className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-400 group-hover:text-orange-500 transition-colors" />
        </Link>
      </div>
    </div>
  );
...
const nodes = [
  ...
  { title: "OLQ", href: "/ftw/12sqn/assessments/olq/results", icon: "hugeicons:star", angle: 30 },
  { title: "Counseling", href: "/ftw/12sqn/assessments/counseling/results", icon: "hugeicons:user-multiple", angle: 90 },
...
const statusCards: StatusCardProps[] = [
  { title: "OLQ Assessment", subtitle: "Officer Qualities", href: "/ftw/12sqn/assessments/olq/results", icon: "hugeicons:star", accentColor: "from-orange-600 to-amber-700", bgImage: "corner-1.png" },
  { title: "Counseling", subtitle: "Pilot Mentoring", href: "/ftw/12sqn/assessments/counseling/results", icon: "hugeicons:user-multiple", accentColor: "from-orange-500 to-amber-600", bgImage: "corner-2.png" },
  { title: "Syllabus", subtitle: "Sortie Matrix", href: "/ftw/12sqn/results/flying/syllabus", icon: "hugeicons:book-02", accentColor: "from-orange-700 to-red-700", bgImage: "corner-1.png" },
];
