...
import { ftw12sqnPenpictureAuthorityService } from "@/libs/services/ftw12sqnPenpictureAuthorityService";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import Ftw12SqnPenpictureAuthorityModal from "@/components/ftw-12sqn-results/Ftw12SqnPenpictureAuthorityModal";
import Pagination from "@/components/ui/Pagination";
import { useCan } from "@/context/PagePermissionsContext";

export default function Ftw12SqnPenpictureAuthoritiesPage() {
  const can = useCan();
  const [authorities, setAuthorities] = useState<Ftw12SqnPenpictureAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<Ftw12SqnPenpictureAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<Ftw12SqnPenpictureAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw12sqnPenpictureAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
  ...
  const handleEditAuthority = (authority: Ftw12SqnPenpictureAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: Ftw12SqnPenpictureAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ftw12sqnPenpictureAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<Ftw12SqnPenpictureAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      ...
      <Ftw12SqnPenpictureAuthorityModal
        isOpen={modalOpen}
