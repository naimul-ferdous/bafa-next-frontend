...
import { Ftw12SqnOlqApprovalAuthority } from "@/libs/types/ftw12sqnOlqApprovalAuthority";
...
import Ftw12SqnOlqApprovalAuthorityModal from "@/components/ftw-12sqn-results/Ftw12SqnOlqApprovalAuthorityModal";
import Pagination from "@/components/ui/Pagination";
// import { useCan } from "@/context/PagePermissionsContext";

export default function Ftw12SqnOlqApprovalAuthoritiesPage() {
  // const can = useCan();
  const [authorities, setAuthorities] = useState<Ftw12SqnOlqApprovalAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<Ftw12SqnOlqApprovalAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<Ftw12SqnOlqApprovalAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw12sqnOlqApprovalAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
  ...
  const handleEditAuthority = (authority: Ftw12SqnOlqApprovalAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: Ftw12SqnOlqApprovalAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ftw12sqnOlqApprovalAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<Ftw12SqnOlqApprovalAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">FTW 12SQN OLQ Assessment Approval Authority Management</h2>
      </div>
      ...
      <Ftw12SqnOlqApprovalAuthorityModal
        isOpen={modalOpen}
