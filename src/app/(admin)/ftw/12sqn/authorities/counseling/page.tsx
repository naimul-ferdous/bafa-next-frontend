...
import { Ftw12SqnCounselingApprovalAuthority } from "@/libs/types/ftw12sqnCounselingApprovalAuthority";
import { Icon } from "@iconify/react";
import { ftw12sqnCounselingApprovalAuthorityService } from "@/libs/services/ftw12sqnCounselingApprovalAuthorityService";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import Ftw12SqnCounselingApprovalAuthorityModal from "@/components/ftw-12sqn-results/Ftw12SqnCounselingApprovalAuthorityModal";
import Pagination from "@/components/ui/Pagination";
// import { useCan } from "@/context/PagePermissionsContext";

export default function Ftw12SqnCounselingApprovalAuthoritiesPage() {
  // const can = useCan();
  const [authorities, setAuthorities] = useState<Ftw12SqnCounselingApprovalAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<Ftw12SqnCounselingApprovalAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<Ftw12SqnCounselingApprovalAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw12sqnCounselingApprovalAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
      ...
      console.error("Failed to load Counseling approval authorities:", error);
    } finally {
      setLoading(false);
    }
  ...
  const handleEditAuthority = (authority: Ftw12SqnCounselingApprovalAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: Ftw12SqnCounselingApprovalAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ftw12sqnCounselingApprovalAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<Ftw12SqnCounselingApprovalAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
    ...
    },
    {
      ...
      render: (auth) => (
        <div className="flex flex-wrap gap-1">
          ...
          {auth.is_signature && (
            <span className="px-1.5 py-0.5 text-[10px] bg-purple-50 text-purple-700 border border-purple-200 rounded">Signature</span>
          )}
          {!auth.is_cadet_approve && !auth.is_final && !auth.is_signature && (
            <span className="text-gray-400 text-xs">—</span>
          )}
        </div>
      ),
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">ATW Counseling Assessment Approval Authority Management</h2>
      </div>
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No Counseling approval authorities found"
        />
      )}
      ...
      <Ftw12SqnCounselingApprovalAuthorityModal
        isOpen={modalOpen}
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to ${statusAuthority?.is_active ? "deactivate" : "activate"} this Counseling approval authority?`}
