...
import { Ftw11SqnAssessmentPenpictureGrade, SystemCourse, SystemSemester } from "@/libs/types/system";
import { Icon } from "@iconify/react";
import { ftw11sqnAssessmentPenpictureGradeService } from "@/libs/services/ftw11sqnAssessmentPenpictureGradeService";
...
export default function Ftw11SqnAssessmentPenpictureGradesPage() {
  const router = useRouter();
  const { permissions } = usePageContext();
  const can = useCan();

  const [grades, setGrades] = useState<Ftw11SqnAssessmentPenpictureGrade[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deletingGrade, setDeletingGrade] = useState<Ftw11SqnAssessmentPenpictureGrade | null>(null);
  ...
  const loadGrades = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw11sqnAssessmentPenpictureGradeService.getAllGrades({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddGrade = () => router.push("/ftw11sqn/assessments/penpicture/grades/create");
  const handleEditGrade = (grade: Ftw11SqnAssessmentPenpictureGrade) => router.push(`/ftw11sqn/assessments/penpicture/grades/${grade.id}/edit`);
  const handleViewGrade = (grade: Ftw11SqnAssessmentPenpictureGrade) => router.push(`/ftw11sqn/assessments/penpicture/grades/${grade.id}`);
  const handleDeleteGrade = (grade: Ftw11SqnAssessmentPenpictureGrade) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ftw11sqnAssessmentPenpictureGradeService.deleteGrade(deletingGrade.id);
      await loadGrades();
  ...
  const columns: Column<Ftw11SqnAssessmentPenpictureGrade>[] = [
