...
export default function PenpictureCourseSemesterResultPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  const [results, setResults] = useState<Ftw11SqnAssessmentPenpictureResult[]>([]);
  ...
  const [deletingResult, setDeletingResult] = useState<Ftw11SqnAssessmentPenpictureResult | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const response = await ftw11sqnAssessmentPenpictureResultService.getAllResults({
        course_id: courseId,
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ftw11sqnAssessmentPenpictureResultService.deleteResult(deletingResult.id);
      await loadData();
              ...
              return (
                <div key={result.id} className="flex items-start gap-3">
                  ...
                  <div className="no-print flex items-center gap-1 shrink-0">
                    <button
                      onClick={() => router.push(`/ftw11sqn/assessments/penpicture/results/${result.id}/edit`)}
