...
export default function Ftw11SqnViewAssessmentPenpictureResultsPage() {
  const router = useRouter();
  const { user, userIsInstructor } = useAuth();
  const can = useCan("/ftw11sqn/assessments/penpicture/results");
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw11sqnAssessmentPenpictureResultService.getGroupedResults({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddResult = () => router.push("/ftw11sqn/assessments/penpicture/results/create");
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          {can("view") && (
            <button
              onClick={() => router.push(`/ftw11sqn/assessments/penpicture/results/course/${row.course_id}/semester/${row.semester_id}`)}
      ...
      {loading ? (
        ...
        <>
          <DataTable
            ...
            onRowClick={can("view") ? (row) => router.push(`/ftw11sqn/assessments/penpicture/results/course/${row.course_id}/semester/${row.semester_id}`) : undefined}
          />
