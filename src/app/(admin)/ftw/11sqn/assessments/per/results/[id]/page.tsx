...
import type { Ftw11SqnAssessmentPenpictureResult, Ftw11SqnAssessmentPenpictureGrade } from "@/libs/types/system";
...
export default function ResultDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [result, setResult] = useState<Ftw11SqnAssessmentPenpictureResult | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const resultData = await ftw11sqnAssessmentPenpictureResultService.getResult(resultId);

        if (resultData) {
          setResult(resultData);

          // Fetch all active grades for this course and semester
          const gradesData = await ftw11sqnAssessmentPenpictureGradeService.getActiveGrades({
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw11sqn/assessments/penpicture/results")}
  ...
  const isGradeSelected = (gradeId: number) => {
    return result.ftw11sqn_assessment_penpicture_grade_id === gradeId;
  };
