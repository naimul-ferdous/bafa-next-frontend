...
export default function Ftw11sqnOlqResultDetailsPage() {
  ...
  useEffect(() => {
    const loadResult = async () => {
      try {
        setLoading(true);
        const data = await ftw11sqnAssessmentOlqResultService.getResult(parseInt(resultId));
        if (data) {
          setResult(data);
        } else {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw/11sqn/assessments/olq/results")}
  ...
  const calculateTotal = (cadetMarks: { achieved_mark: number | string; ftw_11sqn_assessment_olq_type_estimated_mark_id: number }[]) => {
    // Calculate: sum of (estimated_mark * achieved_mark)
    let total = 0;
    estimatedMarks.forEach(em => {
      const mark = cadetMarks.find(m => m.ftw_11sqn_assessment_olq_type_estimated_mark_id === em.id);
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push(`/ftw/11sqn/assessments/olq/results/${result.id}/edit`)}
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
            FTW 11SQN OLQ Assessment Result - {result.olq_type?.type_name}
          </p>
        </div>
              ...
              <table className="w-full border-collapse border border-black text-sm">
                ...
                <tbody>
                  {result.result_cadets.map((cadet, index) => {
                    ...
                    return (
                      <tr key={cadet.cadet_id}>
                        ...
                        {estimatedMarks.map((em) => {
                          const mark = cadet.marks?.find(m => m.ftw_11sqn_assessment_olq_type_estimated_mark_id === em.id);
                        ...
                        <td className="border border-black px-2 py-2 text-center font-medium">
                          ...
                          {cadet.is_present && result.olq_type?.type_code?.toLowerCase() === "for_116b" && (
                  ...
                  <tr className="font-bold bg-gray-50">
                    <td className="border border-black px-3 py-2 text-center" colSpan={6}>Maximum Input / Estimated Mark</td>
                    {estimatedMarks.map(mark => (
                    ...
                    <td className="border border-black px-3 py-2 text-center">
                      ...
                      {result.olq_type?.type_code?.toLowerCase() === "for_116b" && (
                        <span className="block text-xs text-green-600">(x1.5)</span>
                      )}
                    </td>
                  </tr>
