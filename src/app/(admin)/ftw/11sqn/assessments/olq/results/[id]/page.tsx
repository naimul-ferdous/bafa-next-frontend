...
import { ftw11sqnOlqSemesterApprovalService } from "@/libs/services/ftw11sqnOlqSemesterApprovalService";
import { ftw11sqnOlqApprovalAuthorityService } from "@/libs/services/ftw11sqnOlqApprovalAuthorityService";
...
import type { Ftw11SqnOlqCadetApproval } from "@/libs/types/ftw11sqnOlqCadetApproval";
...
export default function OlqResultDetailsPage() {
  ...
  const [approvals, setApprovals] = useState<Ftw11SqnOlqCadetApproval[]>([]);
  const [semesterApprovals, setSemesterApprovals] = useState<Ftw11SqnOlqSemesterApproval[]>([]);
  const [semesterApproval, setSemesterApproval] = useState<Ftw11SqnOlqSemesterApproval | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [data, authRes] = await Promise.all([
        ftw11sqnAssessmentOlqResultService.getResult(parseInt(resultId)),
        ftw11sqnOlqApprovalAuthorityService.getAuthorities({ allData: true, is_active: true }),
      ]);
  ...
  const getCadetApproval = (cadetId: number, authId: number): Ftw11SqnOlqCadetApproval | undefined =>
    approvals.find((a) => a.cadet_id === cadetId && a.authority_id === authId);

  const getCurrentCadetApproval = (cadetId: number): Ftw11SqnOlqCadetApproval | undefined =>
  ...
  const confirmBulkAction = async () => {
    ...
    try {
      if (approvalModal.status === "approved") {
        await ftw11sqnAssessmentOlqResultService.bulkApprove({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
        });
      } else {
        await ftw11sqnAssessmentOlqResultService.bulkReject({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
  ...
  const confirmForward = async () => {
    ...
    try {
      await ftw11sqnOlqSemesterApprovalService.store({
  ...
  const calculateTotal = useCallback((cadetMarks: any[]) => {
    ...
    estimatedMarks.forEach((em) => {
      const mark = cadetMarks.find((m: any) => m.ftw_11sqn_assessment_olq_type_estimated_mark_id === em.id);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw11sqn/assessments/olq/results")}
                    ...
                    return (
                      <tr key={cadet.cadet_id}>
                        ...
                        {estimatedMarks.map((em, i) => {
                          const mark = cadet.marks?.find((m: any) => m.ftw_11sqn_assessment_olq_type_estimated_mark_id === em.id);
