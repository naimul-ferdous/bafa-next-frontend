...
export default function EditOlqTypePage() {
  ...
  useEffect(() => {
    const loadType = async () => {
      try {
        setLoadingType(true);
        const typeData = await ftw11sqnAssessmentOlqTypeService.getType(parseInt(typeId));
        if (typeData) {
          setType(typeData);
        } else {
  ...
  const handleSubmit = async (data: Ftw11sqnAssessmentOlqTypeCreateData) => {
    setLoading(true);
    try {
      await ftw11sqnAssessmentOlqTypeService.updateType(parseInt(typeId), data);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ftw/11sqn/assessments/olq/types")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
            Back to OLQ Types
          </button>
        </div>
      </div>
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">Edit FTW 11sqn OLQ Type</h2>
      </div>
