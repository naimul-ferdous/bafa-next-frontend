...
export default function Ftw11sqnWarningsCourseSemesterResultPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  const handleEdit = (result: CadetWarning) => {
    router.push(`/ftw/11sqn/assessments/warnings/${result.id}/edit`);
  };

  const handleView = (result: CadetWarning) => {
    router.push(`/ftw/11sqn/assessments/warnings/${result.id}`);
  };
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
            FTW 11SQN Cadet Warnings Summary Sheet
          </p>
        </div>
