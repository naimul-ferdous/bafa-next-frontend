...
export default function Ftw11SqnViewCadetWarningsPage() {
  const router = useRouter();
  const can = useCan("/ftw11sqn/assessments/warnings");
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw11sqnCadetWarningService.getGroupedResults({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddWarning = () => router.push("/ftw11sqn/assessments/warnings/create");
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          {can("view") && (
            <button
              onClick={() => router.push(`/ftw11sqn/assessments/warnings/course/${row.course_id}/semester/${row.semester_id}`)}
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase tracking-widest">ATW Cadet Warnings</h2>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          {can("add") && (
            <button onClick={handleAddWarning} className="px-4 py-2 rounded-lg text-white flex items-center gap-1 bg-blue-600 hover:bg-blue-700 transition-all shadow-md active:scale-95">
              <Icon icon="hugeicons:add-circle" className="w-4 h-4 mr-2" />Add Warning
            </button>
          )}
