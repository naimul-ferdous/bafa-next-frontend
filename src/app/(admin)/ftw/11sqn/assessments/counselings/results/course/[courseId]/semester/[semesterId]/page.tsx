...
import { ftw11sqnCounselingSemesterApprovalService } from "@/libs/services/ftw11sqnCounselingSemesterApprovalService";
...
import type { Ftw11SqnCounselingSemesterApproval } from "@/libs/types/ftw11sqnCounselingSemesterApproval";
...
export default function CounselingCourseSemesterResultPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  const { user } = useAuth();
  const can = useCan("/ftw/11sqn/assessments/counselings/results");
  const resolvedParams = use(params);
  const courseId = parseInt(resolvedParams.courseId);
  const semesterId = parseInt(resolvedParams.semesterId);

  const [results, setResults] = useState<Ftw11SqnAssessmentCounselingResult[]>([]);
  const [approvals, setApprovals] = useState<Ftw11SqnCounselingCadetApproval[]>([]);
  const [semesterApprovals, setSemesterApprovals] = useState<Ftw11SqnCounselingSemesterApproval[]>([]);
  const [semesterApproval, setSemesterApproval] = useState<Ftw11SqnCounselingSemesterApproval | null>(null);
  ...
  const [deletingResult, setDeletingResult] = useState<Ftw11SqnAssessmentCounselingResult | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await ftw11sqnAssessmentCounselingResultService.getConsolidatedResults({
        course_id: courseId,
      ...
      if (response) {
        ...
        const assigns = await ftw11sqnUserAssignService.getAll({ user_id: user?.id });
  ...
  const isBatchPendingForMe = (result: Ftw11SqnAssessmentCounselingResult): boolean => {
  ...
  const canCurrentAuthorityApprove = (result: Ftw11SqnAssessmentCounselingResult): boolean => {
  ...
  const confirmForward = async () => {
    ...
    try {
      await ftw11sqnCounselingSemesterApprovalService.store({
        course_id: courseId,
        semester_id: semesterId,
        status: "pending",
  ...
  const handleAddCounseling = () => router.push("/ftw/11sqn/assessments/counselings/results/create");
  const handleEdit = (result: Ftw11SqnAssessmentCounselingResult) => router.push(`/ftw/11sqn/assessments/counselings/results/${result.id}/edit`);
  const handleView = (result: Ftw11SqnAssessmentCounselingResult) => router.push(`/ftw/11sqn/assessments/counselings/results/${result.id}`);
  const handleDelete = (result: Ftw11SqnAssessmentCounselingResult) => { setDeletingResult(result); setDeleteModalOpen(true); };

  const confirmDelete = async () => {
    if (!deletingResult) return;
    try {
      setDeleteLoading(true);
      await ftw11sqnAssessmentCounselingResultService.deleteResult(deletingResult.id);
      await loadData();
  ...
  const uncounseledRows = allCadets
    ...
    .map(c => ({
      ...
      cadet_row: {
        ...
        is_present: false,
        is_active: true,
        ftw11sqn_assessment_counseling_result_id: -1,
        cadet: c,
      },
    }));
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw/11sqn/assessments/counselings/results")}
          ...
          <table className="w-full border-collapse border border-black text-sm">
            ...
            <tbody>
              {flattenedResults.length === 0 ? (
                <tr>
                  <td colSpan={canApproveAny ? 11 : 10} className="border border-black px-4 py-8 text-center text-gray-500">
