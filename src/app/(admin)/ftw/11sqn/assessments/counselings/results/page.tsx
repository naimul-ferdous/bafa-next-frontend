...
import { Ftw11sqnAssessmentCounselingResult } from "@/libs/types/system";
...
export default function Ftw11sqnAssessmentCounselingResultsPage() {
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw11sqnAssessmentCounselingResultService.getAllResults({
        page: currentPage,
        per_page: perPage,
  ...
  const loadOfficialResults = useCallback(async () => {
    try {
      ...
      const data = await ftw11sqnAssessmentCounselingResultService.getGroupedResults({
        search: searchTerm || undefined
      });
  ...
  const handleAddResult = () => router.push("/ftw/11sqn/assessments/counselings/results/create");
  const handleEditResult = (result: Ftw11sqnAssessmentCounselingResult) => router.push(`/ftw/11sqn/assessments/counselings/results/${result.id}/edit`);
  const handleViewResult = (result: Ftw11sqnAssessmentCounselingResult) => router.push(`/ftw/11sqn/assessments/counselings/results/${result.id}`);
  const handleDeleteResult = (result: Ftw11sqnAssessmentCounselingResult) => {
  ...
  const columns: Column<Ftw11sqnAssessmentCounselingResult>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
            <button onClick={() => router.push(`/ftw/11sqn/assessments/counselings/results/course/${row.course_details?.id}/semester/${row.semester_details?.id}`)} className="px-3 py-1 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg flex items-center gap-1 text-[10px] font-black uppercase transition-all" title="View Results">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase tracking-widest underline decoration-2 underline-offset-4">INTERVIEW/COUNSELLING REPORT (FTW 11SQN)</h2>
      </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  {Array.from({ length: Math.min(5, pagination.last_page) }, (_, i) => {
                    ...
                    return (
                      <button key={pageNum} onClick={() => setCurrentPage(pageNum)} className={`w-10 h-10 text-sm rounded-lg transition-all font-bold ${currentPage === pageNum ? "bg-blue-600 text-white shadow-md" : "border border-gray-300 hover:bg-gray-50"}`}>{pageNum}</button>
                    );
                  })}
                </div>
