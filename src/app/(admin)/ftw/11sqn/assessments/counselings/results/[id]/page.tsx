...
import { ftw11sqnCounselingCadetApprovalService } from "@/libs/services/ftw11sqnCounselingCadetApprovalService";
import { useAuth } from "@/context/AuthContext";
import FullLogo from "@/components/ui/fulllogo";
import type { Ftw11SqnAssessmentCounselingResult } from "@/libs/types/ftw11sqnAssessmentCounseling";
import type { Ftw11SqnCounselingCadetApproval } from "@/libs/types/ftw11sqnCounselingCadetApproval";
...
export default function ResultDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [approvals, setApprovals] = useState<Ftw11SqnCounselingCadetApproval[]>([]);
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const data = await ftw11sqnAssessmentCounselingResultService.getResult(resultId);
      if (!data) { setError("Result not found"); return; }
      setResult(data);

      // Load authorities + approvals via consolidated endpoint
      const consolidated = await ftw11sqnAssessmentCounselingResultService.getConsolidatedResults({
  ...
  // --- Current authority detection (same as course/semester page) ---
  ...
  // Find the authority entry that matches the counseling instructor
  ...
  // --- Can current authority approve this result's cadets ---
  ...
  // Per-cadet: already approved by me?
  ...
  // All cadets in result approved by me?
  const allApprovedByMe = (result?.result_cadets ?? []).every(rc => isApprovedByMe(rc.cadet_id));

  // Signature authorities (is_signature=true), sorted
  ...
  const confirmApproval = async () => {
    ...
    try {
      await ftw11sqnCounselingCadetApprovalService.bulkApprove({
        ...
        cadets: approvalModal.cadetIds.map(cadetId => {
          const rc = result.result_cadets?.find(c => Number(c.cadet_id) === Number(cadetId));
          return {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw11sqn/assessments/counselings/results")}
  ...
  // Derive events from counseling_type, or fall back to unique events from remarks
  const availableEvents = (() => {
    if (result.counseling_type?.events?.length) {
      return result.counseling_type.events
        .filter((e: any) => e.is_active)
        .sort((a: any, b: any) => (a.order || 0) - (b.order || 0));
    }
    // Fallback: collect events from all remarks (result-level or cadet-level)
    const allRemarks = [
      ...(result.remarks || []),
      ...(result.result_cadets?.flatMap((rc: any) => rc.remarks || []) || []),
    ];
    ...
    return Array.from(eventMap.values()).sort((a, b) => (a.order || 0) - (b.order || 0));
  })();
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200 min-h-screen">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          {/* Approve all cadets button */}
          {currentAuthority && canCurrentAuthorityApprove && !allApprovedByMe && (
            <button
              onClick={() => openApprovalModal((result.result_cadets ?? []).map(rc => rc.cadet_id))}
      ...
      {/* Approval status bar (no-print) */}
      {/* {allAuthorities.length > 0 && (
        <div className="no-print px-4 pb-3 flex flex-wrap gap-2">
          {[...allAuthorities].sort((a, b) => Number(a.sort) - Number(b.sort)).map((auth: any) => {
            const isMe = currentAuthority && Number(auth.id) === Number(currentAuthority.id);
            const allCadetsApproved = (result.result_cadets ?? []).length > 0 &&
              (result.result_cadets ?? []).every(rc =>
                approvals.some(a => Number(a.cadet_id) === Number(rc.cadet_id) && Number(a.authority_id) === Number(auth.id) && a.status === "approved")
              );
            return (
              <div
                key={auth.id}
              ...
              >
                <span className={`w-2 h-2 rounded-full ${allCadetsApproved ? "bg-green-500" : isMe ? "bg-blue-400" : "bg-gray-300"}`} />
                {auth.role?.name || `Auth #${auth.id}`}
                {isMe && " (Me)"}:
                <span>{allCadetsApproved ? "Approved" : "Pending"}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="p-4 cv-content">
        ...
        <div className="space-y-16">
          {result.result_cadets?.map((rc, rcIdx) => {
            const approvedByMe = isApprovedByMe(rc.cadet_id);
            const canApproveThis = canCurrentAuthorityApprove && !approvedByMe;
            return (
              <div key={rcIdx} className="report-block">
                ...
                <table className="w-full border-collapse border border-black text-sm">
                  ...
                  <tbody>
                    {availableEvents.map((event: any, eventIdx: number) => {
                      const cadetRemark = (rc.remarks || []).find((r: any) => r.ftw_11sqn_assessment_counseling_event_id === event.id)
                        ?? (result.remarks || []).find((r: any) => r.ftw_11sqn_assessment_counseling_event_id === event.id);
                      return (
                            ...
                            <>
                              ...
                              {signatureAuthorities.map((auth: any, sIdx: number) => {
                                ...
                                return (
                                  ...
                                  >
                                    {cadetApproval ? (
                                      /* Approved — show signature block */
                                      <SignatureBlock
                                        signature={cadetApproval.approver?.signature}
                                        name={cadetApproval.approver?.name}
                                        rank={cadetApproval.approver?.rank?.short_name || cadetApproval.approver?.rank?.name}
                                      ...
                                      /* Can approve — show button (no-print) */
                ...
                {/* Per-cadet per-authority status (no-print) */}
                {/* {allAuthorities.length > 0 && (
                  <div className="no-print mt-2 flex flex-wrap gap-2">
                    {[...allAuthorities].sort((a, b) => Number(a.sort) - Number(b.sort)).map((auth: any) => {
                      ...
                      return (
                        ...
                        >
                          {auth.role?.name || `Auth #${auth.id}`}{isMe ? " (Me)" : ""}:
