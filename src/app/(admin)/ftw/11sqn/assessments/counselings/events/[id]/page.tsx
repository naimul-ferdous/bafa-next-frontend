...
export default function CounselingTypeDetailsPage() {
  ...
  const loadType = async () => {
    try {
      setLoading(true);
      const data = await ftw11sqnAssessmentCounselingTypeService.getType(parseInt(typeId));
      if (data) {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      ...
      const success = await ftw11sqnAssessmentCounselingTypeService.reorderEvents(type.id, orders);
      if (!success) {
        loadType(); // Revert
      }
    } catch (err) {
