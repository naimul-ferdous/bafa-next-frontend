...
import type { Ftw11SqnAssessmentCommandType, Ftw11SqnAssessmentCommandTypeEstimatedMark } from "@/libs/types/ftw11sqnAssessmentCommand";
...
export default function CommandTypeDetailsPage() {
  ...
  const loadType = async () => {
    try {
      setLoading(true);
      const data = await ftw11sqnAssessmentCommandTypeService.getType(parseInt(typeId));
      if (data) {
        // Sort estimated marks by order
        if (data.estimated_marks) {
          data.estimated_marks.sort((a: Ftw11SqnAssessmentCommandTypeEstimatedMark, b: Ftw11SqnAssessmentCommandTypeEstimatedMark) => (a.order || 0) - (b.order || 0));
        }
        setType(data);
      } else {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      ...
      const success = await ftw11sqnAssessmentCommandTypeService.reorderEstimatedMarks(type.id, orders);
      if (!success) {
        loadType(); // Revert
      }
    } catch (err) {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ftw/11sqn/assessments/command/types")}
