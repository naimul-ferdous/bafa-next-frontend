...
import { Ftw11SqnAssessmentCommandResultService as ftw11sqnAssessmentCommandResultService } from "@/libs/services/ftw11sqnAssessmentCommandResultService";
import { ftw11sqnCommandSemesterApprovalService } from "@/libs/services/ftw11sqnCommandSemesterApprovalService";
import { ftw11sqnCommandApprovalAuthorityService } from "@/libs/services/ftw11sqnCommandApprovalAuthorityService";
import { useAuth } from "@/context/AuthContext";
import FullLogo from "@/components/ui/fulllogo";
import { getOrdinal } from "@/libs/utils/formatter";
import type { Ftw11SqnAssessmentCommandResult } from "@/libs/types/ftw11sqnAssessmentCommand";
import type { Ftw11SqnCommandCadetApproval } from "@/libs/types/ftw11sqnCommandCadetApproval";
...
import { Ftw11SqnCommandSemesterApproval as Ftw11SqnCommandSemesterApprovalType } from "@/libs/types/ftw11sqnCommandSemesterApproval";
...
export default function CommandResultDetailsPage() {
  const router = useRouter();
  const params = useParams();
  const resultId = params?.id as string;
  const { user } = useAuth();

  const [result, setResult] = useState<Ftw11SqnAssessmentCommandResult | null>(null);
  const [approvals, setApprovals] = useState<Ftw11SqnCommandCadetApproval[]>([]);
  const [semesterApprovals, setSemesterApprovals] = useState<Ftw11SqnCommandSemesterApprovalType[]>([]);
  const [semesterApproval, setSemesterApproval] = useState<Ftw11SqnCommandSemesterApprovalType | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [data, authRes] = await Promise.all([
        ftw11sqnAssessmentCommandResultService.getResult(parseInt(resultId)),
        ftw11sqnCommandApprovalAuthorityService.getAuthorities({ allData: true, is_active: true }),
      ]);
      ...
      setError("Failed to load Command result data");
    } finally {
      setLoading(false);
    }
  }, [resultId, user?.id]);
  ...
  const getCadetApproval = (cadetId: number, authId: number): Ftw11SqnCommandCadetApproval | undefined =>
    approvals.find((a) => a.cadet_id === cadetId && a.authority_id === authId);

  const getCurrentCadetApproval = (cadetId: number): Ftw11SqnCommandCadetApproval | undefined =>
  ...
  const confirmBulkAction = async () => {
    ...
    try {
      if (approvalModal.status === "approved") {
        await ftw11sqnAssessmentCommandResultService.bulkApprove({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
        });
      } else {
        await ftw11sqnAssessmentCommandResultService.bulkReject({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
  ...
  const confirmForward = async () => {
    ...
    try {
      await ftw11sqnCommandSemesterApprovalService.store({
  ...
  const estimatedMarks = result?.command_type?.estimated_marks || [];

  const calculateTotal = useCallback((cadetMarks: any[]) => {
    let total = 0;
    estimatedMarks.forEach((em) => {
      const mark = cadetMarks.find((m: any) => m.ftw_11sqn_assessment_command_type_estimated_mark_id === em.id);
  ...
  const maxTotal = useMemo(() => {
    let total = 0;
    estimatedMarks.forEach((em) => {
      total += parseFloat(String(em.estimated_mark || 0));
    });
    return total || 1;
  }, [estimatedMarks]);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <p className="text-red-600">{error || "Command Result not found"}</p>
          <button
            onClick={() => router.push("/Ftw11Sqn/assessments/Command/results")}
  ...
  const groupedCadets = (result.grouped_cadets && result.grouped_cadets.length > 0)
    ? result.grouped_cadets
    : (result.result_cadets && result.result_cadets.length > 0)
      ? [{ name: result.course?.name || "All Cadets", is_flying_group: false, cadets: result.result_cadets }]
      : [];
      ...
      <style dangerouslySetInnerHTML={{
        ...
        @media print {
          @page {
            ...
            @top-right  {
              content: "BAF - ${result.commandType?.type_name}";
              font-size: 10pt;
              text-align: right;
            }
                ...
                <thead>
                  ...
                  <tr>
                    ...
                    <th className="border border-black px-3 py-2 text-center font-bold" rowSpan={2}>
                      Total {result.commandType?.is_multiplier ? `x ${result.commandType.multiplier}` : ""}
                    </th>
                    ...
                    return (
                      <tr key={cadet.cadet_id}>
                        ...
                        {estimatedMarks.map((em, i) => {
                          const mark = cadet.marks?.find((m: any) => m.ftw_11sqn_assessment_command_type_estimated_mark_id === em.id);
      ...
      <Modal isOpen={approvalModal.open} onClose={() => setApprovalModal(p => ({ ...p, open: false }))} showCloseButton className="max-w-md">
        <div className="p-6">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Command Cadet Approval</h2>
              <p className="text-xs text-gray-500">{currentAuthority?.role?.name || "Authority"} approval</p>
            </div>
          </div>

          <p className="text-sm text-gray-600 mb-4">
            Command Type: <span className="font-medium text-gray-800">{result?.commandType?.type_name || "—"}</span>
          </p>
        ...
        <div className="p-6">
          <div className="flex items-center gap-1 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">ATW Command result forwarding</p>
            </div>
          </div>
          ...
          {result && (
            <div className="mb-5 rounded-lg border border-gray-200 divide-y divide-gray-100 text-sm">
              {([
                ...
                ["Command Type", result.commandType?.type_name || "—"],
          ...
          <p className="text-sm text-gray-600 mb-5">
            This will mark the Command results as forwarded to the higher authority for further review and approval. This action records your approval at the result level.
          </p>
