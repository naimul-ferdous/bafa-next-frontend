...
import { Ftw11SqnAssessmentCommandResult } from "@/libs/types/ftw11sqnAssessmentCommand";
...
export default function Ftw11SqnViewAssessmentCommandResultsPage() {
  const router = useRouter();
  const { user } = useAuth();
  const can = useCan();

  const [hasCommandAssign, setHasCommandAssign] = useState(false);
  const [results, setResults] = useState<Ftw11SqnAssessmentCommandResult[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deletingResult, setDeletingResult] = useState<Ftw11SqnAssessmentCommandResult | null>(null);
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await Ftw11SqnAssessmentCommandResultService.getAllResults({
        page: currentPage,
        per_page: perPage,
      });
  ...
  const handleAddResult = () => router.push("/ftw/11sqn/assessments/command/results/create");
  const handleEditResult = (result: Ftw11SqnAssessmentCommandResult) => router.push(`/ftw/11sqn/assessments/command/results/${result.id}/edit`);
  const handleViewResult = (result: Ftw11SqnAssessmentCommandResult) => router.push(`/ftw/11sqn/assessments/command/results/${result.id}`);
  const handleDeleteResult = (result: Ftw11SqnAssessmentCommandResult) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await Ftw11SqnAssessmentCommandResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<Ftw11SqnAssessmentCommandResult>[] = [
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">11Sqn Assessment Command Results</h2>
      </div>
