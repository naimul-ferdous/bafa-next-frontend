...
import {
  Ftw11sqnAdjustedModalProvider,
  useFtw11sqnAdjustedModal,
} from "@/context/Ftw11sqnAdjustedModalContext";
import Ftw11sqnAdjustedFormModal from "@/components/ftw-11sqn-adjusted/Ftw11sqnAdjustedFormModal";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import Pagination from "@/components/ui/Pagination";

function AdjustedPageContent() {
  const { openModal } = useFtw11sqnAdjustedModal();
  const [records, setRecords] = useState<Ftw11sqnResultsBupAdjustMarkGrading[]>([]);
  const [loading, setLoading] = useState(true);

  // Delete modal state
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<Ftw11sqnResultsBupAdjustMarkGrading | null>(null);
  ...
  const loadRecords = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw11sqnResultsBupAdjustMarkGradingService.getAll({
        page: currentPage,
        per_page: perPage,
      });
  ...
  useEffect(() => {
    ...
    window.addEventListener('ftw11sqnAdjustedUpdated', handleUpdate);
    return () => window.removeEventListener('ftw11sqnAdjustedUpdated', handleUpdate);
  }, [loadRecords]);
  ...
  const handleEdit = (record: Ftw11sqnResultsBupAdjustMarkGrading) => {
    openModal(record);
  };

  const handleView = (record: Ftw11sqnResultsBupAdjustMarkGrading) => {
    openModal(record);
  };

  const handleDelete = (record: Ftw11sqnResultsBupAdjustMarkGrading) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ftw11sqnResultsBupAdjustMarkGradingService.remove(deleteTarget.id);
      await loadRecords();
  ...
  const columns: Column<Ftw11sqnResultsBupAdjustMarkGrading>[] = [
    {
      key: "id",
      header: "SL.",
