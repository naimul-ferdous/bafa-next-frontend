...
export default function Ftw11sqnGroundExaminationMarksPage() {
  ...
  const handleAddMark = () => router.push("/ftw/11sqn/results/ground/marks/create");
  const handleEditMark = (mark: Ftw11sqnGroundExaminationMark) => router.push(`/ftw/11sqn/results/ground/marks/${mark.id}/edit`);
  const handleViewMark = (mark: Ftw11sqnGroundExaminationMark) => router.push(`/ftw/11sqn/results/ground/marks/${mark.id}`);
  ...
  const columns: Column<Ftw11sqnGroundExaminationMark>[] = [
    ...
    },
    {
      ...
      render: (mark) => mark.syllabus?.ground_fullname || "—"
    },
    {
      key: "exercise",
      header: "Test",
      className: "text-gray-700",
      render: (mark) => mark.exercise?.ground_test_name || "—"
    },
    {
      key: "course",
      header: "Course",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      ...
      {loading ? <TableLoading /> : <DataTable columns={columns} data={marks} keyExtractor={(mark) => mark.id.toString()} emptyMessage="No ground examination marks found" />}
