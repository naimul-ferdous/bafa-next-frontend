...
export default function Ftw11sqnGroundExaminationMarksPage() {
  ...
  const loadMarks = useCallback(async () => {
    try {
      ...
      console.error("Failed to load 11sqn ground examination marks:", error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleExport = () => console.log("Export 11sqn ground examination marks");

  const handleViewDetails = (row: SemesterRow) => {
    // Navigate to detailed view with course and semester in URL path
    router.push(`/ftw/11sqn/results/ground/results/course/${row.course_id}/semester/${row.semester_id}`);
  };
    ...
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No 11sqn ground examination results found"
