...
import type {
    Ftw11sqnFlyingSyllabusGroupedSemester,
    Ftw11sqnFlyingSyllabus,
    Ftw11sqnFlyingSyllabusExercise,
} from "@/libs/types/ftw11sqnFlying";

interface FlatExercise extends Ftw11sqnFlyingSyllabusExercise {
    typeName: string;
    typeCode: string;
}
...
export default function Ftw11sqnFullDetailsSyllabusPage() {
    const router = useRouter();
    const [semesterGroups, setSemesterGroups] = useState<Ftw11sqnFlyingSyllabusGroupedSemester[]>([]);
    const [loading, setLoading] = useState(true);

    const loadData = useCallback(async () => {
        try {
            setLoading(true);
            const data = await ftw11sqnFlyingSyllabusService.getSemesterGrouped();
            ...
            <div className="p-8">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
                        FTW 11 SQN — Official Flying Training Syllabus
                    </p>
                </div>
                        ...
                        <div key={semIdx} className={semIdx > 0 ? "report-block" : ""}>
                            <div className="space-y-10">
                                {(semesterGroup.syllabus ?? [])
                                    .slice()
                                    .sort((a: Ftw11sqnFlyingSyllabus, b: Ftw11sqnFlyingSyllabus) => a.phase_sort - b.phase_sort)
                                    .map((phase: Ftw11sqnFlyingSyllabus) => {
