...
import type { Ftw11sqnFlyingSyllabus, Ftw11sqnFlyingSyllabusExercise, Ftw11sqnFlyingSyllabusCreateData } from "@/libs/types/ftw11sqnFlying";
import { ExerciseModalProvider, useExerciseModal } from "@/context/ExerciseModalContext";
...
function ViewFlyingSyllabusContent() {
  ...
  const { openModal } = useExerciseModal();
  ...
  const handleEditExercise = (exercise: AggregatedExercise) => {
    ...
    openModal(exercise, syllabusType?.ftw_11sqn_flying_phase_type_id, id);
  };
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      const updatedSyllabusData: Partial<Ftw11sqnFlyingSyllabusCreateData> = {
      ...
      await ftw11sqnFlyingSyllabusService.update(id, updatedSyllabusData);
      await loadSyllabus();
