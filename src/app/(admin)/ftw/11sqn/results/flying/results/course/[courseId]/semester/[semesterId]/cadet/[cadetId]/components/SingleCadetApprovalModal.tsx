...
const SingleCadetApprovalModal = ({
  ...
  const fetchApprovalProcess = async () => {
    ...
    try {
      ...
      if (result?.data && Array.isArray(result.data) && result.data.length > 0) {
        ...
        const allProcesses = await ftw11sqnFlyingExamApprovalProcessService.getAll({ allData: true });
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await ftw11sqnFlyingExamCadetApprovalStatusService.create(approvalData);
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await ftw11sqnFlyingExamCadetApprovalStatusService.create(rejectData);
