...
export default function Ftw11sqnFlyingExaminationMarksPage() {
  const router = useRouter();
  const { user, userIsInstructor } = useAuth();
  const isInstructor = userIsInstructor;

  // ── Instructor view ────────────────────────────────────────────────────
  const [marks, setMarks] = useState<Ftw11sqnFlyingExaminationMark[]>([]);
  ...
  const loadGrouped = useCallback(async () => {
    try {
      ...
      const response = await ftw11sqnFlyingExaminationMarkService.getSemesterGrouped();
  ...
  // ── Instructor columns ─────────────────────────────────────────────────
  const markColumns: Column<Ftw11sqnFlyingExaminationMark>[] = [
    {
    ...
    },
    {
      ...
      render: (m) => (
        <div className="flex items-center justify-center gap-1" onClick={(e) => e.stopPropagation()}>
          <button onClick={() => router.push(`/ftw/11sqn/results/flying/results/${m.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded" title="View">
            <Icon icon="hugeicons:view" className="w-4 h-4" />
          </button>
          <button onClick={() => router.push(`/ftw/11sqn/results/flying/results/${m.id}/edit`)} className="p-1 text-yellow-600 hover:bg-yellow-50 rounded" title="Edit">
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-center">
          <button
            onClick={() => router.push(`/ftw/11sqn/results/flying/results/course/${row.course_id}/semester/${row.semester_id}`)}
    ...
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3">
          {isInstructor && (
            <button
              onClick={() => router.push("/ftw/11sqn/results/flying/results/create")}
      ...
      {isInstructor ? (
        ...
        groupedLoading ? <TableLoading /> : (
          <DataTable
            ...
            onRowClick={(row) => router.push(`/ftw/11sqn/results/flying/results/course/${row.course_id}/semester/${row.semester_id}`)}
          />
        )
      )}
