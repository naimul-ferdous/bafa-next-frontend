...
export default function Ftw11sqnFlyingExaminationMarksPage() {
  ...
  const loadMarks = useCallback(async () => {
    try {
      ...
      console.error("Failed to load flying examination marks:", error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleAddMark = () => router.push("/ftw/11sqn/results/flying/results/create");
  const handleExport = () => console.log("Export flying examination marks");

  const handleViewDetails = (row: SemesterRow) => {
    // Navigate to detailed view with course and semester in URL path
    router.push(`/ftw/11sqn/results/flying/results/course/${row.course_id}/semester/${row.semester_id}`);
  };
