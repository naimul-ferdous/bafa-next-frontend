...
const ForwardToApprovalModal = ({
  ...
  const fetchApprovalSummary = async () => {
    ...
    try {
      const processResult = await ftw11sqnFlyingExamApprovalProcessService.getByRoleId(user.role_id, { allData: true });
      ...
      if (processResult?.data && Array.isArray(processResult.data) && processResult.data.length > 0) {
        ...
        const result = await ftw11sqnFlyingExamCadetApprovalStatusService.getApprovalSummary(
          courseId,
          semesterId,
          currentProcess.id
        );
  ...
  const handleConfirmForward = async () => {
    ...
    try {
      ...
      await ftw11sqnFlyingExamApprovalStatusService.create(approvalData);
