...
import { Ftw11SqnCounselingApprovalAuthority } from "@/libs/types/ftw11sqnCounselingApprovalAuthority";
import { Icon } from "@iconify/react";
import { ftw11sqnCounselingApprovalAuthorityService } from "@/libs/services/ftw11sqnCounselingApprovalAuthorityService";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import Ftw11SqnCounselingApprovalAuthorityModal from "@/components/ftw-11sqn-results/Ftw11SqnCounselingApprovalAuthorityModal";
import Pagination from "@/components/ui/Pagination";
// import { useCan } from "@/context/PagePermissionsContext";

export default function Ftw11SqnCounselingApprovalAuthoritiesPage() {
  // const can = useCan();
  const [authorities, setAuthorities] = useState<Ftw11SqnCounselingApprovalAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<Ftw11SqnCounselingApprovalAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<Ftw11SqnCounselingApprovalAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw11sqnCounselingApprovalAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
  ...
  const handleEditAuthority = (authority: Ftw11SqnCounselingApprovalAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: Ftw11SqnCounselingApprovalAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ftw11sqnCounselingApprovalAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<Ftw11SqnCounselingApprovalAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      ...
      <Ftw11SqnCounselingApprovalAuthorityModal
        isOpen={modalOpen}
