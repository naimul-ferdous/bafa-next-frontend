...
import { Ftw11SqnPenpictureAuthority } from "@/libs/types/ftw11sqnPenpictureAuthority";
import { Icon } from "@iconify/react";
import { ftw11sqnPenpictureAuthorityService } from "@/libs/services/ftw11sqnPenpictureAuthorityService";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import Ftw11SqnPenpictureAuthorityModal from "@/components/ftw-11sqn-results/Ftw11SqnPenpictureAuthorityModal";
import Pagination from "@/components/ui/Pagination";
import { useCan } from "@/context/PagePermissionsContext";

export default function Ftw11SqnPenpictureAuthoritiesPage() {
  const can = useCan();
  const [authorities, setAuthorities] = useState<Ftw11SqnPenpictureAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<Ftw11SqnPenpictureAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<Ftw11SqnPenpictureAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw11sqnPenpictureAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
      ...
      console.error("Failed to load penpicture authorities:", error);
    } finally {
      setLoading(false);
    }
  ...
  const handleEditAuthority = (authority: Ftw11SqnPenpictureAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: Ftw11SqnPenpictureAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ftw11sqnPenpictureAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<Ftw11SqnPenpictureAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">ATW Pen Picture Authority Management</h2>
      </div>
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No pen picture authorities found"
        />
      )}
      ...
      <Ftw11SqnPenpictureAuthorityModal
        isOpen={modalOpen}
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to ${statusAuthority?.is_active ? "deactivate" : "activate"} this pen picture authority?`}
