...
import Ftw11SqnOlqApprovalAuthorityModal from "@/components/ftw-11sqn-results/Ftw11SqnOlqApprovalAuthorityModal";
import Pagination from "@/components/ui/Pagination";
// import { useCan } from "@/context/PagePermissionsContext";

export default function Ftw11SqnOlqApprovalAuthoritiesPage() {
  // const can = useCan();
  const [authorities, setAuthorities] = useState<Ftw11SqnOlqApprovalAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<Ftw11SqnOlqApprovalAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<Ftw11SqnOlqApprovalAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ftw11sqnOlqApprovalAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
  ...
  const handleEditAuthority = (authority: Ftw11SqnOlqApprovalAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: Ftw11SqnOlqApprovalAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ftw11sqnOlqApprovalAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<Ftw11SqnOlqApprovalAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">FTW 11SQN OLQ Assessment Approval Authority Management</h2>
      </div>
      ...
      <Ftw11SqnOlqApprovalAuthorityModal
        isOpen={modalOpen}
