...
export default function EditSubjectPage() {
  ...
  useEffect(() => {
    const loadSubject = async () => {
      try {
        ...
        const subjectData = await atwSubjectModuleService.getSubject(parseInt(subjectId));
        if (subjectData) {
  ...
  const handleSubmit = async (data: any) => {
    setLoading(true);
    try {
      await atwSubjectModuleService.updateSubject(parseInt(subjectId), data);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/atw/subjects/modules")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
            Back to Subjects
          </button>
        </div>
      </div>
