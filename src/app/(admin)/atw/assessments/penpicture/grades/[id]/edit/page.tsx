...
import GradeForm, { GradeFormData } from "@/components/atw-assessment-penpicture-grades/GradeForm";
...
export default function EditGradePage() {
  ...
  const [grade, setGrade] = useState<AtwAssessmentPenpictureGrade | null>(null);
  ...
  useEffect(() => {
    const loadGrade = async () => {
      try {
        ...
        const gradeData = await atwAssessmentPenpictureGradeService.getGrade(parseInt(gradeId));
        if (gradeData) {
          setGrade(gradeData);
        } else {
  ...
  const handleSubmit = async (data: GradeFormData) => {
    ...
    try {
      ...
      await atwAssessmentPenpictureGradeService.updateGrade(parseInt(gradeId), submitData);
      router.push("/atw/assessments/penpicture/grades");
    } catch (err: any) {
      throw err;
    } finally {
      setLoading(false);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/atw/assessments/penpicture/grades")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
            Back to Grades
          </button>
        </div>
      </div>
