...
export default function AtwViewAssessmentPenpictureResultsPage() {
  const router = useRouter();
  const { user, userIsInstructor } = useAuth();
  const can = useCan("/atw/assessments/penpicture/results");
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await atwAssessmentPenpictureResultService.getGroupedResults({
        page: currentPage,
        per_page: perPage,
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          {can("view") && (
            <button
              onClick={() => router.push(`/atw/assessments/penpicture/results/course/${row.course_id}/semester/${row.semester_id}`)}
      ...
      {loading ? (
        ...
        <>
          <DataTable
            ...
            onRowClick={can("view") ? (row) => router.push(`/atw/assessments/penpicture/results/course/${row.course_id}/semester/${row.semester_id}`) : undefined}
          />
