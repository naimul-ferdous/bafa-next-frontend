...
import { AtwAssessmentPenpictureResult } from "@/libs/types/system";
...
export default function AtwAssessmentPenpictureResultsPage() {
  ...
  const [results, setResults] = useState<AtwAssessmentPenpictureResult[]>([]);
  ...
  const loadOfficialResults = useCallback(async () => {
    try {
      ...
      const data = await atwAssessmentPenpictureResultService.getGroupedResults({
        search: searchTerm || undefined
      });
  ...
  const handleAddResult = () => router.push("/atw/assessments/penpicture/results/create");
  const handleEditResult = (result: AtwAssessmentPenpictureResult) => router.push(`/atw/assessments/penpicture/results/${result.id}/edit`);
  const handleViewResult = (result: AtwAssessmentPenpictureResult) => router.push(`/atw/assessments/penpicture/results/${result.id}`);
  const handleDeleteResult = (result: AtwAssessmentPenpictureResult) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await atwAssessmentPenpictureResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<AtwAssessmentPenpictureResult>[] = [
    ...
    },
    {
      ...
      render: (result) => (
        <div>
          <div className="font-medium text-gray-900">{result.cadet?.name || "N/A"}</div>
          <div className="text-xs text-blue-600 font-mono font-bold uppercase">{result.cadet?.cadet_number || ""}</div>
        </div>
      ),
    },
    {
      ...
      render: (result) => (
        <span className="inline-flex items-center px-2.5 py-1 text-[10px] font-black uppercase rounded-full bg-blue-50 text-blue-700 border border-blue-100">
          {result.grade?.grade_name || "N/A"}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
            {can('view') && (
              <button onClick={() => router.push(`/atw/assessments/penpicture/results/course/${row.course_details?.id}/semester/${row.semester_details?.id}`)} className="px-3 py-1 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg flex items-center gap-1 text-[10px] font-black uppercase transition-all" title="View Results">
    ...
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div className="relative w-80">
          ...
          <input type="text" placeholder="Search by course, semester, cadet..." value={searchTerm} onChange={(e) => handleSearchChange(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-200 rounded-lg bg-gray-50 text-gray-900 w-full focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" />
        </div>
      ...
      <ConfirmationModal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} onConfirm={confirmDelete} title="Delete Result" message={`Are you sure you want to delete this assessment result for "${deletingResult?.cadet?.name}"? This will also delete all associated strengths and weaknesses. This action cannot be undone.`} confirmText="Delete" cancelText="Cancel" loading={deleteLoading} variant="danger" />
    </div>
  );
}
