...
export default function EditResultPage() {
  ...
  const [result, setResult] = useState<AtwAssessmentPenpictureResult | null>(null);
  ...
  useEffect(() => {
    const loadResult = async () => {
      try {
        ...
        const resultData = await atwAssessmentPenpictureResultService.getResult(parseInt(resultId));
        if (resultData) {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/atw/assessments/penpicture/results")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
            Back to Results
          </button>
        </div>
      </div>
