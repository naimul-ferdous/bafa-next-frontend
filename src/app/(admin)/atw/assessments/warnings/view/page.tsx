...
export default function AtwViewCadetWarningsPage() {
  const router = useRouter();
  const can = useCan("/atw/assessments/warnings");
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await atwCadetWarningService.getGroupedResults({
        page: currentPage,
        per_page: perPage,
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          {can("view") && (
            <button
              onClick={() => router.push(`/atw/assessments/warnings/course/${row.course_id}/semester/${row.semester_id}`)}
