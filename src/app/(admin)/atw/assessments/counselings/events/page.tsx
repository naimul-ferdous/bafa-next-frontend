...
import { AtwAssessmentCounselingType } from "@/libs/types/atwAssessmentCounseling";
...
export default function AtwAssessmentCounselingTypesPage() {
  const router = useRouter();
  const [types, setTypes] = useState<AtwAssessmentCounselingType[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deletingType, setDeletingType] = useState<AtwAssessmentCounselingType | null>(null);
  ...
  const handleAddType = () => router.push("/atw/assessments/counselings/events/create");
  const handleEditType = (type: AtwAssessmentCounselingType) => router.push(`/atw/assessments/counselings/events/${type.id}/edit`);
  const handleViewType = (type: AtwAssessmentCounselingType) => router.push(`/atw/assessments/counselings/events/${type.id}`);
  const handleDeleteType = (type: AtwAssessmentCounselingType) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await atwAssessmentCounselingTypeService.deleteType(deletingType.id);
      await loadTypes();
  ...
  const columns: Column<AtwAssessmentCounselingType>[] = [
