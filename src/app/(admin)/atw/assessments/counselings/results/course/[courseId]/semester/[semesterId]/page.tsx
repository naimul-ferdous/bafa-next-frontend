...
export default function CounselingCourseSemesterResultPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await atwAssessmentCounselingResultService.getConsolidatedResults({
        course_id: courseId,
  ...
  const handleAddCounseling = () => router.push("/atw/assessments/counselings/results/create");
  const handleEdit = (result: AtwAssessmentCounselingResult) => router.push(`/atw/assessments/counselings/results/${result.id}/edit`);
  const handleView = (result: AtwAssessmentCounselingResult) => router.push(`/atw/assessments/counselings/results/${result.id}`);
  const handleDelete = (result: AtwAssessmentCounselingResult) => { setDeletingResult(result); setDeleteModalOpen(true); };
  ...
  const uncounseledRows = allCadets
    ...
    .map(c => ({
      ...
      cadet_row: {
        ...
        is_present: false,
        is_active: true,
        atw_assessment_counseling_result_id: -1,
        cadet: c,
      },
    }));
