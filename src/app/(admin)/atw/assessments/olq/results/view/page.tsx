...
import { AtwAssessmentOlqResult } from "@/libs/types/atwAssessmentOlq";
...
export default function AtwViewAssessmentOlqResultsPage() {
  const router = useRouter();
  const { user } = useAuth();
  const can = useCan("/atw/assessments/olq/results");
  ...
  const [deletingResult, setDeletingResult] = useState<AtwAssessmentOlqResult | null>(null);
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await atwAssessmentOlqResultService.getAllResults({
        page: currentPage,
        per_page: perPage,
      });
  ...
  const handleAddResult = () => router.push("/atw/assessments/olq/results/create");
  const handleEditResult = (result: AtwAssessmentOlqResult) => router.push(`/atw/assessments/olq/results/${result.id}/edit`);
  const handleViewResult = (result: AtwAssessmentOlqResult) => router.push(`/atw/assessments/olq/results/${result.id}`);
  const handleDeleteResult = (result: AtwAssessmentOlqResult) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await atwAssessmentOlqResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<AtwAssessmentOlqResult>[] = [
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">ATW Assessment OLQ Results</h2>
      </div>
