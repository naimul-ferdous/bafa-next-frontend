...
import { AtwAssessmentOlqType } from "@/libs/types/atwAssessmentOlq";
...
export default function AtwAssessmentOlqTypesPage() {
  ...
  const [deletingType, setDeletingType] = useState<AtwAssessmentOlqType | null>(null);
  ...
  const loadTypes = useCallback(async () => {
    try {
      setLoading(true);
      const response = await atwAssessmentOlqTypeService.getAllTypes({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddType = () => router.push("/atw/assessments/olq/types/create");
  const handleEditType = (type: AtwAssessmentOlqType) => router.push(`/atw/assessments/olq/types/${type.id}/edit`);
  const handleViewType = (type: AtwAssessmentOlqType) => router.push(`/atw/assessments/olq/types/${type.id}`);
  const handleDeleteType = (type: AtwAssessmentOlqType) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await atwAssessmentOlqTypeService.deleteType(deletingType.id);
      await loadTypes();
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">ATW Assessment OLQ Types</h2>
      </div>
