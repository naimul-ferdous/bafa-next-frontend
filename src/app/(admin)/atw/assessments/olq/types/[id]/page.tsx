...
import type { AtwAssessmentOlqType } from "@/libs/types/atwAssessmentOlq";
...
export default function OlqTypeDetailsPage() {
  ...
  const loadType = async () => {
    try {
      setLoading(true);
      const data = await atwAssessmentOlqTypeService.getType(parseInt(typeId));
      if (data) {
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      ...
      const success = await atwAssessmentOlqTypeService.reorderEstimatedMarks(type.id, orders);
      if (!success) {
        loadType(); // Revert
      }
    } catch (err) {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/atw/assessments/olq/types")}
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push(`/atw/assessments/olq/types/${type.id}/edit`)}
