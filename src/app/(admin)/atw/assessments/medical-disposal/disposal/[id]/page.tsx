...
export default function ViewAtwMedicalDisposalResultPage() {
  const router = useRouter();
  const params = useParams();
  const can = useCan();
  const id = parseInt(params?.id as string);

  const [result, setResult] = useState<AtwMedicalDisposalResult | null>(null);
  ...
  useEffect(() => {
    ...
    atwMedicalDisposalResultService.getOne(id)
  ...
  result.result_schemas?.forEach((rs) => {
    schemaContentMap[rs.atw_medical_disposal_syllabus_schema_id] = rs.result_content || "";
  });
