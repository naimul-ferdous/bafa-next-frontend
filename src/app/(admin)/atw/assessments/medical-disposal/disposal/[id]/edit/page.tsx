...
export default function EditAtwMedicalDisposalResultPage() {
  ...
  const handleSubmit = async (data: AtwMedicalDisposalResultPayload) => {
    setLoading(true);
    try {
      await atwMedicalDisposalResultService.update(id, data);
  ...
  if (error || !result) {
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200 text-center py-16">
        ...
        <button onClick={() => router.push("/atw/assessments/medical-disposal/disposal")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-lg">
          Go Back
        </button>
      </div>
    );
