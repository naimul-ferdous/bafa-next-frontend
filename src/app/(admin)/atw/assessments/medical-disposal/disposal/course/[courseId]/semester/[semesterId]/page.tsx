...
import type { AtwMedicalDisposalResult } from "@/libs/types/atwMedicalDisposal";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import { useCan } from "@/context/PagePermissionsContext";

export default function AtwMedicalDisposalCourseSemesterPage({
  params,
}: {
  params: Promise<{ courseId: string; semesterId: string }>;
}) {
  const router = useRouter();
  const can = useCan("/atw/assessments/medical-disposal/disposal");
  const resolvedParams = use(params);
  const courseId   = parseInt(resolvedParams.courseId);
  const semesterId = parseInt(resolvedParams.semesterId);

  const [results, setResults]   = useState<AtwMedicalDisposalResult[]>([]);
  const [loading, setLoading]   = useState(true);
  const [error, setError]       = useState("");

  const [deleteModal, setDeleteModal] = useState<{ open: boolean; item: AtwMedicalDisposalResult | null; loading: boolean }>({
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const res = await atwMedicalDisposalResultService.getAll({
        course_id: courseId,
  ...
  const confirmDelete = async () => {
    ...
    try {
      await atwMedicalDisposalResultService.delete(deleteModal.item.id);
      await load();
  ...
  const columns: Column<AtwMedicalDisposalResult>[] = [
    {
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-center gap-1" onClick={(e) => e.stopPropagation()}>
          {can("view") && (
            <button
              onClick={(e) => { e.stopPropagation(); router.push(`/atw/assessments/medical-disposal/disposal/${row.id}`); }}
          ...
          {can("edit") && (
            <button
              onClick={(e) => { e.stopPropagation(); router.push(`/atw/assessments/medical-disposal/disposal/${row.id}/edit`); }}
  ...
  if (error || results.length === 0) {
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200 text-center py-16">
        ...
        <button
          onClick={() => router.push("/atw/assessments/medical-disposal/disposal")}
    ...
    <div className="bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          {can("add") && (
            <button
              onClick={() => router.push("/atw/assessments/medical-disposal/disposal/create")}
      ...
      <div className="p-8">
        ...
        <DataTable
          ...
          onRowClick={can("view") ? (row) => router.push(`/atw/assessments/medical-disposal/disposal/${row.id}`) : undefined}
        />
      </div>
