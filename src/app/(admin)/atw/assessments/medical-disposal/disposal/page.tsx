...
export default function AtwMedicalDisposalDisposalPage() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await atwMedicalDisposalResultService.getGroupedResults({
        page: currentPage,
        per_page: perPage,
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-center gap-1" onClick={(e) => e.stopPropagation()}>
          {can("view") && (
            <button
              onClick={(e) => { e.stopPropagation(); router.push(`/atw/assessments/medical-disposal/disposal/course/${row.course_id}/semester/${row.semester_id}`); }}
      ...
      {loading ? (
        ...
        <>
          <DataTable
            ...
            onRowClick={can("view") ? (row) => router.push(`/atw/assessments/medical-disposal/disposal/course/${row.course_id}/semester/${row.semester_id}`) : undefined}
          />
