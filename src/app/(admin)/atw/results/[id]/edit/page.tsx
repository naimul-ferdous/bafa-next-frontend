...
import ResultForm from "@/components/atw-results/ResultForm";
  ...
  useEffect(() => {
    const loadResult = async () => {
      try {
        ...
        const resultData = await atwResultService.getResult(parseInt(resultId));
        if (resultData) {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/atw/results")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
            Back to Results
          </button>
        </div>
      </div>
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">Edit ATW Result</h2>
      </div>
