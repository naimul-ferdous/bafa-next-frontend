...
import { atwPenpictureAuthorityService } from "@/libs/services/atwPenpictureAuthorityService";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import AtwPenpictureAuthorityModal from "@/components/atw-results/AtwPenpictureAuthorityModal";
import Pagination from "@/components/ui/Pagination";
import { useCan } from "@/context/PagePermissionsContext";

export default function AtwPenpictureAuthoritiesPage() {
  const can = useCan();
  const [authorities, setAuthorities] = useState<AtwPenpictureAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<AtwPenpictureAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<AtwPenpictureAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await atwPenpictureAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
  ...
  const handleEditAuthority = (authority: AtwPenpictureAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: AtwPenpictureAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await atwPenpictureAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<AtwPenpictureAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
