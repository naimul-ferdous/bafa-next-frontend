...
import AtwCounselingApprovalAuthorityModal from "@/components/atw-results/AtwCounselingApprovalAuthorityModal";
import Pagination from "@/components/ui/Pagination";
import { useCan } from "@/context/PagePermissionsContext";

export default function AtwCounselingApprovalAuthoritiesPage() {
  const can = useCan();
  const [authorities, setAuthorities] = useState<AtwCounselingApprovalAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<AtwCounselingApprovalAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<AtwCounselingApprovalAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await atwCounselingApprovalAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
  ...
  const handleEditAuthority = (authority: AtwCounselingApprovalAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: AtwCounselingApprovalAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await atwCounselingApprovalAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<AtwCounselingApprovalAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      ...
      <AtwCounselingApprovalAuthorityModal
        isOpen={modalOpen}
