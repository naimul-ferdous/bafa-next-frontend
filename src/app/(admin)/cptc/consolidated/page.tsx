...
function CptcConsolidatedContent() {
  ...
  const handleViewResults = (course: SystemCourse) => {
    router.push(`/cptc/consolidated/course/${course.id}`);
  };
    ...
    },
    {
      ...
      render: (course) => (
        <div className="flex items-center justify-center gap-1" onClick={(e) => e.stopPropagation()}>
          <button onClick={() => handleViewResults(course)} className="p-1 text-blue-600 hover:bg-blue-50 rounded" title="View Results"><Icon icon="hugeicons:view" className="w-4 h-4" /></button>
          <button onClick={() => handleEditCourse(course)} className="p-1 text-yellow-600 hover:bg-yellow-50 rounded" title="Edit"><Icon icon="hugeicons:pencil-edit-01" className="w-4 h-4" /></button>
          {course.is_active ? (
            <button onClick={() => handleToggleStatus(course)} className="p-1 text-red-600 hover:bg-red-50 rounded transition-colors" title="Deactivate"><Icon icon="hugeicons:unavailable" className="w-4 h-4" /></button>
          ) : (
            <button onClick={() => handleToggleStatus(course)} className="p-1 text-green-600 hover:bg-green-50 rounded transition-colors" title="Activate"><Icon icon="hugeicons:checkmark-circle-02" className="w-4 h-4" /></button>
          )}
        </div>
      ),
    },
...
export default function CptcConsolidatedPage() {
  return (
    <CourseModalProvider>
      <CptcConsolidatedContent />
    </CourseModalProvider>
  );
}
