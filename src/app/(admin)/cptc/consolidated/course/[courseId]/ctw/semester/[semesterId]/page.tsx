...
export default function CptcCtwCourseSemesterConsolidatedPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  // Calculate MSE from all non-END exam types (includes MID, BMA, etc.)
  ...
  const handleBack = () => router.push(`/cptc/consolidated/course/${courseId}/ctw`);
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        <button onClick={handleBack} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 font-medium transition-all">
          <Icon icon="hugeicons:arrow-left-01" className="w-4 h-4" />Back to Consolidated
        </button>
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2 inline-block w-full">
            CTW Consolidated Result Sheet — {course?.name || ""} ({semester?.name || ""})
          </p>
        </div>
        ...
        <div className="mb-6">
          <div className="flex justify-end items-center gap-4 mb-4">
            <div className="flex items-center gap-1 p-1 rounded-full border border-gray-200 text-xs mb-2">
              <button
                onClick={() => setConsolidateTab("main")}
                className={`px-3 py-1 rounded-full border border-gray-200 transition-all ${consolidateTab === "main" ? "bg-indigo-50 text-indigo-600 border-indigo-200" : ""}`}
              >
                Main
              </button>
              <button
                onClick={() => setConsolidateTab("breakdown")}
                className={`px-3 py-1 rounded-full border border-gray-200 transition-all ${consolidateTab === "breakdown" ? "bg-indigo-50 text-indigo-600 border-indigo-200" : ""}`}
              >
                Breakdown
              </button>
            </div>
                        ...
                        <tr key={item.cadet_details.id} className="transition-colors hover:bg-gray-50">
                          ...
                          {allExamTypes.map((et: any) => {
                            ...
                            return Object.entries(examModules).map(([assessKey]) => {
                              ...
                              return (
                                <td key={`cell-${item.cadet_details.id}-${key}`} className="border border-black px-1 py-1 text-center">
