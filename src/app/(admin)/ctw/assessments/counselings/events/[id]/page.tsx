...
import type { CtwAssessmentCounselingType } from "@/libs/types/ctwAssessmentCounseling";

export default function CounselingTypeDetailsPage() {
  const router = useRouter();
  const params = useParams();
  const typeId = params?.id as string;
  const parsedId = typeId ? parseInt(typeId) : NaN;
  ...
  const loadType = async () => {
    try {
      setLoading(true);
      const data = await ctwAssessmentCounselingTypeService.getType(parsedId);
      if (data) {
  ...
  useEffect(() => {
    if (!isNaN(parsedId)) {
      ...
      setError("Invalid Counseling Type ID");
      setLoading(false);
    }
  }, [parsedId]);
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      ...
      const success = await ctwAssessmentCounselingTypeService.reorderEvents(type.id, orders);
      if (!success) {
        loadType(); // Revert
      }
    } catch (err) {
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {type.semesters.map((sem) => (
                ...
                >
                  {sem.name || sem.semester?.name || `Semester ${sem.semester_id}`}
                </span>
              ))}
            </div>
          </div>
