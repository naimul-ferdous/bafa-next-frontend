...
import { CtwAssessmentCounselingType } from "@/libs/types/ctwAssessmentCounseling";
...
export default function CtwAssessmentCounselingTypesPage() {
  const router = useRouter();
  const can = useCan();

  const [types, setTypes] = useState<CtwAssessmentCounselingType[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deletingType, setDeletingType] = useState<CtwAssessmentCounselingType | null>(null);
  ...
  const loadTypes = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwAssessmentCounselingTypeService.getAllTypes({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddType = () => router.push("/ctw/assessments/counselings/events/create");
  const handleEditType = (type: CtwAssessmentCounselingType) => router.push(`/ctw/assessments/counselings/events/${type.id}/edit`);
  const handleViewType = (type: CtwAssessmentCounselingType) => router.push(`/ctw/assessments/counselings/events/${type.id}`);
  const handleDeleteType = (type: CtwAssessmentCounselingType) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwAssessmentCounselingTypeService.deleteType(deletingType.id);
      await loadTypes();
  ...
  const columns: Column<CtwAssessmentCounselingType>[] = [
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Assessment Counseling Types</h2>
      </div>
