    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200 min-h-screen">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push(`/ctw/assessments/counselings/results/${result.id}/edit`)}
      ...
      <div className="p-12 cv-content">
        ...
        <div className="border-b border-dashed border-gray-300 py-3 mb-8 px-2">
          <div className="flex flex-wrap justify-between items-center text-sm">
            ...
            <div className="flex gap-1.5">
              <span className="text-gray-600">Officer Cadet:</span>
              <span className="font-bold text-gray-900 uppercase">
                {result.cadet?.name} ({(result.cadet as any).cadet_number || result.cadet?.service_number || "—"})
              </span>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {availableEvents.length > 0 ? (
                availableEvents.map((event, eventIdx) => {
                  const remark = result.remarks?.find(r => r.ctw_assessment_counseling_event_id === event.id);
                  return (
