...
export default function CounselingCourseSemesterResultPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  const handleEdit = (result: CtwAssessmentCounselingResult) => {
    router.push(`/ctw/assessments/counselings/results/${result.id}/edit`);
  };

  const handleView = (result: CtwAssessmentCounselingResult) => {
    router.push(`/ctw/assessments/counselings/results/${result.id}`);
  };

  const handleDelete = (result: CtwAssessmentCounselingResult) => {
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2 underline decoration-2 underline-offset-4">
            CTW Counseling Assessment Summary Sheet
          </p>
        </div>
