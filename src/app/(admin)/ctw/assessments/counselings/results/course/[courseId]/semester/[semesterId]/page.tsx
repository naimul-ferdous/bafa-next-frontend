...
import { ctwCounselingSemesterApprovalService } from "@/libs/services/ctwCounselingSemesterApprovalService";
...
import type { CtwAssessmentCounselingResult } from "@/libs/types/ctwAssessmentCounseling";
import type { CtwCounselingCadetApproval } from "@/libs/types/ctwCounselingCadetApproval";
import type { CtwCounselingSemesterApproval } from "@/libs/types/ctwCounselingSemesterApproval";
...
export default function CounselingCourseSemesterResultPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  const { user } = useAuth();
  const can = useCan("/ctw/assessments/counselings/results");
  const resolvedParams = use(params);
  const courseId = parseInt(resolvedParams.courseId);
  const semesterId = parseInt(resolvedParams.semesterId);

  const [results, setResults] = useState<CtwAssessmentCounselingResult[]>([]);
  const [approvals, setApprovals] = useState<CtwCounselingCadetApproval[]>([]);
  const [semesterApprovals, setSemesterApprovals] = useState<CtwCounselingSemesterApproval[]>([]);
  const [semesterApproval, setSemesterApproval] = useState<CtwCounselingSemesterApproval | null>(null);
  ...
  const [deletingResult, setDeletingResult] = useState<CtwAssessmentCounselingResult | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await ctwAssessmentCounselingResultService.getConsolidatedResults({
        course_id: courseId,
      ...
      if (response) {
        ...
        const assigns = await ctwUserAssignService.getAll({ user_id: user?.id });
  ...
  const isBatchPendingForMe = (result: CtwAssessmentCounselingResult): boolean => {
  ...
  void isBatchPendingForMe;

  const canCurrentAuthorityApprove = (result: CtwAssessmentCounselingResult): boolean => {
  ...
  const confirmForward = async () => {
    ...
    try {
      await ctwCounselingSemesterApprovalService.store({
        course_id: courseId,
        semester_id: semesterId,
        status: "pending",
  ...
  const handleAddCounseling = () => router.push("/ctw/assessments/counselings/results/create");
  const handleEdit = (result: CtwAssessmentCounselingResult) => router.push(`/ctw/assessments/counselings/results/${result.id}/edit`);
  const handleView = (result: CtwAssessmentCounselingResult) => router.push(`/ctw/assessments/counselings/results/${result.id}`);
  const handleDelete = (result: CtwAssessmentCounselingResult) => { setDeletingResult(result); setDeleteModalOpen(true); };

  const confirmDelete = async () => {
    if (!deletingResult) return;
    try {
      setDeleteLoading(true);
      await ctwAssessmentCounselingResultService.deleteResult(deletingResult.id);
      await loadData();
  ...
  const uncounseledRows = allCadets
    ...
    .map(c => ({
      ...
      cadet_row: {
        ...
        is_present: false,
        is_active: true,
        ctw_assessment_counseling_result_id: -1,
        cadet: c,
      },
    }));
      ...
      >
        <div className="p-6">
          <div className="text-center mb-4">
            ...
            <p className="text-sm font-medium text-gray-700 uppercase">Cadet Training Wing</p>
          </div>
        ...
        <div className="p-6">
          <div className="flex items-center gap-1 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">CTW counseling result forwarding</p>
            </div>
          </div>
