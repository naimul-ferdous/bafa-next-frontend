...
export default function CtwWarningsCourseSemesterResultPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  const handleEdit = (result: CadetWarning) => {
    router.push(`/ctw/assessments/warnings/${result.id}/edit`);
  };

  const handleView = (result: CadetWarning) => {
    router.push(`/ctw/assessments/warnings/${result.id}`);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwCadetWarningService.delete(deletingResult.id);
      await loadData();
    ...
    },
    {
      ...
      render: (res) => (
        <div className="flex items-center justify-center gap-1" onClick={(e) => e.stopPropagation()}>
          <button onClick={() => handleView(result)} className="p-1 text-blue-600 hover:bg-blue-50 rounded transition-all" title="View"><Icon icon="hugeicons:view" className="w-4 h-4" /></button>
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
            CTW Cadet Warnings Summary Sheet
          </p>
        </div>
