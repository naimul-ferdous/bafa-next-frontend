...
export default function CtwCadetWarningsPage() {
  ...
  const loadWarnings = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwCadetWarningService.getAll({
        page: currentPage,
        per_page: perPage,
  ...
  const loadOfficialResults = useCallback(async () => {
    try {
      ...
      const data = await ctwCadetWarningService.getGroupedResults({
        search: searchTerm || undefined
      });
  ...
  const handleAddWarning = () => router.push("/ctw/assessments/warnings/create");
  const handleEditWarning = (warning: CadetWarning) => router.push(`/ctw/assessments/warnings/${warning.id}/edit`);
  const handleViewWarning = (warning: CadetWarning) => router.push(`/ctw/assessments/warnings/${warning.id}`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwCadetWarningService.delete(deletingWarning.id);
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
            <button onClick={() => router.push(`/ctw/assessments/warnings/course/${row.course_details?.id}/semester/${row.semester_details?.id}`)} className="px-3 py-1 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg flex items-center gap-1 text-[10px] font-black uppercase transition-all" title="View Details">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase tracking-widest underline decoration-2 underline-offset-4">CTW Cadet Warnings Management</h2>
      </div>
