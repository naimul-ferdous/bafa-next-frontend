...
export default function CtwViewCadetWarningsPage() {
  const router = useRouter();
  const can = useCan("/ctw/assessments/warnings");
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwCadetWarningService.getGroupedResults({
        page: currentPage,
        per_page: perPage,
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          {can("view") && (
            <button
              onClick={() => router.push(`/ctw/assessments/warnings/course/${row.course_id}/semester/${row.semester_id}`)}
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase tracking-widest">CTW Cadet Warnings</h2>
      </div>
