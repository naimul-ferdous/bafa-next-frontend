...
export default function OlqResultDetailsPage() {
  ...
  useEffect(() => {
    const loadResult = async () => {
      try {
        setLoading(true);
        const data = await ctwAssessmentOlqResultService.getResult(parseInt(resultId));
        if (data) {
          setResult(data);
        } else {
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push(`/ctw/assessments/olq/results/${result.id}/edit`)}
                  ...
                  return(
                    <tr key={cadet.cadet_id}>
                      ...
                      {estimatedMarks.map((em, index) => {
                        const mark = cadet.marks?.find(m => m.ctw_assessment_olq_type_estimated_mark_id === em.id);
