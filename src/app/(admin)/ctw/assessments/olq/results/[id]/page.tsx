...
import { ctwOlqCadetApprovalService } from "@/libs/services/ctwOlqCadetApprovalService";
import { ctwOlqSemesterApprovalService } from "@/libs/services/ctwOlqSemesterApprovalService";
...
import type { CtwOlqCadetApproval } from "@/libs/types/ctwOlqCadetApproval";
...
export default function OlqResultDetailsPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [data, authRes] = await Promise.all([
        ctwAssessmentOlqResultService.getResult(parseInt(resultId)),
        ctwOlqApprovalAuthorityService.getAuthorities({ allData: true, is_active: true }),
      ]);
  ...
  const getCadetApproval = (cadetId: number, authId: number): CtwOlqCadetApproval | undefined =>
    approvals.find((a) => a.cadet_id === cadetId && a.authority_id === authId);

  const getCurrentCadetApproval = (cadetId: number): CtwOlqCadetApproval | undefined =>
  ...
  const confirmBulkAction = async () => {
    ...
    try {
      if (approvalModal.status === "approved") {
        await ctwAssessmentOlqResultService.bulkApprove({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
        });
      } else {
        await ctwAssessmentOlqResultService.bulkReject({
          result_id: result.id,
          cadet_ids: approvalModal.cadetIds,
          authority_id: currentAuthorityId,
  ...
  const confirmForward = async () => {
    ...
    try {
      await ctwOlqSemesterApprovalService.store({
  ...
  const calculateTotal = useCallback((cadetMarks: any[]) => {
    ...
    estimatedMarks.forEach((em) => {
      const mark = cadetMarks.find((m: any) => m.ctw_assessment_olq_type_estimated_mark_id === em.id);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ctw/assessments/olq/results")}
                    ...
                    return (
                      <tr key={cadet.cadet_id}>
                        ...
                        {estimatedMarks.map((em, i) => {
                          const mark = cadet.marks?.find((m: any) => m.ctw_assessment_olq_type_estimated_mark_id === em.id);
