...
export default function CtwViewAssessmentOlqResultsPage() {
  const router = useRouter();
  const { user } = useAuth();
  const can = useCan("/ctw/assessments/olq/results");
  ...
  useEffect(() => {
    if (!user?.id) return;
    ctwUserAssignService.getAll({ user_id: user.id }).then((data) => {
