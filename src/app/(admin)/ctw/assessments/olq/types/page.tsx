...
import { CtwAssessmentOlqType } from "@/libs/types/ctwAssessmentOlq";
...
export default function CtwAssessmentOlqTypesPage() {
  const router = useRouter();
  const { permissions } = usePageContext();
  const can = useCan();

  const [types, setTypes] = useState<CtwAssessmentOlqType[]>([]);
  const [loading, setLoading] = useState(true);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deletingType, setDeletingType] = useState<CtwAssessmentOlqType | null>(null);
  ...
  const loadTypes = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwAssessmentOlqTypeService.getAllTypes({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddType = () => router.push("/ctw/assessments/olq/types/create");
  const handleEditType = (type: CtwAssessmentOlqType) => router.push(`/ctw/assessments/olq/types/${type.id}/edit`);
  const handleViewType = (type: CtwAssessmentOlqType) => router.push(`/ctw/assessments/olq/types/${type.id}`);
  const handleDeleteType = (type: CtwAssessmentOlqType) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwAssessmentOlqTypeService.deleteType(deletingType.id);
      await loadTypes();
  ...
  const columns: Column<CtwAssessmentOlqType>[] = [
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">ctw Assessment OLQ Types</h2>
      </div>
