...
import type { CtwAssessmentOlqType, CtwAssessmentOlqTypeCreateData } from "@/libs/types/ctwAssessmentOlq";
...
export default function EditOlqTypePage() {
  ...
  const [type, setType] = useState<CtwAssessmentOlqType | null>(null);
  ...
  useEffect(() => {
    const loadType = async () => {
      try {
        setLoadingType(true);
        const typeData = await ctwAssessmentOlqTypeService.getType(parseInt(typeId));
        if (typeData) {
          setType(typeData);
        } else {
  ...
  const handleSubmit = async (data: CtwAssessmentOlqTypeCreateData) => {
    setLoading(true);
    try {
      await ctwAssessmentOlqTypeService.updateType(parseInt(typeId), data);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ctw/assessments/olq/types")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
            Back to OLQ Types
          </button>
        </div>
      </div>
