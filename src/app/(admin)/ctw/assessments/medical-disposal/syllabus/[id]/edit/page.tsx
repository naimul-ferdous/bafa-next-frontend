...
export default function EditCtwMedicalDisposalSyllabusPage() {
  ...
  const handleSubmit = async (data: SyllabusFormData) => {
    setLoading(true);
    try {
      await ctwMedicalDisposalSyllabusService.update(id, data);
      router.push(`/ctw/assessments/medical-disposal/syllabus/${id}`);
    } catch (err: any) {
      throw err;
    } finally {
      setLoading(false);
  ...
  if (error || !syllabus) {
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200 text-center py-16">
        ...
        <button onClick={() => router.push("/ctw/assessments/medical-disposal/syllabus")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-lg">
          Go Back
        </button>
      </div>
    );
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6 shadow-sm">
      ...
      <SyllabusForm
        ...
        onCancel={() => router.push(`/ctw/assessments/medical-disposal/syllabus/${id}`)}
        loading={loading}
        isEdit
      />
    </div>
