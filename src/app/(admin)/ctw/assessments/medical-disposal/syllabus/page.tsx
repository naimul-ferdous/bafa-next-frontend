...
export default function CtwMedicalDisposalSyllabusPage() {
  ...
  const [statusModal, setStatusModal] = useState<{ open: boolean; item: CtwMedicalDisposalSyllabus | null; loading: boolean }>({ open: false, item: null, loading: false });
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; item: CtwMedicalDisposalSyllabus | null; loading: boolean }>({ open: false, item: null, loading: false });

  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await ctwMedicalDisposalSyllabusService.getAll({ page: currentPage, per_page: perPage, search: searchTerm || undefined });
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      await ctwMedicalDisposalSyllabusService.update(statusModal.item.id, { is_active: !statusModal.item.is_active });
      await load();
  ...
  const confirmDelete = async () => {
    ...
    try {
      await ctwMedicalDisposalSyllabusService.delete(deleteModal.item.id);
      await load();
      setDeleteModal({ open: false, item: null, loading: false });
    } catch { setDeleteModal((p) => ({ ...p, loading: false })); }
  };

  const columns: Column<CtwMedicalDisposalSyllabus>[] = [
    {
