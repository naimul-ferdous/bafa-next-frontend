...
import { CtwAssessmentPenpictureGrade } from "@/libs/types/ctw";
...
export default function ctwAssessmentPenpictureGradesPage() {
  ...
  const [deletingGrade, setDeletingGrade] = useState<CtwAssessmentPenpictureGrade | null>(null);
  ...
  const loadGrades = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwAssessmentPenpictureGradeService.getAllGrades({
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddGrade = () => router.push("/ctw/assessments/penpicture/grades/create");
  const handleEditGrade = (grade: CtwAssessmentPenpictureGrade) => router.push(`/ctw/assessments/penpicture/grades/${grade.id}/edit`);
  const handleViewGrade = (grade: CtwAssessmentPenpictureGrade) => router.push(`/ctw/assessments/penpicture/grades/${grade.id}`);
  const handleDeleteGrade = (grade: CtwAssessmentPenpictureGrade) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwAssessmentPenpictureGradeService.deleteGrade(deletingGrade.id);
      await loadGrades();
  ...
  const columns: Column<CtwAssessmentPenpictureGrade>[] = [
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Assessment Pen Picture Grades</h2>
      </div>
