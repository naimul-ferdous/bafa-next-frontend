...
export default function CtwPenpictureCourseSemesterResultPage({ params }: { params: Promise<{ courseId: string; semesterId: string }> }) {
  ...
  const handleEdit = (result: CtwAssessmentPenpictureResult) => {
    router.push(`/ctw/assessments/penpicture/results/${result.id}/edit`);
  };

  const handleView = (result: CtwAssessmentPenpictureResult) => {
    router.push(`/ctw/assessments/penpicture/results/${result.id}`);
  };

  const handleDelete = (result: CtwAssessmentPenpictureResult) => {
  ...
  const columns: Column<CtwAssessmentPenpictureResult>[] = [
    ...
    { 
      key: "rank", 
      header: "Rank", 
      render: (res) => (res.cadet as any)?.assigned_ranks?.find((ar: any) => ar.is_current)?.rank?.short_name || (res.cadet as any)?.rank?.short_name || "Cdt" 
    },
    { 
      key: "name", 
      header: "Name", 
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
            CTW Pen Picture Summary Result Sheet
          </p>
        </div>
