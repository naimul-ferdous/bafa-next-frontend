...
export default function CtwViewAssessmentPenpictureResultsPage() {
  const router = useRouter();
  const { user, userIsInstructor } = useAuth();
  const can = useCan("/ctw/assessments/penpicture/results");
  ...
  const loadResults = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwAssessmentPenpictureResultService.getGroupedResults({
        page: currentPage,
        per_page: perPage,
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          {can("view") && (
            <button
              onClick={() => router.push(`/ctw/assessments/penpicture/results/course/${row.course_id}/semester/${row.semester_id}`)}
      ...
      {loading ? (
        ...
        <>
          <DataTable
            ...
            onRowClick={can("view") ? (row) => router.push(`/ctw/assessments/penpicture/results/course/${row.course_id}/semester/${row.semester_id}`) : undefined}
          />
