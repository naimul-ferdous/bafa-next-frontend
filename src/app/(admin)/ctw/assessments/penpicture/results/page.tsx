...
import { CtwAssessmentPenpictureResult } from "@/libs/types/ctw";
...
export default function CtwAssessmentPenpictureResultsPage() {
  ...
  const loadOfficialResults = useCallback(async () => {
    try {
      ...
      const data = await ctwAssessmentPenpictureResultService.getGroupedResults({
        search: searchTerm || undefined
      });
  ...
  const handleAddResult = () => router.push("/ctw/assessments/penpicture/results/create");
  const handleEditResult = (result: CtwAssessmentPenpictureResult) => router.push(`/ctw/assessments/penpicture/results/${result.id}/edit`);
  const handleViewResult = (result: CtwAssessmentPenpictureResult) => router.push(`/ctw/assessments/penpicture/results/${result.id}`);
  const handleDeleteResult = (result: CtwAssessmentPenpictureResult) => {
  ...
  const columns: Column<CtwAssessmentPenpictureResult>[] = [
    ...
    },
    {
      ...
      render: (result) => (
        <div>
          ...
          <div className="text-xs text-blue-600 font-mono font-bold uppercase">{result.cadet?.service_number || ""}</div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
            <button onClick={() => router.push(`/ctw/assessments/penpicture/results/course/${row.course_details?.id}/semester/${row.semester_details?.id}`)} className="px-3 py-1 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg flex items-center gap-1 text-[10px] font-black uppercase transition-all" title="View Results">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase tracking-widest">CTW Pen Picture Results</h2>
      </div>
