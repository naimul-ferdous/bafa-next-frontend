...
import { ctwCounselingApprovalAuthorityService } from "@/libs/services/ctwCounselingApprovalAuthorityService";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import CtwCounselingApprovalAuthorityModal from "@/components/ctw-results/CtwCounselingApprovalAuthorityModal";
import Pagination from "@/components/ui/Pagination";
import { useCan } from "@/context/PagePermissionsContext";

export default function CtwCounselingApprovalAuthoritiesPage() {
  const can = useCan();
  const [authorities, setAuthorities] = useState<CtwCounselingApprovalAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<CtwCounselingApprovalAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<CtwCounselingApprovalAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwCounselingApprovalAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
      ...
      console.error("Failed to load CTW Counseling approval authorities:", error);
    } finally {
      setLoading(false);
    }
  ...
  const handleEditAuthority = (authority: CtwCounselingApprovalAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: CtwCounselingApprovalAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ctwCounselingApprovalAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<CtwCounselingApprovalAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Counseling Assessment Approval Authority Management</h2>
      </div>
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No CTW Counseling approval authorities found"
        />
      )}
      ...
      <CtwCounselingApprovalAuthorityModal
        isOpen={modalOpen}
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to ${statusAuthority?.is_active ? "deactivate" : "activate"} this CTW Counseling approval authority?`}
