...
import { ctwOlqApprovalAuthorityService } from "@/libs/services/ctwOlqApprovalAuthorityService";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import CtwOlqApprovalAuthorityModal from "@/components/ctw-results/CtwOlqApprovalAuthorityModal";
import Pagination from "@/components/ui/Pagination";
import { useCan } from "@/context/PagePermissionsContext";

export default function CtwOlqApprovalAuthoritiesPage() {
  const can = useCan();
  const [authorities, setAuthorities] = useState<CtwOlqApprovalAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<CtwOlqApprovalAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<CtwOlqApprovalAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwOlqApprovalAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
      ...
      console.error("Failed to load CTW OLQ approval authorities:", error);
    } finally {
      setLoading(false);
    }
  ...
  const handleEditAuthority = (authority: CtwOlqApprovalAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: CtwOlqApprovalAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ctwOlqApprovalAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<CtwOlqApprovalAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
    ...
    },
    {
      ...
      render: (auth) => (
        <div className="flex flex-wrap gap-1">
          ...
          {auth.is_signature && (
            <span className="px-1.5 py-0.5 text-[10px] bg-yellow-50 text-yellow-700 border border-yellow-200 rounded">Signature</span>
          )}
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW OLQ Assessment Approval Authority Management</h2>
      </div>
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No CTW OLQ approval authorities found"
        />
      )}
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to ${statusAuthority?.is_active ? "deactivate" : "activate"} this CTW OLQ approval authority?`}
