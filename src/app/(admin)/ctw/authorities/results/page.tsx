...
import { CtwResultApprovalAuthority } from "@/libs/types/ctwApproval";
import { Icon } from "@iconify/react";
import { ctwApprovalService } from "@/libs/services/ctwApprovalService";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import CtwApprovalAuthorityModal from "@/components/ctw-results/CtwApprovalAuthorityModal";
import Pagination from "@/components/ui/Pagination";
import { useCan } from "@/context/PagePermissionsContext";

export default function CtwApprovalAuthoritiesPage() {
  const can = useCan();
  const [authorities, setAuthorities] = useState<CtwResultApprovalAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<CtwResultApprovalAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<CtwResultApprovalAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwApprovalService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        const arr = response as unknown as CtwResultApprovalAuthority[];
        setAuthorities(arr);
        setPagination({ current_page: 1, last_page: 1, per_page: arr.length, total: arr.length, from: 1, to: arr.length });
      }
    } catch (error) {
  ...
  const handleEditAuthority = (authority: CtwResultApprovalAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: CtwResultApprovalAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ctwApprovalService.updateAuthority(statusAuthority.id, {
  ...
  const columns: Column<CtwResultApprovalAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
    ...
    },
    {
      ...
      render: (auth) => (
        <div className="flex flex-wrap gap-1">
          {auth.is_initial_cadet_approve && <span className="px-1.5 py-0.5 text-[10px] bg-teal-50 text-teal-700 border border-teal-200 rounded">Initial Cadet</span>}
          {auth.is_initial_forward && <span className="px-1.5 py-0.5 text-[10px] bg-indigo-50 text-indigo-700 border border-indigo-200 rounded">Initial Fwd</span>}
          ...
          {auth.is_semester_forward && <span className="px-1.5 py-0.5 text-[10px] bg-cyan-50 text-cyan-700 border border-cyan-200 rounded">Semester Fwd</span>}
          ...
          {!auth.is_cadet_approve && !auth.is_forward && !auth.is_final && !auth.is_initial_cadet_approve && !auth.is_initial_forward && !auth.is_semester_forward && !auth.is_signature && (
            <span className="text-gray-400 text-xs">—</span>
          )}
        </div>
      ),
    ...
    },
    {
      ...
      render: (auth) => (
        <div className="flex items-center justify-center gap-1" onClick={(e) => e.stopPropagation()}>
          ...
          {can('delete') ? (
            auth.is_active ? (
              <button onClick={() => handleToggleStatus(auth)} className="p-1 text-red-600 hover:bg-red-50 rounded transition-colors" title="Deactivate">
                <Icon icon="hugeicons:unavailable" className="w-4 h-4" />
              </button>
            ) : (
              <button onClick={() => handleToggleStatus(auth)} className="p-1 text-green-600 hover:bg-green-50 rounded transition-colors" title="Activate">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Approval Authority Management</h2>
      </div>
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No CTW approval authorities found"
        />
      )}
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to ${statusAuthority?.is_active ? "deactivate" : "activate"} this approval authority?`}
