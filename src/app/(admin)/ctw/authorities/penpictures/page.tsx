...
import { CtwPenpictureAuthority } from "@/libs/types/ctwPenpictureAuthority";
import { Icon } from "@iconify/react";
import { ctwPenpictureAuthorityService } from "@/libs/services/ctwPenpictureAuthorityService";
import FullLogo from "@/components/ui/fulllogo";
import DataTable, { Column } from "@/components/ui/DataTable";
import ConfirmationModal from "@/components/ui/modal/ConfirmationModal";
import CtwPenpictureApprovalAuthorityModal from "@/components/ctw-results/CtwPenpictureApprovalAuthorityModal";
import Pagination from "@/components/ui/Pagination";
import { useCan } from "@/context/PagePermissionsContext";

export default function CtwPenpictureApprovalAuthoritiesPage() {
  const can = useCan();
  const [authorities, setAuthorities] = useState<CtwPenpictureAuthority[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingAuthority, setEditingAuthority] = useState<CtwPenpictureAuthority | null>(null);
  ...
  const [statusAuthority, setStatusAuthority] = useState<CtwPenpictureAuthority | null>(null);
  const [statusLoading, setStatusLoading] = useState(false);

  const loadAuthorities = useCallback(async () => {
    try {
      setLoading(true);
      const response = await ctwPenpictureAuthorityService.getAuthorities({
        page: currentPage,
        per_page: perPage,
      });
      ...
      console.error("Failed to load CTW penpicture authorities:", error);
    } finally {
      setLoading(false);
    }
  ...
  const handleEditAuthority = (authority: CtwPenpictureAuthority) => {
    setEditingAuthority(authority);
    setModalOpen(true);
  };

  const handleToggleStatus = (authority: CtwPenpictureAuthority) => {
  ...
  const confirmToggleStatus = async () => {
    ...
    try {
      ...
      await ctwPenpictureAuthorityService.update(statusAuthority.id, {
  ...
  const columns: Column<CtwPenpictureAuthority>[] = [
    {
      key: "id",
      header: "SL.",
      headerAlign: "center",
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Pen Picture Authority Management</h2>
      </div>
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage="No CTW pen picture authorities found"
        />
      )}
      ...
      <CtwPenpictureApprovalAuthorityModal
        isOpen={modalOpen}
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to ${statusAuthority?.is_active ? "deactivate" : "activate"} this CTW pen picture authority?`}
