...
export default function FootDrillResultDetailsPage() {
  ...
  const loadData = useCallback(async () => {
    if (footDrillModuleId === null || resultId === undefined) return;
    try {
      setLoading(true);
      const data = await ctwFootDrillResultService.getResult(footDrillModuleId, parseInt(resultId));
      if (data) {
  ...
  const handleConfirmResubmit = async () => {
    ...
    if (!item || !footDrillModuleId) return;
      ...
      >
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Module: <span className="font-medium text-gray-700">Foot Drill</span>
          </p>
