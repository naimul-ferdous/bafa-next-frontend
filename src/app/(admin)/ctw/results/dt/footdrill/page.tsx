...
export default function CtwFootDrillResultsPage() {
  ...
  const [deletingResult, setDeletingResult] = useState<CtwFootDrillResult | null>(null);
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwFootDrillResultService.getAllResults(footDrillModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, footDrillModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (footDrillModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwFootDrillResultService.getGroupedResults({
  ...
  }, [footDrillModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && footDrillModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, footDrillModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/dt/footdrill/create");
  const handleEditResult = (result: CtwFootDrillResult) => router.push(`/ctw/results/dt/footdrill/${result.id}/edit`);
  const handleViewResult = (result: CtwFootDrillResult | any) => router.push(`/ctw/results/dt/footdrill/${result.id}`);
  const handleDeleteResult = (result: CtwFootDrillResult) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwFootDrillResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<CtwFootDrillResult>[] = [
    ...
    {
      key: "actions" as keyof CtwFootDrillResult,
      header: "Actions",
      headerAlign: "center" as const,
      className: "text-center no-print",
      render: (result: CtwFootDrillResult) => (
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/dt/footdrill/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Foot Drill Results</h2>
      </div>
