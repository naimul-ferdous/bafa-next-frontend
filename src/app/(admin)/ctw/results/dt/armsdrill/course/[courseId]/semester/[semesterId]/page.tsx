...
export default function ArmsDrillCourseSemesterResultPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const data = await ctwArmsDrillResultService.getInitialFetchData({
      ...
      if (data) {
        ...
        const approvals = data.cadet_approvals || [];
        setApprovalAuthorities(data.approval_authorities || []);
        setCadetApprovals(approvals);
        setModuleApprovals(data.module_approvals || []);

        console.log('[DEBUG] cadet_approvals count:', approvals.length);
        console.log('[DEBUG] cadet_approvals ctw_result_ids:', [...new Set(approvals.map((a: any) => a.ctw_result_id))]);
        if (approvals.length > 0) console.log('[DEBUG] first approval:', JSON.stringify(approvals[0]));
        ...
        if (data.grouped_results && data.grouped_results.length > 0) {
          ...
          console.log('[DEBUG] submissions IDs:', resultGroup.submissions?.map((s: any) => s.id));
        } else {
  ...
  // Use primary role for authority matching, fall back to all roles
  ...
  // isForwarded: only true when ALL results have a module_approval record
  ...
  // --- Data calculations (must be before approval logic that references aggregatedMarks) ---
    ...
    submissions.forEach(sub => {
      ...
      marks.forEach((markItem: any) => {
        ...
        if (cadetMap.has(cadetId)) {
          ...
          cadetData.instructorMarks[instructorId] = markVal;
  ...
  const allResultsApprovedByPreviousLevel = useMemo(() => {
    ...
    const prevAuthorities = approvalAuthorities.filter((a: any) =>
      a.is_active && (a.sort ?? 0) < mySort
    );
  ...
  const allCadetsApproved = useMemo(() => {
    const marks = aggregatedMarks;
    if (marks.length === 0 || !myAuthority) return false;
    const authorityId = (myAuthority as any)?.id;
    return marks.every((item: any) => {
  ...
  // Per-result approval status for course-semester page (ATW-style timeline)
  // For each authority in the timeline, check its approval for this result+cadet
  ...
  // Check if ALL results are approved (for forward/approve gating)
  const allResultsApproved = useMemo(() => {
  ...
  }, [submissions, aggregatedMarks, cadetApprovals]);
  ...
  const confirmApproval = async () => {
    ...
    try {
      // Approve for each instructor's result (one approval per result)
      for (const sub of submissions) {
        await ctwApprovalService.approveCadets({
          course_id: courseId,
          semester_id: semesterId,
          module_id: moduleDetails.id,
          ctw_result_id: sub?.id,
  ...
  const confirmForward = async () => {
    ...
    try {
      ...
      // Forward for each instructor's result (one approval per result)
      for (const sub of submissions) {
  ...
  const rankedData = useMemo(() => {
    ...
    const withConv = aggregatedMarks.map(item => ({
      ...item,
      convertedMark: isComplete && totalWeight > 0
        ? (item.totalAchieved / totalWeight) * convMarkLimit
        : 0,
      position: 0,
      remark: "-",
    }));
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ctw/results/dt/armsdrill")}
          ...
          {canApproveAction && selectedCadetIds.length > 0 && allInstructorsSubmitted && (
            <button
              onClick={() => setApprovalModal({
                open: true, cadetIds: selectedCadetIds,
                status: "approved", rejectedReason: "", loading: false, error: ""
              })}
                      ...
                      {canApproveAction && !isForwarded && (
                        <td className="border border-black px-2 py-2 text-center no-print">
                          {isPending ? (
                            <button
                              onClick={() => setApprovalModal({
                                open: true, cadetIds: [item.cadet.id],
                                status: "approved", rejectedReason: "", loading: false, error: ""
                              })}
      ...
      <Modal isOpen={approvalModal.open} onClose={() => setApprovalModal(prev => ({ ...prev, open: false }))} showCloseButton className="max-w-lg">
        <div className="p-6">
          <div className="text-center mb-4">
            ...
            <p className="text-sm font-medium text-gray-700 uppercase">CTW Arms Drill</p>
          </div>
