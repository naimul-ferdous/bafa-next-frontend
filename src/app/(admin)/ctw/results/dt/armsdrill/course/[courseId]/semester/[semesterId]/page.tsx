...
export default function ArmsDrillCourseSemesterResultPage() {
  ...
  // Aggregate marks by cadet
  const aggregatedMarks = useMemo(() => {
    const cadetMap = new Map<number, any>();

    // Initialize map with all cadets from the course
    cadets.forEach(cadet => {
      cadetMap.set(cadet.id, {
        cadet: cadet,
        instructorMarks: {}, // instructor_id -> mark
        totalAchieved: 0,
        submissionCount: 0
      });
    });

    // Fill in marks from submissions
    ...
    // Filter to show only cadets who have at least one mark
  ...
  // Only calculates Total/Conv/Position/Remarks when all expected instructors have submitted
  const rankedData = useMemo(() => {
    ...
    // Re-sort by cadet number for display (positions already assigned above)
    withConv.sort((a, b) => {
  ...
  // Slots for all expected instructor columns (even those not yet submitted)
      ...
      <div className="p-8 cv-content">
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
            CTW {moduleDetails?.full_name || "Arms Drill"} Course Result Sheet
          </p>
        </div>
