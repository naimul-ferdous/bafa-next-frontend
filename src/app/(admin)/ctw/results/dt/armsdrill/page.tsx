...
export default function CtwArmsDrillResultsPage() {
  ...
  const [deletingResult, setDeletingResult] = useState<CtwArmsDrillResult | null>(null);
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwArmsDrillResultService.getAllResults(armsDrillModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, armsDrillModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (armsDrillModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwArmsDrillResultService.getGroupedResults({
  ...
  useEffect(() => {
    if (!moduleLoading && armsDrillModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, armsDrillModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/dt/armsdrill/create");
  const handleEditResult = (result: CtwArmsDrillResult) => router.push(`/ctw/results/dt/armsdrill/${result.id}/edit`);
  const handleViewResult = (result: CtwArmsDrillResult | any) => router.push(`/ctw/results/dt/armsdrill/${result.id}`);
  const handleDeleteResult = (result: CtwArmsDrillResult) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwArmsDrillResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<CtwArmsDrillResult>[] = [
    ...
    {
      key: "actions" as keyof CtwArmsDrillResult,
      header: "Actions",
      headerAlign: "center" as const,
      className: "text-center no-print",
      render: (result: CtwArmsDrillResult) => (
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
            <button onClick={() => router.push(`/ctw/results/dt/armsdrill/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Arms Drill Results</h2>
      </div>
