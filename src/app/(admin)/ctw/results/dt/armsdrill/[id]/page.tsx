...
export default function ArmsDrillResultDetailsPage() {
  ...
  useEffect(() => {
    const loadResult = async () => {
      if (armsDrillModuleId === null || resultId === undefined) {
        return; // Don't load until module ID is fetched and resultId is available
      }

      try {
        setLoading(true);
        const data = await ctwArmsDrillResultService.getResult(armsDrillModuleId, parseInt(resultId)); // Pass module ID
        if (data) {
          setResult(data);
        } else {
  ...
  }, [resultId, armsDrillModuleId, moduleLoading]); // Add armsDrillModuleId and moduleLoading to dependencies
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push(`/ctw/results/dt/armsdrill/${result.id}/edit`)}
