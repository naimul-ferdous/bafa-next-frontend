...
export default function ArmsDrillResultDetailsPage() {
  ...
  const loadData = useCallback(async () => {
    if (armsDrillModuleId === null || resultId === undefined) return;
    try {
      setLoading(true);
      const data = await ctwArmsDrillResultService.getResult(armsDrillModuleId, parseInt(resultId));
      if (data) {
  ...
  }, [result, armsDrillModuleId]);

  useEffect(() => {
    loadRejectedPanel();
  }, [loadRejectedPanel]);

  // --- Authority / permission logic (mirrors ATW) ---
  ...
  const handleConfirmResubmit = async () => {
    ...
    if (!item || !armsDrillModuleId) return;
              ...
              <table className="w-full border-collapse border border-black text-sm">
                <thead>
                  <tr>
                    ...
                    <th rowSpan={2} className="border border-black px-2 py-2 text-center align-middle">Achieved Mark</th>
                    <th rowSpan={2} className="border border-black px-2 py-2 text-center align-middle no-print">Status</th>
                    {canApprove && !isForwarded && (
      ...
      >
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Module: <span className="font-medium text-gray-700">Arms Drill</span>
          </p>
