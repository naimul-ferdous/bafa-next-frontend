...
export default function WrittenResultDetailsPage() {
  ...
  const loadData = useCallback(async () => {
    if (writtenModuleId === null || resultId === undefined) return;
    try {
      setLoading(true);
      const data = await ctwWrittenResultService.getResult(writtenModuleId, parseInt(resultId));
      if (data) {
  ...
  const confirmApproval = async () => {
    if (!result || !writtenModuleId) return;
  ...
  const rankedMarks = useMemo(() => {
    ...
    const parsed = marks.map((m: any) => ({
      ...m,
      markValue: parseFloat(String(m.achieved_mark || 0)),
      position: 0,
      remark: "-",
    }));
    parsed.sort((a: any, b: any) => b.markValue - a.markValue);
    parsed.forEach((item: any, idx: number) => {
      if (idx === 0) item.position = 1;
      else if (item.markValue === parsed[idx - 1].markValue) item.position = parsed[idx - 1].position;
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ctw/results/dt/written")}
              ...
              <table className="w-full border-collapse border border-black text-sm">
                ...
                <tbody>
                    ...
                    return (
                      <tr key={mark.id}>
                        ...
                        <td className="border border-black px-2 py-2 text-center text-blue-700 font-bold">
                          {mark.markValue.toFixed(2)}
                        </td>
                        <td className="border border-black px-2 py-2 text-center">{getOrdinal(mark.position)}</td>
                        <td className={`border border-black px-2 py-2 text-center ${mark.remark === "Failed" ? "text-red-600" : "text-gray-400"}`}>{mark.remark}</td>
                  ...
                  <tr className="font-semibold">
                    <td colSpan={6 + (canApproveAction && pendingCadetIds.length > 0 ? 1 : 0)} className="border border-black px-2 py-2 text-center font-bold">TOTAL</td>
                    <td className="border border-black px-2 py-2 text-center text-blue-700 font-bold">
                      {calculateTotalMarks().toFixed(2)}
                    </td>
                    <td colSpan={3 + (canApproveAction && !isForwarded ? 1 : 0)} className="border border-black px-2 py-2 no-print"></td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      >
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Module: <span className="font-medium text-gray-700">Written</span>
          </p>
