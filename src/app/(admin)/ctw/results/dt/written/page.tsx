...
export default function CtwWrittenResultsPage() {
  ...
  const [deletingResult, setDeletingResult] = useState<CtwWrittenResult | null>(null);
  ...
  }, [currentPage, perPage, searchTerm, writtenModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (writtenModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwWrittenResultService.getGroupedResults({
  ...
  }, [writtenModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && writtenModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, writtenModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/dt/written/create");
  const handleEditResult = (result: CtwWrittenResult) => router.push(`/ctw/results/dt/written/${result.id}/edit`);
  const handleViewResult = (result: CtwWrittenResult | any) => router.push(`/ctw/results/dt/written/${result.id}`);
  const handleDeleteResult = (result: CtwWrittenResult) => { setDeletingResult(result); setDeleteModalOpen(true); };

  const confirmDelete = async () => {
    if (!deletingResult) return;
    try {
      setDeleteLoading(true);
      await ctwWrittenResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<CtwWrittenResult>[] = [
    ...
    {
      key: "actions" as keyof CtwWrittenResult, header: "Actions", headerAlign: "center" as const, className: "text-center no-print",
      render: (result: CtwWrittenResult) => (
    ...
    { key: "created_at", header: "Created At", className: "text-gray-700 text-sm", render: (row) => row.submissions?.[0]?.created_at ? new Date(row.submissions[0].created_at).toLocaleDateString("en-GB") : "—" },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/dt/written/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Written Results</h2>
      </div>
