...
export default function WrittenCourseSemesterResultPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const data = await ctwWrittenResultService.getInitialFetchData({
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ctw/results/dt/written")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-all font-bold">
            Back to Results
          </button>
        </div>
      </div>
  ...
  return (
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        <button onClick={() => router.push("/ctw/results/dt/written")} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 font-medium transition-all">
      ...
      <div className="p-8 cv-content">
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
            CTW {moduleDetails?.full_name || "Written"} Course Result Sheet
          </p>
        </div>
                                      ...
                                      return (
                                        <div key={auth.id} className="flex items-center gap-1">
                                          <span className="text-[8px] text-gray-500 flex-1">
                                            {auth.role?.name || `Auth ${auth.sort}`}{isMe && <span className="text-gray-700">(me)</span>}
                                          </span>
                                          {authStatus === "approved" ? (
      ...
      <Modal isOpen={approvalModal.open} onClose={() => setApprovalModal(prev => ({ ...prev, open: false }))} showCloseButton className="max-w-lg">
        <div className="p-6">
          <div className="text-center mb-4">
            ...
            <p className="text-sm font-medium text-gray-700 uppercase">CTW Written</p>
          </div>
