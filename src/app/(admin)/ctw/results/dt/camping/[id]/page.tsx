...
export default function CampingResultDetailsPage() {
    ...
    const fetchModuleId = async () => {
      try {
        ...
        if (campingModule) {
          ...
          setError(`Module with code ${CAMPING_MODULE_CODE} not found.`);
        }
      } catch (err) {
  ...
  useEffect(() => {
    const loadResult = async () => {
      if (campingModuleId === null || resultId === undefined) {
        return; // Don't load until module ID is fetched and resultId is available
      }

      try {
        setLoading(true);
        const data = await ctwDrillResultService.getResult(campingModuleId, parseInt(resultId)); // Pass module ID
        if (data) {
          setResult(data);
        } else {
  ...
  }, [resultId, campingModuleId, moduleLoading]); // Add campingModuleId and moduleLoading to dependencies
