...
export default function CampingCourseSemesterResultPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const data = await ctwDrillResultService.getInitialFetchData({
        module_code: CAMPING_MODULE_CODE,
        course_id: courseId,
  ...
  // Check if ALL instructor results are approved by the PREVIOUS authority level
  ...
  // Per-result approval status for course-semester page
  ...
  // Rank cadets by converted mark, assign ordinal positions and remarks
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ctw/results/dt/camping")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-all font-bold">
            Back to Results
          </button>
        </div>
      </div>
  ...
  return (
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        <button onClick={() => router.push("/ctw/results/dt/camping")} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 font-medium transition-all">
        ...
        <div className="mb-8">
          ...
          <div className="mb-2">
            ...
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">
              {courseDetails?.name} ({moduleDetails?.full_name || "Camping"})
            </p>
                    ...
                    <tr key={item.cadet.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      {canApproveAction && !isForwarded && (
                        <td className="border border-black px-2 py-2 text-center no-print">
                          {isPending ? (
                            <button
                              onClick={() => setApprovalModal({ open: true, cadetIds: [item.cadet.id], status: "approved", rejectedReason: "", loading: false, error: "" })}
