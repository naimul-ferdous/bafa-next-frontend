...
export default function CtwCampingDrillResultsPage() {
  ...
  useEffect(() => {
    const fetchModuleId = async () => {
      try {
        ...
        const campingModule = modulesRes.data.find((m: any) => m.code === CAMPING_MODULE_CODE);
        if (campingModule) {
          setCampingModuleId(campingModule.id);
        } else {
          console.error(`Module with code ${CAMPING_MODULE_CODE} not found.`);
        }
      } catch (err) {
  ...
  const loadResults = useCallback(async () => {
    if (campingModuleId === null || !user?.id) return;
    try {
      setLoading(true);
      const response = await ctwDrillResultService.getAllResults(campingModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, campingModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (campingModuleId === null) return;
    try {
  ...
  }, [campingModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && campingModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, campingModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/dt/camping/create");
  const handleEditResult = (result: CtwDrillResult) => router.push(`/ctw/results/dt/camping/${result.id}/edit`);
  const handleViewResult = (result: CtwDrillResult | any) => router.push(`/ctw/results/dt/camping/${result.id}`);
    ...
    { key: "created_at", header: "Created At", className: "text-gray-700 text-sm", render: (row) => row.submissions?.[0]?.created_at ? new Date(row.submissions[0].created_at).toLocaleDateString("en-GB") : "—" },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/dt/camping/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Camping Results</h2>
      </div>
