...
export default function CtwLeadershipSkillResultsPage() {
  ...
  const [deletingResult, setDeletingResult] = useState<CtwLeadershipSkillResult | null>(null);
  ...
  }, [currentPage, perPage, searchTerm, leadershipSkillModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (leadershipSkillModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwLeadershipSkillResultService.getGroupedResults({
        ctw_results_module_id: leadershipSkillModuleId,
        search: searchTerm || undefined,
      });
      const flattened: any[] = [];
      data.forEach(course => {
  ...
  }, [leadershipSkillModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && leadershipSkillModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, leadershipSkillModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/dt/leadershipskill/create");
  const handleEditResult = (result: CtwLeadershipSkillResult) => router.push(`/ctw/results/dt/leadershipskill/${result.id}/edit`);
  const handleViewResult = (result: CtwLeadershipSkillResult | any) => router.push(`/ctw/results/dt/leadershipskill/${result.id}`);
  const handleDeleteResult = (result: CtwLeadershipSkillResult) => { setDeletingResult(result); setDeleteModalOpen(true); };

  const confirmDelete = async () => {
    if (!deletingResult) return;
    try {
      setDeleteLoading(true);
      await ctwLeadershipSkillResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<CtwLeadershipSkillResult>[] = [
    ...
    {
      key: "actions" as keyof CtwLeadershipSkillResult, header: "Actions", headerAlign: "center" as const, className: "text-center no-print",
      render: (result: CtwLeadershipSkillResult) => (
    ...
    { key: "created_at", header: "Created At", className: "text-gray-700 text-sm", render: (row) => row.submissions?.[0]?.created_at ? new Date(row.submissions[0].created_at).toLocaleDateString("en-GB") : "—" },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/dt/leadershipskill/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Leadership Skill Results</h2>
      </div>
