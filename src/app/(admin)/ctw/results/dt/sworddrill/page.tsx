...
export default function CtwSwordDrillResultsPage() {
  ...
  const [deletingResult, setDeletingResult] = useState<CtwSwordDrillResult | null>(null);
  ...
  }, [currentPage, perPage, searchTerm, swordDrillModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (swordDrillModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwSwordDrillResultService.getGroupedResults({
        ctw_results_module_id: swordDrillModuleId,
        search: searchTerm || undefined,
      });
      const flattened: any[] = [];
      data.forEach(course => {
  ...
  }, [swordDrillModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && swordDrillModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, swordDrillModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/dt/sworddrill/create");
  const handleEditResult = (result: CtwSwordDrillResult) => router.push(`/ctw/results/dt/sworddrill/${result.id}/edit`);
  const handleViewResult = (result: CtwSwordDrillResult | any) => router.push(`/ctw/results/dt/sworddrill/${result.id}`);
  const handleDeleteResult = (result: CtwSwordDrillResult) => { setDeletingResult(result); setDeleteModalOpen(true); };

  const confirmDelete = async () => {
    if (!deletingResult) return;
    try {
      setDeleteLoading(true);
      await ctwSwordDrillResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<CtwSwordDrillResult>[] = [
    ...
    {
      key: "actions" as keyof CtwSwordDrillResult, header: "Actions", headerAlign: "center" as const, className: "text-center no-print",
      render: (result: CtwSwordDrillResult) => (
    ...
    { key: "created_at", header: "Created At", className: "text-gray-700 text-sm", render: (row) => row.submissions?.[0]?.created_at ? new Date(row.submissions[0].created_at).toLocaleDateString("en-GB") : "—" },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/dt/sworddrill/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Sword Drill Results</h2>
      </div>
