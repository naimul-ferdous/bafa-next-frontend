...
export default function SwordDrillResultDetailsPage() {
  ...
  const loadData = useCallback(async () => {
    if (swordDrillModuleId === null || resultId === undefined) return;
    try {
      setLoading(true);
      const data = await ctwSwordDrillResultService.getResult(swordDrillModuleId, parseInt(resultId));
      if (data) {
  ...
  const handleConfirmResubmit = async () => {
    ...
    if (!item || !swordDrillModuleId) return;
      ...
      >
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Module: <span className="font-medium text-gray-700">Sword Drill</span>
          </p>
