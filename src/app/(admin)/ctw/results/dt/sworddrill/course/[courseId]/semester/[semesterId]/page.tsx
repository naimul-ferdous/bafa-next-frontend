...
export default function SwordDrillCourseSemesterResultPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const data = await ctwSwordDrillResultService.getInitialFetchData({
        module_code: SWORD_DRILL_MODULE_CODE,
        course_id: courseId,
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ctw/results/dt/sworddrill")}
        ...
        <div className="mb-8">
          ...
          <div className="mb-2">
            ...
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">
              {courseDetails?.name} ({moduleDetails?.full_name || "Sword Drill"})
            </p>
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">
                  ...
                  return (
                    <tr key={item.cadet.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="border border-black px-2 py-2">
                        {item.cadet?.assigned_branchs?.find((b: any) => b.is_current)?.branch?.name || item.cadet?.assigned_branchs?.[0]?.branch?.name || "-"}
                      </td>
                      {instructorSlots.map(i => {
      ...
      <Modal isOpen={approvalModal.open} onClose={() => setApprovalModal(prev => ({ ...prev, open: false }))} showCloseButton className="max-w-lg">
        <div className="p-6">
          <div className="text-center mb-4">
            ...
            <p className="text-sm font-medium text-gray-700 uppercase">CTW Sword Drill</p>
          </div>
