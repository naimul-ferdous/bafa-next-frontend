...
export default function ThreeKmCourseSemesterResultPage() {
  ...
  // Aggregated marks per cadet - handles all 3 report types
  const aggregatedMarks = useMemo(() => {
    const cadetMap = new Map<number, any>();
    const convPracticeWeight = parseFloat(String((estimatedMark as any)?.convert_of_practice || 0));
    const convExamWeight = parseFloat(String((estimatedMark as any)?.convert_of_exam || 0));
    ...
    cadets.forEach(cadet => {
      cadetMap.set(cadet.id, {
        cadet,
        instructorData: {} as Record<number, any>,
        instructorMarks: {} as Record<number, number>,
        instructorStationData: {} as Record<number, any>,
        totalFinal: 0,
        submissionCount: 0,
      });
    });
    ...
    submissions.forEach(sub => {
      ...
      marks.forEach((markItem: any) => {
        ...
        if (reportType === "practices") {
          ...
          cadetData.instructorData[instructorId] = { practices, avg_practice, exam_mark, conv_practice, conv_exam, finalMark };
          ...
          // Details score wise - like 2km5station
          ...
          const conv = maxTotal > 0 && convMarkLimit > 0 ? (totalMks / maxTotal) * convMarkLimit : totalMks;
          ...
          // Direct marks - like swimming/footdrill
          const markVal = markItem.achieved_mark ?? markItem.mark;
          const val = parseFloat(String(markVal || 0));
          cadetData.instructorMarks[instructorId] = val;
          cadetData.totalFinal += val;
          cadetData.submissionCount += 1;
        }
      });
    });
  ...
  }, [submissions, cadets, estimatedMark, stationDetails, reportType]);

  // Rank cadets
  const rankedData = useMemo(() => {
    const convMarkLimit = parseFloat(String(estimatedMark?.conversation_mark || 0));
    const estimatedMarkPerInst = parseFloat(String((estimatedMark as any)?.estimated_mark_per_instructor || convMarkLimit || 100));
    const expectedCount = moduleDetails?.instructor_count || 0;
    const isComplete = expectedCount > 0 && submissions.length >= expectedCount;
    const totalWeight = reportType === "direct_marks" ? estimatedMarkPerInst * expectedCount : 0;
    const passThreshold = convMarkLimit * 0.5;

    const withConv = aggregatedMarks.map(item => {
      let convertedMark = 0;
      if (isComplete && item.submissionCount > 0) {
        if (reportType === "direct_marks") {
          convertedMark = totalWeight > 0 ? (item.totalFinal / totalWeight) * convMarkLimit : 0;
        } else if (reportType === "practices") {
          convertedMark = item.totalFinal / item.submissionCount;
        } else {
          convertedMark = item.totalFinal / item.submissionCount;
        }
      }
    ...
    return withConv;
  }, [aggregatedMarks, estimatedMark, submissions, moduleDetails, reportType]);
  ...
  const allCadetsApprovedByMe = useMemo(() => {
    if (!myAuthority || rankedData.length === 0) return false;
    return rankedData.every((item: any) => {
  ...
  const myModuleApproval = myAuthority ? moduleApprovals?.find((sa: any) => sa.authority_id === (myAuthority as any)?.id) : null;
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ctw/results/pf/3km")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-all font-bold">
            Back to Results
          </button>
        </div>
      </div>
  ...
  return (
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        <button onClick={() => router.push("/ctw/results/pf/3km")} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 font-medium transition-all">
      ...
      <div className="p-8 cv-content">
        <div className="mb-8">
          ...
          <div className="mb-2">
            ...
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">
              {courseDetails?.name} ({moduleDetails?.full_name || "3KM"} - {reportTypeLabel})
            </p>
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">{semesterDetails?.name}</p>
          </div>
        </div>

        {/* === DIRECT MARKS REPORT (Swimming/FootDrill style) === */}
        {reportType === "direct_marks" && (
          <div className="mb-6">
              ...
              <table className="w-full border-collapse border border-black text-sm">
                <thead>
                  <tr>
                    ...
                    {instructorCount > 1 ? (
                      <>
                        <th colSpan={instructorCount} className="border border-black px-2 py-1 text-center align-middle font-bold">Instructors</th>
                      ...
                      <tr key={item.cadet.id} className="hover:bg-gray-50 transition-colors">
                        {canApproveAction && pendingCadetIds.length > 0 && allInstructorsSubmitted && (
                          <td className="border border-black px-2 py-2 text-center no-print">
                            {isPending ? <input type="checkbox" checked={selectedCadetIds.includes(item.cadet.id)} onChange={() => toggleCadet(item.cadet.id)} className="w-4 h-4" /> : <span className="text-xs text-gray-400">-</span>}
                          </td>
                        )}
                        ...
                        <td className="border border-black px-2 py-2">{item.cadet?.assigned_branchs?.filter((b: any) => b.is_current)?.[0]?.branch?.name || item.cadet?.assigned_branchs?.[0]?.branch?.name || "-"}</td>
                        {instructorCount > 1 ? (
                          <>
                            {instructorSlots.map(i => {
                              ...
                              return <td key={i} className="border border-black px-2 py-2 text-center">{mark !== undefined ? parseFloat(String(mark)).toFixed(1) : "-"}</td>;
                            })}
                            <td className="border border-black px-2 py-2 text-center font-bold">{isComplete ? item.totalFinal.toFixed(1) : "-"}</td>
                          </>
                        ) : (
                          <td className="border border-black px-2 py-2 text-center font-bold">{isComplete ? item.totalFinal.toFixed(1) : "-"}</td>
                        )}
                        ...
                        <td className={`border border-black px-2 py-2 text-center ${item.remark === "Failed" ? "text-red-600" : "text-gray-400"}`}>{item.remark}</td>
                                      ...
                                      {approvalAuthorities.filter((a: any) => a.is_active).sort((a: any, b: any) => (a.sort ?? 0) - (b.sort ?? 0)).map((auth: any) => {
                                        ...
                                        return (
                                          <div key={auth.id} className="flex items-center gap-1">
                                            <span className="text-[8px] text-gray-500 flex-1">{auth.role?.name || `Auth ${auth.sort}`}{isMe && <span className="text-blue-600">(me)</span>}</span>
                                            {authStatus === "approved" ? (
                                              <span className="inline-flex items-center gap-0.5 text-green-700 text-[8px] font-bold"><Icon icon="hugeicons:checkmark-circle-02" className="w-2.5 h-2.5" />Approved</span>
                                            ) : authStatus === "rejected" ? (
                                              <span className="inline-flex items-center gap-0.5 text-red-700 text-[8px] font-bold"><Icon icon="hugeicons:cancel-circle" className="w-2.5 h-2.5" />Rejected</span>
                                            ) : (
                                              <span className="inline-flex items-center gap-0.5 text-yellow-700 text-[8px] font-bold"><Icon icon="hugeicons:clock-01" className="w-2.5 h-2.5" />Pending</span>
                                            )}
                                          </div>
                                        );
                                      })}
        ...
        {/* === PRACTICES REPORT (OneMile style) === */}
              ...
              <table className="w-full border-collapse border border-black text-sm">
                <thead>
                  {instructorCount > 1 && (
                    <tr>
                      {canApproveAction && pendingCadetIds.length > 0 && allInstructorsSubmitted && (
                        <th rowSpan={hasPractices ? 3 : 2} className="border border-black px-2 py-2 text-center align-middle no-print">
                          <input type="checkbox" checked={allPendingSelected} onChange={toggleSelectAll} className="w-4 h-4" />
                        </th>
                      )}
                      ...
                      {instructorSlots.map(i => (
                        <th key={i} colSpan={hasPractices ? practiceCount + 5 : 5} className="border border-black px-2 py-1 text-center align-middle font-bold">{`Instr ${i + 1}`}</th>
                      ))}
                      <th rowSpan={hasPractices ? 3 : 2} className="border border-black px-2 py-2 text-center align-middle font-bold whitespace-nowrap">Conv<br />Mark</th>
                      ...
                      <th rowSpan={hasPractices ? 3 : 2} className="border border-black px-2 py-2 text-center align-middle no-print">Status</th>
                      {canApproveAction && !isForwarded && (
                        <th rowSpan={hasPractices ? 3 : 2} className="border border-black px-2 py-2 text-center align-middle no-print">Action</th>
                      )}
                    </tr>
                  )}
                  <tr>
                    {instructorCount === 1 && (
                      <>
                        {canApproveAction && pendingCadetIds.length > 0 && allInstructorsSubmitted && (
                          <th rowSpan={hasPractices ? 2 : 1} className="border border-black px-2 py-2 text-center align-middle no-print">
                            <input type="checkbox" checked={allPendingSelected} onChange={toggleSelectAll} className="w-4 h-4" />
                          </th>
                        )}
                    ...
                    {instructorSlots.map(i => (
                      <React.Fragment key={i}>
                        {hasPractices ? (
                          <th colSpan={practiceCount} className="border border-black px-2 py-1 text-center align-middle font-semibold text-xs">Practices</th>
                        ) : null}
                        <th rowSpan={hasPractices ? 2 : 1} className="border border-black px-2 py-1 text-center align-middle whitespace-nowrap text-xs font-semibold">Avg.<br />Prac</th>
                        <th rowSpan={hasPractices ? 2 : 1} className="border border-black px-2 py-1 text-center align-middle whitespace-nowrap text-xs font-semibold">Exam</th>
                        <th rowSpan={hasPractices ? 2 : 1} className="border border-black px-2 py-1 text-center align-middle whitespace-nowrap text-xs font-semibold">Prac<br />({(estimatedMark as any)?.convert_of_practice || 0}%)</th>
                        <th rowSpan={hasPractices ? 2 : 1} className="border border-black px-2 py-1 text-center align-middle whitespace-nowrap text-xs font-semibold">Exam<br />({(estimatedMark as any)?.convert_of_exam || 0}%)</th>
                        <th rowSpan={hasPractices ? 2 : 1} className="border border-black px-2 py-1 text-center align-middle text-xs font-semibold">Total</th>
                        <th rowSpan={hasPractices ? 2 : 1} className="border border-black px-2 py-1 text-center align-middle text-xs font-semibold whitespace-nowrap">Conv<br />({conversationMarkLimit})</th>
                      </React.Fragment>
                    ))}
                    {instructorCount === 1 && (
                      <>
                        ...
                        <th rowSpan={hasPractices ? 2 : 1} className="border border-black px-2 py-2 text-center align-middle no-print">Status</th>
                        {canApproveAction && !isForwarded && (
                          <th rowSpan={hasPractices ? 2 : 1} className="border border-black px-2 py-2 text-center align-middle no-print">Action</th>
                        )}
                      </>
                    )}
                  </tr>
                      ...
                      {instructorSlots.map(i => (
                        <React.Fragment key={i}>
                          {Array.from({ length: practiceCount }, (_, p) => (
                            <th key={p} className="border border-black px-1 py-1 text-center align-middle text-xs font-normal">P{p + 1}</th>
                          ))}
                        </React.Fragment>
                      ))}
                    </tr>
                ...
                <tbody>
                  {rankedData.map((item: any, index: number) => {
                    ...
                    <tr key={item.cadet.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        return (
                          <React.Fragment key={i}>
                            {hasPractices && Array.from({ length: practiceCount }, (_, p) => (
                              <td key={p} className="border border-black px-2 py-2 text-center">{instrData?.practices[p] !== undefined ? instrData.practices[p].toFixed(2) : "-"}</td>
                            ))}
                            <td className="border border-black px-2 py-2 text-center">{instrData !== undefined ? instrData.avg_practice.toFixed(2) : "-"}</td>
                            <td className="border border-black px-2 py-2 text-center">{instrData !== undefined ? instrData.exam_mark.toFixed(2) : "-"}</td>
                            <td className="border border-black px-2 py-2 text-center font-semibold">{instrData !== undefined ? instrData.conv_practice.toFixed(2) : "-"}</td>
                            <td className="border border-black px-2 py-2 text-center font-semibold">{instrData !== undefined ? instrData.conv_exam.toFixed(2) : "-"}</td>
                            <td className="border border-black px-2 py-2 text-center font-bold">{instrData !== undefined ? instrData.finalMark.toFixed(2) : "-"}</td>
                      ...
                      {instructorCount > 1 && (
                        <td className="border border-black px-2 py-2 text-center font-bold text-blue-700">{isComplete ? item.convertedMark.toFixed(2) : "-"}</td>
                      )}
                      <td className="border border-black px-2 py-2 text-center">{isComplete ? getOrdinal(item.position) : "-"}</td>
                      <td className={`border border-black px-2 py-2 text-center ${item.remark === "Failed" ? "text-red-600 font-semibold" : "text-gray-400"}`}>{item.remark}</td>
                      <td className="border border-black px-2 py-2 text-center no-print">
        ...
        {/* === DETAILS SCORE WISE REPORT (2KM5Station style) === */}
        {reportType === "details_score" && (
          <div className="mb-6">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse border border-black text-sm">
                <thead>
                  ...
                  {rankedData.map((item: any, index: number) => {
                    ...
                    return (
                      <tr key={item.cadet.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="border border-black px-2 py-2 text-center">{index + 1}</td>
                        <td className="border border-black px-2 py-2 text-center">{item.cadet?.cadet_number || "N/A"}</td>
                        <td className="border border-black px-2 py-2 text-center">{item.cadet?.assigned_ranks?.find((ar: any) => ar.rank)?.rank?.short_name || "-"}</td>
                        ...
                        <td className="border border-black px-2 py-2 text-center font-bold">{isComplete ? item.instructorStationData?.[submissions[0]?.instructor_details?.id]?.totalMks?.toFixed(2) || "0.00" : "-"}</td>
                        <td className="border border-black px-2 py-2 text-center font-bold">{isComplete ? item.convertedMark.toFixed(2) : "-"}</td>
                        <td className="border border-black px-2 py-2 text-center">{isComplete ? getOrdinal(item.position) : "-"}</td>
                        <td className={`border border-black px-2 py-2 text-center ${item.remark === "Failed" ? "text-red-600" : "text-gray-400"}`}>{item.remark}</td>
                        <td className="border border-black px-2 py-2 no-print">
