...
export default function ThreeKmResultDetailsPage() {
  ...
  useEffect(() => {
    const fetchModule = async () => {
      try {
        ...
        const ctwModule = modulesRes.data.find((m: any) => m.code === THREE_KM_MODULE_CODE);
        if (ctwModule) {
          setThreeKmModuleId(ctwModule.id);
          setThreeKmModule(ctwModule);
        } else {
  ...
  useEffect(() => {
    const loadEstimatedMarks = async () => {
      if (!threeKmModuleId || !result?.course_id || !result?.semester_id) {
        return;
      }
  ...
  useEffect(() => {
    const loadResult = async () => {
      if (threeKmModuleId === null || resultId === undefined) {
        return;
      }
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ctw/results/pf/3km")}
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push(`/ctw/results/pf/3km/${result.id}/edit`)}
                  ...
                  <table className="w-full border-collapse border border-black text-sm">
                    <thead>
                      <tr>
                        ...
                        {threeKmModule?.practice_count > 0 && (
                          <th className="border border-black px-2 py-2 text-center align-middle" colSpan={threeKmModule.practice_count}>Practices</th>
                        )}
                      ...
                      {threeKmModule?.practice_count > 0 && (
                        <tr>
                          {Array.from({ length: threeKmModule.practice_count }, (_, i) => i + 1).map(p => (
                      ...
                      {result.achieved_marks.map((mark, index) => {
                        ...
                        return (
                          <tr key={mark.id}>
                            ...
                            {threeKmModule?.practice_count > 0 && (
                              Array.from({ length: threeKmModule.practice_count }, (_, i) => (
