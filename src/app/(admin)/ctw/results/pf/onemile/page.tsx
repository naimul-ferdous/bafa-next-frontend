...
export default function CtwOneMileResultsPage() {
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwOneMileResultService.getAllResults(oneMileModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, oneMileModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (oneMileModuleId === null) return;
    try {
  ...
  }, [oneMileModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && oneMileModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, oneMileModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/pf/onemile/create");
  const handleEditResult = (result: CtwOneMileResult) => router.push(`/ctw/results/pf/onemile/${result.id}/edit`);
  const handleViewResult = (result: CtwOneMileResult | any) => router.push(`/ctw/results/pf/onemile/${result.id}`);
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/pf/onemile/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW One Mile Results</h2>
      </div>
