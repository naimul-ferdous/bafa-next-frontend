...
export default function OneMileResultDetailsPage() {
  ...
  useEffect(() => {
    const fetchModule = async () => {
      try {
        ...
        const ctwModule = modulesRes.data.find((m: any) => m.code === ONE_MILE_MODULE_CODE);
        if (ctwModule) {
          setOneMileModuleId(ctwModule.id);
          setOneMileModule(ctwModule);
        } else {
          console.error(`Module with code ${ONE_MILE_MODULE_CODE} not found.`);
          setError(`Module with code ${ONE_MILE_MODULE_CODE} not found.`);
        }
      } catch (err) {
  ...
  //   if (!result?.achieved_marks || !oneMileModule) return 0;

  //   const convPracticeWeight = parseFloat(oneMileModule.convert_of_practice || 0);
  //   const convExamWeight = parseFloat(oneMileModule.convert_of_exam || 0);

  //   return result.achieved_marks.reduce((sum, mark) => {
  ...
  //     const finalMark = conv_practice + conv_exam;
        ...
        {result.achieved_marks && result.achieved_marks.length > 0 && (
          <div className="mb-6">
            ...
            <div className="overflow-x-auto">
              {(() => {
                const convPracticeWeight = oneMileModule ? parseFloat(oneMileModule.convert_of_practice || 0) : 0;
                const convExamWeight = oneMileModule ? parseFloat(oneMileModule.convert_of_exam || 0) : 0;
                  ...
                  <table className="w-full border-collapse border border-black text-sm">
                    <thead>
                      <tr>
                        ...
                        {oneMileModule?.practice_count > 0 && (
                          <th className="border border-black px-2 py-2 text-center align-middle" colSpan={oneMileModule.practice_count}>Practices</th>
                        )}
                      ...
                      {oneMileModule?.practice_count > 0 && (
                        <tr>
                          {Array.from({ length: oneMileModule.practice_count }, (_, i) => i + 1).map(p => (
                      ...
                      {result.achieved_marks.map((mark, index) => {
                        ...
                        return (
                          <tr key={mark.id}>
                            ...
                            {oneMileModule?.practice_count > 0 && (
                              Array.from({ length: oneMileModule.practice_count }, (_, i) => (
