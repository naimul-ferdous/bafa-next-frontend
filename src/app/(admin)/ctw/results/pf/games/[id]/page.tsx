...
export default function GamesResultDetailsPage() {
  ...
  useEffect(() => {
    const loadResult = async () => {
      if (gamesModuleId === null || !resultId) return;
      try {
        setLoading(true);
  ...
  // Collect all unique detail columns across all achieved marks
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => router.push(`/ctw/results/pf/games/${result.id}/edit`)} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
              ...
              <table className="w-full border-collapse border border-black text-sm">
                <thead>
                  <tr>
                    ...
                    {hasDetails ? (
                      allDetails.map((det: any) => (
                        <th key={det.ctw_results_module_estimated_marks_details_id} className="border border-black px-2 py-2 text-center align-middle text-blue-700">
                    ...
                    <th className="border border-black px-2 py-2 text-center align-middle text-blue-700">Total</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={mark.id}>
                      ...
                      {hasDetails ? (
                        allDetails.map((det: any) => {
                          ...
                          return (
                            <td key={did} className="border border-black px-2 py-2 text-center text-blue-700 font-semibold">
                              {detEntry ? parseFloat(detEntry.marks || 0).toFixed(2) : "—"}
                            </td>
                          );
                        })
                      ) : (
                  ...
                  <tr className="font-semibold">
                    <td colSpan={hasDetails ? 5 + allDetails.length : 5} className="border border-black px-2 py-2 text-center font-bold">TOTAL</td>
                    <td className="border border-black px-2 py-2 text-center text-blue-700 font-bold">
