...
export default function GamesCourseSemesterResultPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const data = await ctwGamesResultService.getInitialFetchData({
        module_code: GAMES_MODULE_CODE,
        course_id: courseId,
    ...
    submissions.forEach(sub => {
      ...
      marks.forEach((markItem: any) => {
        ...
        if (cadetMap.has(cadetId)) {
          ...
          cadetData.instructorMarks[instructorId] = parseFloat(String(markVal || 0));
          cadetData.totalAchieved += parseFloat(String(markVal || 0));
  ...
  const rankedData = useMemo(() => {
    const estimatedMarkPerInst = parseFloat(String((estimatedMark as any)?.estimated_mark_per_instructor || 0));
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ctw/results/pf/games")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-all font-bold">
            Back to Results
          </button>
        </div>
      </div>
  ...
  const estimatedMarkPerInstructor = parseFloat(String((estimatedMark as any)?.estimated_mark_per_instructor || 0));
  ...
  return (
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      <div className="p-4 flex items-center justify-between no-print">
        <button onClick={() => router.push("/ctw/results/pf/games")} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 font-medium transition-all">
        ...
        <div className="mb-8">
          ...
          <div className="mb-2">
            ...
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">
              {courseDetails?.name} ({moduleDetails?.full_name || "Games"})
            </p>
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">{semesterDetails?.name}{examType ? ` | ${examType}` : ""}</p>
          </div>
        </div>
