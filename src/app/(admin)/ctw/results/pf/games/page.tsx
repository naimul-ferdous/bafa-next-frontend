...
import { CtwGamesResult } from "@/libs/types/ctwGames";
...
export default function CtwGamesResultsPage() {
  ...
  const [deletingResult, setDeletingResult] = useState<CtwGamesResult | null>(null);
  ...
  useEffect(() => {
    const fetchModuleId = async () => {
      try {
        ...
        if (gamesModule) {
          setGamesModuleId(gamesModule.id);
        } else {
          console.error(`Module with code ${GAMES_MODULE_CODE} not found.`);
        }
      } catch (err) {
  ...
  const loadResults = useCallback(async () => {
    if (gamesModuleId === null || !user?.id) return;
    try {
      setLoading(true);
      const response = await ctwGamesResultService.getAllResults(gamesModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, gamesModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (gamesModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwGamesResultService.getGroupedResults({
  ...
  }, [gamesModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && gamesModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, gamesModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/pf/games/create");
  const handleEditResult = (result: CtwGamesResult) => router.push(`/ctw/results/pf/games/${result.id}/edit`);
  const handleViewResult = (result: CtwGamesResult | any) => router.push(`/ctw/results/pf/games/${result.id}`);
  const handleDeleteResult = (result: CtwGamesResult) => { setDeletingResult(result); setDeleteModalOpen(true); };

  const confirmDelete = async () => {
    if (!deletingResult) return;
    try {
      setDeleteLoading(true);
      await ctwGamesResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<CtwGamesResult>[] = [
    ...
    {
      key: "actions" as keyof CtwGamesResult, header: "Actions", headerAlign: "center" as const, className: "text-center no-print",
      render: (result: CtwGamesResult) => (
    ...
    { key: "created_at", header: "Created At", className: "text-gray-700 text-sm", render: (row) => row.submissions?.[0]?.created_at ? new Date(row.submissions[0].created_at).toLocaleDateString("en-GB") : "—" },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/pf/games/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Games Results</h2>
      </div>
      ...
      {isInstructor ? (
        ...
        officialLoading ? <TableLoading /> : <DataTable columns={officialColumns} data={officialResults} keyExtractor={(row, index) => `official-${row.id}-${index}`} emptyMessage="No grouped results found" />
      )}

      <ConfirmationModal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} onConfirm={confirmDelete} title="Delete Result" message={`Are you sure you want to delete this result? This action cannot be undone.`} confirmText="Delete" cancelText="Cancel" loading={deleteLoading} variant="danger" />
    </div>
  );
}
