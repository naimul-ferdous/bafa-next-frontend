...
import { CtwSwimmingResult } from "@/libs/types/ctwSwimming";
...
export default function CtwSwimmingResultsPage() {
  ...
  const [deletingResult, setDeletingResult] = useState<CtwSwimmingResult | null>(null);
  ...
  const loadResults = useCallback(async () => {
    if (swimmingModuleId === null || !user?.id) { setLoading(false); return; }
    try {
      setLoading(true);
      const response = await ctwSwimmingResultService.getAllResults(swimmingModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, swimmingModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (swimmingModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwSwimmingResultService.getGroupedResults({
  ...
  }, [swimmingModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && swimmingModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, swimmingModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/pf/swimming/create");
  const handleEditResult = (result: CtwSwimmingResult) => router.push(`/ctw/results/pf/swimming/${result.id}/edit`);
  const handleViewResult = (result: CtwSwimmingResult | any) => router.push(`/ctw/results/pf/swimming/${result.id}`);
  const handleDeleteResult = (result: CtwSwimmingResult) => { setDeletingResult(result); setDeleteModalOpen(true); };

  const confirmDelete = async () => {
    if (!deletingResult) return;
    try {
      setDeleteLoading(true);
      await ctwSwimmingResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<CtwSwimmingResult>[] = [
    ...
    {
      key: "actions" as keyof CtwSwimmingResult, header: "Actions", headerAlign: "center" as const, className: "text-center no-print",
      render: (result: CtwSwimmingResult) => (
    ...
    { key: "created_at", header: "Created At", className: "text-gray-700 text-sm", render: (row) => row.submissions?.[0]?.created_at ? new Date(row.submissions[0].created_at).toLocaleDateString("en-GB") : "—" },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/pf/swimming/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Swimming Results</h2>
      </div>
