...
export default function PtExamResultDetailsPage() {
  ...
  const loadData = useCallback(async () => {
    if (ptExamModuleId === null || resultId === undefined) return;
    try {
      setLoading(true);
      const data = await ctwPtExamResultService.getResult(ptExamModuleId, parseInt(resultId));
      if (data) {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ctw/results/pf/ptexam")}
                ...
                <thead>
                  <tr>
                    ...
                    {hasStationData && stationDetails.map((sd: any, i: number) => (
                      <th key={sd.id || i} className="border border-black px-1 py-1 text-center align-middle" colSpan={hasStationQty(sd) ? 2 : 1}>
                      ...
                      {stationDetails.map((sd: any, i: number) => {
                        ...
                        return (
                          <React.Fragment key={sd.id || i}>
                            {needsQty && <th className="border border-black px-1 py-1 text-center text-xs font-normal">Qty</th>}
                  ...
                  {result.achieved_marks.map((mark: any, index: number) => {
                    ...
                    return (
                      <tr key={mark.id}>
                        ...
                        {stationDetails.map((sd: any, di: number) => {
                          const detail = getStationDataForCadet(mark, sd.id);
                          const needsQty = hasStationQty(sd);
                          if (needsQty) {
                            return React.createElement(React.Fragment, { key: sd.id || di },
                          ...
                          return React.createElement('td', { key: sd.id || di, className: 'border border-black px-1 py-1 text-center' },
                            parseFloat(String(detail?.marks || 0)).toFixed(2)
                          );
                        })}
                        {hasStationData && (
      ...
      >
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Module: <span className="font-medium text-gray-700">PT Exam</span>
          </p>
