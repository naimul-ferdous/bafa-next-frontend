...
export default function CtwPtExamResultsPage() {
  ...
  const [deletingResult, setDeletingResult] = useState<CtwPtExamResult | null>(null);
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwPtExamResultService.getAllResults(ptExamModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, ptExamModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (ptExamModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwPtExamResultService.getGroupedResults({
  ...
  }, [ptExamModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && ptExamModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, ptExamModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/pf/ptexam/create");
  const handleEditResult = (result: CtwPtExamResult) => router.push(`/ctw/results/pf/ptexam/${result.id}/edit`);
  const handleViewResult = (result: CtwPtExamResult | any) => router.push(`/ctw/results/pf/ptexam/${result.id}`);
  const handleDeleteResult = (result: CtwPtExamResult) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwPtExamResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<CtwPtExamResult>[] = [
    ...
    {
      key: "actions" as keyof CtwPtExamResult,
      header: "Actions",
      headerAlign: "center" as const,
      className: "text-center no-print",
      render: (result: CtwPtExamResult) => (
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/pf/ptexam/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW PT Exam Results</h2>
      </div>
