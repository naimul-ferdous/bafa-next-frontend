...
export default function CtwTwoKmFiveStationResultsPage() {
  ...
  useEffect(() => {
    const fetchModuleId = async () => {
      try {
        ...
        const twoKmModule = modulesRes.data.find((m: any) => m.code === TWO_KM_5_STATION_MODULE_CODE);
        if (twoKmModule) {
          setTwoKmModuleId(twoKmModule.id);
        } else {
          console.error(`Module with code ${TWO_KM_5_STATION_MODULE_CODE} not found.`);
        }
      } catch (err) {
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwOneMileResultService.getAllResults(twoKmModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  const handleAddResult = () => router.push("/ctw/results/pf/2km5station/create");
  const handleEditResult = (result: CtwOneMileResult) => router.push(`/ctw/results/pf/2km5station/${result.id}/edit`);
  const handleViewResult = (result: CtwOneMileResult | any) => router.push(`/ctw/results/pf/2km5station/${result.id}`);
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/pf/2km5station/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW 2KM 5 Station Results</h2>
      </div>
