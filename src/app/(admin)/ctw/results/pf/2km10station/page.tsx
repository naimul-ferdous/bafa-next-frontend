...
export default function CtwTwoKmTenStationResultsPage() {
  ...
  const [twoKm10ModuleId, setTwoKm10ModuleId] = useState<number | null>(null);
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwOneMileResultService.getAllResults(twoKm10ModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, twoKm10ModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (twoKm10ModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwOneMileResultService.getGroupedResults({
        ctw_results_module_id: twoKm10ModuleId,
        search: searchTerm || undefined,
      });
      const flattened: any[] = [];
      data.forEach(course => {
  ...
  }, [twoKm10ModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && twoKm10ModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, twoKm10ModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/pf/2km10station/create");
  const handleEditResult = (result: CtwOneMileResult) => router.push(`/ctw/results/pf/2km10station/${result.id}/edit`);
  const handleViewResult = (result: CtwOneMileResult | any) => router.push(`/ctw/results/pf/2km10station/${result.id}`);
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/pf/2km10station/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW 2KM 10 Station Results</h2>
      </div>
