...
export default function EditTwoKmTenStationResultPage() {
    ...
    const fetchModuleAndResult = async () => {
      try {
        ...
        if (options?.module) {
          ...
          setError(`Module with code ${TWO_KM_10_STATION_MODULE_CODE} not found.`);
        }
      } catch (err) {
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ctw/results/pf/2km10station")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600">
            Back to Results
          </button>
        </div>
      </div>
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">Edit CTW 2KM 10 Station Result</h2>
      </div>
