...
export default function TwoKmResultDetailsPage() {
  ...
  const loadData = useCallback(async () => {
    if (twoKmModuleId === null || resultId === undefined) return;
    try {
      setLoading(true);
  ...
  useEffect(() => {
    const loadEstimatedMarks = async () => {
      if (!twoKmModuleId || !result?.course_id || !result?.semester_id) return;
      try {
  ...
  const stationDetails: any[] = (estimatedMarkInfo as any)?.details ?? [];
                    ...
                    {hasStationData && (
                      <>
                        <th className="border border-black px-2 py-2 text-center align-middle" rowSpan={2}>
                          Total {stationDetails.reduce((sum: number, d: any) => sum + parseFloat(String(d.male_marks || 0)), 0) || ""}
                        </th>
                        <th className="border border-black px-2 py-2 text-center align-middle" rowSpan={2}>
                          Conv {estimatedMarkInfo?.conversation_mark ? `(${estimatedMarkInfo.conversation_mark})` : ""}
                        </th>
                      </>
                    )}
                    {canApprove && (
                          ...
                          <>
                            ...
                            <td className="border border-black px-2 py-2 text-center font-bold">
                              {(() => {
                                ...
                                const convLimit = parseFloat(String(estimatedMarkInfo?.conversation_mark || 0));
      ...
      >
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Module: <span className="font-medium text-gray-700">2KM</span>
          </p>
