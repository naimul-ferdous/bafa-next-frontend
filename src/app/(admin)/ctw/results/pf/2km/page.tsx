...
import { Ctw2kmResult } from "@/libs/types/ctw2km";
...
export default function Ctw2kmResultsPage() {
  const router = useRouter();
  const { user, userIsInstructor } = useAuth();
  const isInstructor = userIsInstructor;

  const [results, setResults] = useState<Ctw2kmResult[]>([]);
  ...
  const [deletingResult, setDeletingResult] = useState<Ctw2kmResult | null>(null);
  ...
  const loadResults = useCallback(async () => {
    if (twoKmModuleId === null || !user?.id) { setLoading(false); return; }

    try {
      setLoading(true);
      const response = await ctw2kmResultService.getAllResults(twoKmModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, twoKmModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (twoKmModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctw2kmResultService.getGroupedResults({
  ...
  }, [twoKmModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && twoKmModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, twoKmModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/pf/2km/create");
  const handleEditResult = (result: Ctw2kmResult) => router.push(`/ctw/results/pf/2km/${result.id}/edit`);
  const handleViewResult = (result: Ctw2kmResult | any) => router.push(`/ctw/results/pf/2km/${result.id}`);
  const handleDeleteResult = (result: Ctw2kmResult) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctw2kmResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<Ctw2kmResult>[] = [
    ...
    {
      key: "actions" as keyof Ctw2kmResult,
      header: "Actions",
      headerAlign: "center" as const,
      className: "text-center no-print",
      render: (result: Ctw2kmResult) => (
  ...
  const officialColumns: Column<any>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <div>
          <div className="font-medium text-gray-900">{row.module?.name || "N/A"}</div>
          <div className="text-xs text-gray-500">{row.module?.code || ""}</div>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/pf/2km/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW 2KM Results</h2>
      </div>
