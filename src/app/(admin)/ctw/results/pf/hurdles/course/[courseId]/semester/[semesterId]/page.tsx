...
export default function HurdlesCourseSemesterResultPage() {
  ...
  const hasStationQty = (sd: any): boolean => {
    ...
    if (scores.length === 0) {
      ...
      const hasMaleTime = details.some((d: any) => d?.male_time !== null);
      const hasFemaleTime = details.some((d: any) => d?.female_time !== null);
      if (hasMaleTime || hasFemaleTime) return true;
      return false;
    }
    const hasMaleQty = scores.some((s: any) => s?.male_quantity !== null);
    const hasFemaleQty = scores.some((s: any) => s?.female_quantity !== null);
    if (hasMaleQty || hasFemaleQty) return true;
    const hasMaleTime = scores.some((s: any) => s?.male_time !== null);
    const hasFemaleTime = scores.some((s: any) => s?.female_time !== null);
    if (hasMaleTime || hasFemaleTime) return true;
    return false;
  };
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const data = await ctwHurdlesResultService.getInitialFetchData({
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button onClick={() => router.push("/ctw/results/pf/hurdles")} className="mt-4 px-6 py-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600 transition-all font-bold">
            Back to Results
          </button>
        </div>
      </div>
  ...
  return (
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        <button onClick={() => router.push("/ctw/results/pf/hurdles")} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 font-medium transition-all">
        ...
        <div className="mb-8">
          ...
          <div className="mb-2">
            ...
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">
              {courseDetails?.name} ({moduleDetails?.full_name || "Hurdles"})
            </p>
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">
            ...
            <table className="w-full border-collapse border border-black text-sm">
              <thead>
                <tr>
                  ...
                  <th rowSpan={2} className="border border-black px-2 py-2 text-center align-middle font-bold">
                    <div className="h-32 flex flex-col items-center justify-center w-12 mx-auto">
                      <span className="font-semibold" style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>Conv {convLimit ? `(${convLimit})` : ""}</span>
                    </div>
                  </th>
