...
export default function HurdlesCourseSemesterResultPage() {
  ...
  // Hurdles mark calculation per submission per cadet (exam mark only)
  const aggregatedMarks = useMemo(() => {
    const cadetMap = new Map<number, any>();
    const convExamWeight = parseFloat(String(estimatedMark?.convert_of_exam || 100));
    ...
    submissions.forEach(sub => {
      ...
      marks.forEach((markItem: any) => {
        ...
        if (cadetMap.has(cadetId)) {
          ...
          let finalMark = (test_mark * convExamWeight) / 100;
      ...
      <div className="p-8 cv-content">
        <div className="mb-8">
          ...
          <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">
            CTW {moduleDetails?.full_name || "Hurdle Test"} Course Result Sheet
          </p>
        </div>
