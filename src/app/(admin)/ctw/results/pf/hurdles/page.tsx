...
export default function CtwHurdlesResultsPage() {
  ...
  const [deletingResult, setDeletingResult] = useState<CtwHurdlesResult | null>(null);
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwHurdlesResultService.getAllResults(hurdlesModuleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, hurdlesModuleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (hurdlesModuleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwHurdlesResultService.getGroupedResults({
  ...
  }, [hurdlesModuleId, searchTerm]);

  useEffect(() => {
    if (!moduleLoading && hurdlesModuleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, hurdlesModuleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/pf/hurdles/create");
  const handleEditResult = (result: CtwHurdlesResult) => router.push(`/ctw/results/pf/hurdles/${result.id}/edit`);
  const handleViewResult = (result: CtwHurdlesResult | any) => router.push(`/ctw/results/pf/hurdles/${result.id}`);
  const handleDeleteResult = (result: CtwHurdlesResult) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwHurdlesResultService.deleteResult(deletingResult.id);
      await loadResults();
  ...
  const columns: Column<CtwHurdlesResult>[] = [
    ...
    {
      key: "actions" as keyof CtwHurdlesResult,
      header: "Actions",
      headerAlign: "center" as const,
      className: "text-center no-print",
      render: (result: CtwHurdlesResult) => (
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
          <button onClick={() => router.push(`/ctw/results/pf/hurdles/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW Hurdles Results</h2>
      </div>
