...
export default function HurdlesResultDetailsPage() {
  ...
  const loadData = useCallback(async () => {
    if (hurdlesModuleId === null || resultId === undefined) return;
    try {
      setLoading(true);
      const data = await ctwHurdlesResultService.getResult(hurdlesModuleId, parseInt(resultId));
      if (data) {
  ...
  const handleConfirmResubmit = async () => {
    ...
    if (!item || !hurdlesModuleId) return;
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ctw/results/pf/hurdles")}
                ...
                <thead>
                  <tr>
                    ...
                    {hasStationData ? stationDetails.map((sd: any, i: number) => {
                      const maleMark = parseFloat(sd.male_marks) || 0;
                      const femaleMark = parseFloat(sd.female_marks) || 0;
                      const firstScore = sd.scores?.[0];
                      const hasTime = !!firstScore?.male_time || !!firstScore?.female_time;
                      if (!hasTime) {
                        return (
                          <th key={sd.id || i} className={`border border-black px-1 py-2 text-center`} colSpan={hasTime ? 2 : 1} rowSpan={hasTime ? 1 : 2}>
                            <div className="h-32 flex flex-col items-center justify-center w-12 mx-auto">
                              <span className="font-semibold [writing-mode:vertical-rl] rotate-180">{sd.name} - {maleMark}/{femaleMark}</span>
                            </div>
                          </th>
                        )
                      }
                    ...
                    {stationDetails.some((sd: any) => {
                      const firstScore = sd.scores?.[0];
                      return !firstScore?.male_time && !firstScore?.female_time;
                    }) && (
                        <th className="border border-black px-2 py-2 text-center align-middle" rowSpan={2}>
                          <div className="h-32 flex flex-col items-center justify-center w-12 mx-auto">
                            <span className="font-semibold [writing-mode:vertical-rl] rotate-180">HD Mks</span>
                          </div>
                        </th>
                      )}
                    {stationDetails.some((sd: any) => {
                      ...
                      return !!(!!firstScore?.male_time || !!firstScore?.female_time);
                        ...
                        <>
                          <th className="border border-black px-2 py-2 text-center align-middle" rowSpan={2}>
                            <div className="h-32 flex flex-col items-center justify-center w-12 mx-auto">
                              <span className="font-semibold [writing-mode:vertical-rl] rotate-180">Time</span>
                            </div>
                          </th>
                          <th className="border border-black px-2 py-2 text-center align-middle" rowSpan={2}>
                            <div className="h-32 flex flex-col items-center justify-center w-12 mx-auto">
                              <span className="font-semibold [writing-mode:vertical-rl] rotate-180">Time Mks</span>
                            </div>
                          </th>
                        </>
                      )}
                    <th className="border border-black px-2 py-2 text-center align-middle" rowSpan={2}>
                      <div className="h-32 flex flex-col items-center justify-center w-12 mx-auto">
                        <span className="font-semibold [writing-mode:vertical-rl] rotate-180">Ttl Mks</span>
                      </div>
                    </th>
                    <th className="border border-black px-2 py-2 text-center align-middle" rowSpan={2}>
                      <div className="h-32 flex flex-col items-center justify-center w-12 mx-auto">
                        <span className="font-semibold [writing-mode:vertical-rl] rotate-180">Conv - {Number(estimatedMark?.conversation_mark || 15).toFixed(0)}</span>
                      </div>
                    </th>
                    <th className="border border-black px-2 py-2 text-center align-middle" rowSpan={2}>
                      <div className="h-32 flex flex-col items-center justify-center w-12 mx-auto">
                        <span className="font-semibold [writing-mode:vertical-rl] rotate-180">Rmks</span>
                      </div>
                    </th>
                    {canApprove && (
                  ...
                  {/* {hasStationData && stationDetails.some((sd: any) => {
                    const firstScore = sd.scores?.[0];
                    return !!firstScore?.male_time || !!firstScore?.female_time;
                  }) && (
                      <tr>
                        {stationDetails.filter((sd: any) => {
                          const firstScore = sd.scores?.[0];
                          return !!firstScore?.male_time || !!firstScore?.female_time;
                        }).map((sd: any, i: number) => (
                          <React.Fragment key={sd.id || i}>
                            <th className="border border-black px-1 py-1 text-center text-xs font-normal">Time</th>
                        ...
                        {hasStationData ? (
                          <>
                            {stationDetails.filter((sd: any) => {
                              const firstScore = sd.scores?.[0];
                              return !firstScore?.male_time && !firstScore?.female_time;
                            }).map((sd: any, di: number) => {
                              const detail = getStationDataForCadet(mark, sd.id);
                              return (
                                <td key={sd.id || di} className="border border-black px-1 py-1 text-center">
                            ...
                            <td className="border border-black px-2 py-2 text-center font-bold">
                              {(() => {
                                const directDetails = stationDetails.filter((sd: any) => {
                                  const firstScore = sd.scores?.[0];
                                  return !firstScore?.male_time && !firstScore?.female_time;
                                });
                                const directIds = directDetails.map((sd: any) => sd.id);
                                return mark.details
                                  ?.filter((d: any) => directIds.includes(d.ctw_results_module_estimated_marks_details_id))
                                  ?.reduce((sum: number, d: any) => sum + parseFloat(String(d.marks || 0)), 0)
                                  ?.toFixed(2) || "0.00";
                              })()}
                            </td>
                            {stationDetails.filter((sd: any) => {
      ...
      >
        <div className="p-6">
          <div className="text-center mb-4">
            ...
            <p className="text-sm font-medium text-gray-700 uppercase">CTW Hurdles</p>
          </div>
          <h2 className="text-lg font-bold text-gray-900 mb-1">
            {approvalModal.cadetIds.length === 1 ? "Cadet Approval" : `Cadets Approval (${approvalModal.cadetIds.length} cadets)`}
          </h2>
          <p className="text-sm text-gray-500 mb-4">
            Module: <span className="font-medium text-gray-700">Hurdles</span>
          </p>
