...
export default function BmaResultDetailsPage() {
    ...
    useEffect(() => {
        const loadResult = async () => {
            ...
            try {
                setLoading(true);
                const data = await ctwBmaResultService.getResult(bmaModuleId, parseInt(resultId));
                if (data) {
    ...
    }, [resultId, bmaModuleId, moduleLoading]);
    ...
    const rankedMarks = useMemo(() => {
        ...
        const ranked = sorted.map((mark, idx) => {
            ...
            if (idx > 0 && parseFloat(String(mark.achieved_mark || 0)) === parseFloat(String(sorted[idx - 1].achieved_mark || 0))) {
                ...
                const prevResult = rankedMarks?.[idx - 1];
                position = prevResult?.position ?? idx;
            }
            const remark = parseFloat(String(mark.achieved_mark || 0)) <= 0 ? "Failed" : "-";
    ...
    return (
        <div className="print-no-border bg-white rounded-lg border border-gray-200">
            ...
            <div className="p-4 flex items-center justify-between no-print">
                <button onClick={() => router.push("/ctw/results/bma")} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                    <Icon icon="hugeicons:arrow-left-01" className="w-4 h-4" />
                    Back to List
                </button>
                <div className="flex items-center gap-3">
                    {canApproveAction && selectedCadetIds.length > 0 && (
                        <button onClick={() => openApprovalModal(selectedCadetIds)}
                    ...
                    <button onClick={() => router.push(`/ctw/results/bma/${result.id}/edit`)} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
            ...
            <div className="p-8 cv-content">
                <div className="mb-8">
                    ...
                    <p className="text-center font-medium text-gray-900 uppercase tracking-wider pb-2">CTW BMA Result Sheet</p>
                </div>
                                    ...
                                    {rankedMarks.map((mark, index) => {
                                        ...
                                        return (
                                            <tr key={mark.id}>
                                                ...
                                                <td className="border border-black px-2 py-2 text-center">{(mark.cadet as any)?.cadet_number || "N/A"}</td>
                                                <td className="border border-black px-2 py-2 text-center">
                                                    {(mark.cadet as any)?.assigned_ranks?.find((ar: any) => ar.rank)?.rank?.short_name ||
                                                        (mark.cadet as any)?.assigned_ranks?.find((ar: any) => ar.rank)?.rank?.name ||
                                                        "Officer Cadet"}
                                                </td>
                                                <td className="border border-black px-2 py-2 font-medium">{(mark.cadet as any)?.name || "N/A"}</td>
                                                <td className="border border-black px-2 py-2 text-center">{(mark.cadet as any)?.assigned_branchs?.find((b: any) => b.is_current)?.branch?.name || result.branch?.name || "N/A"}</td>
                                                <td className="border border-black px-2 py-2 text-center">
                                                    {result.instructor?.name || "N/A"}
                                                    <div className="text-xs text-gray-500">{(result.instructor as any)?.service_number || ""}</div>
                                                </td>
                                                ...
                                                <td className="border border-black px-2 py-2 text-center">
                                                    {getOrdinal(mark.position)}
                                                </td>
                                                <td className={`border border-black px-2 py-2 text-center ${mark.remark === "Failed" ? "text-red-600" : "text-gray-400"}`}>
                                                    {mark.remark}
                                                </td>
                ...
                <div className="mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-x-12 gap-y-3">
                        <div className="flex">
                            ...
                            <span className="mr-4">:</span>
                            <span className={`flex-1 ${result.is_active ? "text-green-600" : "text-red-600"}`}>{result.is_active ? "Active" : "Inactive"}</span>
                        </div>
                        <div className="flex">
                        ...
                        <div className="flex">
                            ...
                            <span className="text-gray-900 flex-1">
                                {result.created_at ? new Date(result.created_at).toLocaleString("en-GB", { day: "2-digit", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "N/A"}
                            </span>
                        </div>
                        <div className="flex">
                            ...
                            <span className="mr-4">:</span>
                            <span className="text-gray-900 flex-1">
                                {result.updated_at ? new Date(result.updated_at).toLocaleString("en-GB", { day: "2-digit", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "N/A"}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            >
                <div className="p-6">
                    <div className="text-center mb-4">
                        ...
                        <p className="text-sm font-medium text-gray-700 uppercase">CTW BMA</p>
                    </div>
                    <h2 className="text-lg font-bold text-gray-900 mb-1">
                        {approvalModal.cadetIds.length === 1 ? "Cadet Approval" : `Cadets Approval (${approvalModal.cadetIds.length} cadets)`}
                    </h2>
                    <p className="text-sm text-gray-500 mb-4">
                        Module: <span className="font-medium text-gray-700">BMA</span>
                    </p>
                                    ...
                                    {approvalModal.cadetIds.map((cadetId, idx) => {
                                        ...
                                        return (
                                            <tr key={cadetId} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50/30"}>
                                                ...
                                                <td className="border border-gray-200 px-2 py-1 text-center font-mono">{(mark.cadet as any)?.cadet_number || "N/A"}</td>
                                                <td className="border border-gray-200 px-2 py-1 font-bold text-gray-900">{(mark.cadet as any)?.name || "N/A"}</td>
