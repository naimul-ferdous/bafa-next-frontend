...
export default function CtwBmaResultsPage() {
    ...
    const [deletingResult, setDeletingResult] = useState<CtwBmaResult | null>(null);
    ...
    const loadResults = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const response = await ctwBmaResultService.getAllResults(bmaModuleId, {
                page: currentPage,
                per_page: perPage,
    ...
    }, [currentPage, perPage, searchTerm, bmaModuleId, isInstructor, user?.id]);

    const loadOfficialResults = useCallback(async () => {
        if (bmaModuleId === null) return;
        try {
            setOfficialLoading(true);
            const data = await ctwBmaResultService.getGroupedResults({
                ctw_results_module_id: bmaModuleId,
                search: searchTerm || undefined
            });
            const flattened: any[] = [];
            data.forEach(course => {
    ...
    }, [bmaModuleId, searchTerm]);

    useEffect(() => {
        if (!moduleLoading && bmaModuleId !== null && user?.id) {
            if (isInstructor) {
                loadResults();
            } else {
                loadOfficialResults();
    ...
    }, [loadResults, loadOfficialResults, moduleLoading, bmaModuleId, user?.id, isInstructor]);

    const handleAddResult = () => router.push("/ctw/results/bma/create");
    const handleEditResult = (result: CtwBmaResult) => router.push(`/ctw/results/bma/${result.id}/edit`);
    const handleViewResult = (result: CtwBmaResult | any) => router.push(`/ctw/results/bma/${result.id}`);
    const handleDeleteResult = (result: CtwBmaResult) => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            await ctwBmaResultService.deleteResult(deletingResult.id);
            await loadResults();
    ...
    const columns: Column<CtwBmaResult>[] = [
        ...
        {
            key: "actions" as keyof CtwBmaResult,
            header: "Actions",
            headerAlign: "center" as const,
            className: "text-center no-print",
            render: (result: CtwBmaResult) => (
        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex flex-col items-center gap-1">
                    <button onClick={() => router.push(`/ctw/results/bma/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
    ...
    return (
        <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
            <div className="text-center mb-8">
                ...
                <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW BMA Results</h2>
            </div>
