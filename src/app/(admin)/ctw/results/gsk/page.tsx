...
export default function CtwGskResultsPage() {
    ...
    const [deletingResult, setDeletingResult] = useState<CtwGskResult | null>(null);
    ...
    const loadResults = useCallback(async () => {
        ...
        try {
            setLoading(true);
            const response = await ctwGskResultService.getAllResults(gskModuleId, {
                page: currentPage,
                per_page: perPage,
    ...
    }, [currentPage, perPage, searchTerm, gskModuleId, isInstructor, user?.id]);

    const loadOfficialResults = useCallback(async () => {
        if (gskModuleId === null) return;
        try {
            setOfficialLoading(true);
            const data = await ctwGskResultService.getGroupedResults({
    ...
    useEffect(() => {
        if (!moduleLoading && gskModuleId !== null && user?.id) {
            if (isInstructor) {
                loadResults();
            } else {
                loadOfficialResults();
    ...
    }, [loadResults, loadOfficialResults, moduleLoading, gskModuleId, user?.id, isInstructor]);

    const handleAddResult = () => router.push("/ctw/results/gsk/create");
    const handleEditResult = (result: CtwGskResult) => router.push(`/ctw/results/gsk/${result.id}/edit`);
    const handleViewResult = (result: CtwGskResult | any) => router.push(`/ctw/results/gsk/${result.id}`);
    const handleDeleteResult = (result: CtwGskResult) => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            await ctwGskResultService.deleteResult(deletingResult.id);
            await loadResults();
    ...
    const columns: Column<CtwGskResult>[] = [
        ...
        {
            key: "actions" as keyof CtwGskResult,
            header: "Actions",
            headerAlign: "center" as const,
            className: "text-center no-print",
            render: (result: CtwGskResult) => (
        ...
        },
        {
            ...
            render: (row) => (
                <div className="flex flex-col items-center gap-1">
                    <button onClick={() => router.push(`/ctw/results/gsk/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
    ...
    return (
        <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
            <div className="text-center mb-8">
                ...
                <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW GSK Results</h2>
            </div>
