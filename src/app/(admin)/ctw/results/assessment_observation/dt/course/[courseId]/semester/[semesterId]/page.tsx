...
export default function DtAssessmentCourseSemesterResultPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const data = await ctwDtAssessmentResultService.getInitialFetchData({
        module_code: DT_ASSESSMENT_MODULE_CODE,
        course_id: courseId,
  ...
  return (
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        <button onClick={() => router.push("/ctw/results/assessment_observation/dt")} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 font-medium transition-all">
        ...
        <div className="mb-8 text-center">
          ...
          <div className="mb-2">
            ...
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">
              {courseDetails?.name} ({moduleDetails?.full_name || "Arms Drill"})
            </p>
            <p className="text-center font-medium text-gray-900 uppercase underline tracking-wider">
                      ...
                      return (
                        <tr key={item.cadet.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="border border-black px-2 py-2 font-bold uppercase">
                            <button onClick={() => router.push(`/ctw/results/assessment_observation/dt/course/${courseId}/semester/${semesterId}/cadet/${item.cadet.id}`)} className="hover:underline text-blue-600 hover:text-blue-800 text-left">
                              {item.cadet?.name || "N/A"}
                            </button>
                          </td>
                              ...
                              <td key={sub.id} className={`border border-black px-2 py-2 text-center ${!isCalc && hasEntry ? "bg-gray-50" : ""}`}>
                                {hasEntry ? (
                                  <span className={!isCalc ? "line-through text-gray-400" : ""} title={!isCalc ? "Not calculable (excused)" : ""}>
                                    {parseFloat(String(mark)).toFixed(3)}
                                  </span>
                                ) : "-"}
                              </td>
                            );
        ...
        <div className="p-6">
          ...
          {approvalModal.status === "rejected" && (
            <div className="mb-4">
              ...
              <textarea rows={3} value={approvalModal.rejectedReason} onChange={e => setApprovalModal(prev => ({ ...prev, rejectedReason: e.target.value }))}
                placeholder="Enter rejection reason..." className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-400 resize-none" />
            </div>
          )}
          {approvalModal.error && (
