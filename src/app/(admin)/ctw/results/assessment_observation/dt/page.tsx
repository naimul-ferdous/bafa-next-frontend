...
export default function CtwDtAssessmentResultsPage() {
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwDtAssessmentResultService.getAllResults(moduleId, {
        page: currentPage,
        per_page: perPage,
  ...
  const loadOfficialResults = useCallback(async () => {
    ...
    try {
      ...
      const data = await ctwDtAssessmentResultService.getGroupedResults({
  ...
  const handleAddResult = () => router.push("/ctw/results/assessment_observation/dt/create");
  const handleEditResult = (result: any) => router.push(`/ctw/results/assessment_observation/dt/${result.id}/edit`);
  const handleViewResult = (result: any) => router.push(`/ctw/results/assessment_observation/dt/${result.id}`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwDtAssessmentResultService.deleteResult(deletingResult.id);
      await loadResults();
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
            <button onClick={() => router.push(`/ctw/results/assessment_observation/dt/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW DT Assessment Observations</h2>
      </div>
