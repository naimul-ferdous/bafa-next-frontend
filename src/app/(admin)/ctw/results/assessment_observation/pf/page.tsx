...
export default function CtwPfAssessmentResultsPage() {
  ...
  useEffect(() => {
    const fetchModuleId = async () => {
      try {
        ...
        const foundModule = modulesRes.data.find((m: any) => m.code === PF_ASSESSMENT_MODULE_CODE);
        if (foundModule) {
          setModuleId(foundModule.id);
        } else {
          console.error(`Module with code ${PF_ASSESSMENT_MODULE_CODE} not found.`);
        }
      } catch (err) {
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwPfAssessmentResultService.getAllResults(moduleId, {
        page: currentPage,
        per_page: perPage,
  ...
  }, [currentPage, perPage, searchTerm, moduleId, isInstructor, user?.id]);

  const loadOfficialResults = useCallback(async () => {
    if (moduleId === null) return;
    try {
      setOfficialLoading(true);
      const data = await ctwPfAssessmentResultService.getGroupedResults({
  ...
  useEffect(() => {
    if (!moduleLoading && moduleId !== null && user?.id) {
      if (isInstructor) {
        loadResults();
      } else {
        loadOfficialResults();
  ...
  }, [loadResults, loadOfficialResults, moduleLoading, moduleId, user?.id, isInstructor]);

  const handleAddResult = () => router.push("/ctw/results/assessment_observation/pf/create");
  const handleEditResult = (result: any) => router.push(`/ctw/results/assessment_observation/pf/${result.id}/edit`);
  const handleViewResult = (result: any) => router.push(`/ctw/results/assessment_observation/pf/${result.id}`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwPfAssessmentResultService.deleteResult(deletingResult.id);
      await loadResults();
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
            <button onClick={() => router.push(`/ctw/results/assessment_observation/pf/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW PF Assessment Observations</h2>
      </div>
