    ...
    const fetchModuleId = async () => {
      try {
        ...
        if (foundModule) {
          ...
          setError(`Module with code ${PF_ASSESSMENT_MODULE_CODE} not found.`);
        }
      } catch (err) {
              ...
              <table className="w-full border-collapse border border-black">
                <thead>
                  <tr>
                    ...
                    <th className="border border-black px-2 py-2 text-left align-middle">Name</th>
                    <th className="border border-black px-2 py-2 text-left align-middle">Branch</th>
                    <th className="border border-black px-2 py-2 text-center align-middle font-bold">Mark</th>
                  </tr>
                </thead>
                <tbody>
                  {result.achieved_marks.map((mark: any, index: number) => (
                    <tr key={mark.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="border border-black px-2 py-2 font-bold">{mark.cadet?.name || "N/A"}</td>
                      <td className="border border-black px-2 py-2">{mark.cadet?.assigned_branchs?.[0]?.branch?.name || "N/A"}</td>
                      <td className="border border-black px-2 py-2 text-center font-black">
