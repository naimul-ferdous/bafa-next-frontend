...
export default function PfAssessmentResultDetailsPage() {
  ...
  const loadData = useCallback(async () => {
    if (moduleId === null || resultId === undefined) return;
    try {
      setLoading(true);
      const data = await ctwPfAssessmentResultService.getResult(moduleId, parseInt(resultId));
      if (data) {
  ...
  // ── Criteria details ───────────────────────────────────────────────────────
  const criteriaDetails: any[] = result?.estimated_mark?.details || [];
  const hasDetails = criteriaDetails.length > 0;
  const maxDetailTotal = criteriaDetails.reduce((sum: number, d: any) => sum + parseFloat(d.male_marks || d.female_marks || 0), 0);

  const getCadetDetailMark = (mark: any, detailId: number): number => {
    const d = mark.details?.find((x: any) => x.ctw_results_module_estimated_marks_details_id === detailId);
    ...
    return (
      <div className="bg-white p-6 rounded-lg border border-gray-200">
        <div className="text-center py-12">
          ...
          <button
            onClick={() => router.push("/ctw/results/assessment_observation/pf")}
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-4 flex items-center justify-between no-print">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push(`/ctw/results/assessment_observation/pf/${result.id}/edit`)}
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="font-medium text-gray-900 uppercase tracking-wider pb-2">CTW PF Assessment Observation Result Sheet</p>
        </div>
              ...
              <table className="w-full border-collapse border border-black text-xs">
                <thead>
                  <tr className="bg-gray-50">
                    ...
                    <th className="border border-black px-2 py-2 text-center align-middle w-12" rowSpan={hasDetails ? 2 : 1}>Ser</th>
                    ...
                    {canApproveAction && !isForwarded && (
                      <th className="border border-black px-2 py-2 text-center align-middle no-print" rowSpan={hasDetails ? 2 : 1}>Action</th>
                    )}
                  </tr>
                  {hasDetails && (
                    <tr className="bg-gray-50">
                ...
                <tbody>
                  {result.achieved_marks.map((mark: any, index: number) => {
                    ...
                    const detailTotal = hasDetails
                      ? criteriaDetails.reduce((sum: number, d: any) => sum + getCadetDetailMark(mark, d.id), 0)
                      : 0;
                    ...
                    return (
                      <tr key={mark.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="border border-black px-2 py-2 text-center">
                          {mark.cadet?.assigned_ranks?.find((ar: any) => ar.rank)?.rank?.short_name || mark.cadet?.assigned_ranks?.[0]?.rank?.name || "Officer Cadet"}
                        </td>
                        <td className="border border-black px-2 py-2">
                          <button onClick={() => router.push(`/ctw/results/assessment_observation/pf/course/${result.course_id}/semester/${result.semester_id}/cadet/${mark.cadet_id}`)} className="text-blue-600 font-bold uppercase hover:underline hover:text-blue-800 text-left">
                            {mark.cadet?.name || "N/A"}
                          </button>
                        </td>
                        {hasDetails ? (
                                ...
                                {visibleAuthorities.map((authority: any, idx: number) => {
                                  ...
                                  return (
                                    <div key={authority.id} className="flex items-center gap-1">
                                      ...
                                      <span className={`inline-flex items-center px-1.5 py-0.5 text-[9px] font-bold rounded-full uppercase ${colors[st] ?? ""}`}>{st}</span>
                                    </div>
                                  );
                                })}
                              </div>
      ...
      <Modal isOpen={approvalModal.open} onClose={() => setApprovalModal(prev => ({ ...prev, open: false }))} showCloseButton className="max-w-lg">
        <div className="p-6">
          <div className="text-center mb-4">
            ...
            <p className="text-sm font-medium text-gray-700 uppercase">CTW PF Assessment</p>
          </div>
          <h2 className="text-lg font-bold text-gray-900 mb-1">
            {approvalModal.cadetIds.length === 1 ? "Cadet Approval" : `Cadets Approval (${approvalModal.cadetIds.length} cadets)`}
          </h2>
          <p className="text-sm text-gray-500 mb-4">Module: <span className="font-medium text-gray-700">PF Assessment</span></p>
                    ...
                    return (
                      <tr key={cadetId} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50/30"}>
                        ...
                        <td className="border border-gray-200 px-2 py-1 text-center font-bold">
                          {hasDetails
                            ? mark.details?.reduce((sum: number, d: any) => sum + parseFloat(String(d.marks || 0)), 0).toFixed(2)
                            : parseFloat(String(mark.achieved_mark || 0)).toFixed(2)}
                        </td>
                      </tr>
                    );
      ...
      <Modal isOpen={moduleApprovalModal.open} onClose={() => setModuleApprovalModal(prev => ({ ...prev, open: false }))} showCloseButton className="max-w-lg">
        <div className="p-6">
          ...
          <p className="text-sm text-gray-600 mb-5">This will approve the module result at your authority level and forward it to the next authority for further review.</p>
        ...
        <div className="p-6">
          ...
          {rejectDownModal.error && (
            <div className="mb-3 p-2 bg-red-50 border border-red-200 rounded text-red-700 text-sm flex items-center gap-2">
              <Icon icon="hugeicons:alert-circle" className="w-4 h-4 flex-shrink-0" />{rejectDownModal.error}
            </div>
          )}
          <div className="flex gap-3 justify-end">
            ...
            <button onClick={handleRejectDown} disabled={rejectDownModal.loading} className="px-4 py-2 bg-orange-600 text-white rounded-lg text-sm font-semibold hover:bg-orange-700 disabled:opacity-50 flex items-center gap-2">
      ...
      {/* ── Resubmit Modal ── */}
      <Modal isOpen={resubmitModal.open} onClose={() => !resubmitModal.loading && setResubmitModal({ open: false, item: null, loading: false, error: "" })} showCloseButton className="max-w-md">
        <div className="p-6">
          ...
          {resubmitModal.error && (
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm flex items-center gap-2">
              <Icon icon="hugeicons:alert-circle" className="w-4 h-4 flex-shrink-0" />{resubmitModal.error}
            </div>
          )}
          <div className="flex gap-3 justify-end">
            <button onClick={() => setResubmitModal({ open: false, item: null, loading: false, error: "" })} disabled={resubmitModal.loading} className="px-5 py-2 border border-gray-300 rounded-xl text-gray-700 text-sm font-bold hover:bg-gray-50">Cancel</button>
            <button onClick={handleConfirmResubmit} disabled={resubmitModal.loading} className="px-8 py-2 bg-green-600 text-white rounded-xl text-sm font-bold hover:bg-green-700 shadow-lg shadow-green-200 flex items-center gap-2">
      ...
      {/* ── View Rejected Detail Modal ── */}
      <Modal isOpen={viewRejectedModal.open} onClose={() => setViewRejectedModal({ open: false, item: null })} showCloseButton className="max-w-lg">
        <div className="p-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 bg-orange-50 p-4 rounded-xl border border-orange-100">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-800">{viewRejectedModal.item.cadet_rank}</p>
                </div>
                <div>
          ...
          <div className="flex justify-end mt-6">
            <button onClick={() => setViewRejectedModal({ open: false, item: null })} className="px-6 py-2 bg-gray-900 text-white rounded-xl text-sm font-bold hover:bg-gray-800 shadow-lg shadow-gray-200">Close Details</button>
          </div>
        </div>
      </Modal>
    </div>
