...
export default function GstoAssessmentResultDetailsPage() {
  ...
  const [viewRejectedModal, setViewRejectedModal] = useState<{
    open: boolean; item: any | null;
  }>({ open: false, item: null });

  const [resubmitModal, setResubmitModal] = useState<{
    open: boolean; item: any | null; loading: boolean; error: string;
  ...
  // ── Load Result + Estimated Marks + DT/PF ──────────────────────────────────
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.log("[GSTO] DT+PF assessment data:", dtPfRes);
      console.log("[GSTO] Per-cadet DT+PF map:", Object.fromEntries(marksMap));
  ...
  const isMyTurn = canInitialForward
    ? !isForwarded
    : (((result as any)?.module_approvals?.find((sa: any) => sa.authority_id === (myAuthority as any)?.id))?.status === "pending");
  ...
  // ── Derived marks config ────────────────────────────────────────────────────
  const selectedEM = estimatedMarks.find((em: any) => em.exam_type_id === result?.exam_type_id);
  const allDetails: any[] = selectedEM?.details || [];
  const detailsTotal = allDetails.reduce((sum: number, d: any) => sum + parseFloat(d.male_marks || 0), 0);
    ...
    <div className="print-no-border bg-white rounded-lg border border-gray-200">
      ...
      <div className="p-8 cv-content">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-sm text-gray-700 uppercase">{result.course?.name || ""} {result.semester?.name ? `— ${result.semester.name}` : ""} {result.exam_type?.name ? `— ${result.exam_type.name}` : ""}</p>
        </div>

        {/* Rejected Panel */}
        {rejectedPanelItems.length > 0 && (
          <div className="mb-6 overflow-hidden no-print">
            <div className="flex items-center gap-2 py-2 font-semibold text-orange-700">
              <Icon icon="hugeicons:alert-02" className="w-5 h-5 text-orange-500" />
              Rejected Cadets
              <span className="ml-1 px-2 py-0.5 bg-orange-500 text-white text-xs font-bold rounded-full">{rejectedPanelItems.length}</span>
            </div>
                      ...
                      <tr key={idx} className="hover:bg-orange-50/40">
                        ...
                        <td className="px-2 py-2 border border-orange-100 text-gray-600 max-w-[200px]">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <button onClick={(e) => { e.stopPropagation(); setViewRejectedModal({ open: true, item }); }} className="p-1 text-orange-600 hover:bg-orange-100 rounded" title="View Details">
                              <Icon icon="hugeicons:view" className="w-4 h-4" />
                            </button>
                          </div>
                        </td>
                        <td className="px-2 py-2 border border-orange-100">
                          <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${item.state === "updated_pending_review" || item.state === "instructor_updated" ? "bg-blue-100 text-blue-800" : "bg-red-100 text-red-800"}`}>
                            {item.message}
                          </span>
                        </td>
                        <td className="px-2 py-2 border border-orange-100 text-center">
                          <div className="flex items-center justify-center gap-1">
                            {item.can_resubmit && (
                              <button onClick={() => handleResubmit(item)} className="px-2 py-1 text-[9px] bg-green-600 text-white rounded hover:bg-green-700">Re-submit</button>
                            )}
                            {item.can_approve && (
                              <button onClick={() => handleApproveUpdated(item)} disabled={resubmitLoading === item.cadet_id} className="px-2 py-1 text-[9px] bg-green-600 text-white rounded hover:bg-green-700 flex items-center gap-1 disabled:opacity-50">
                                {resubmitLoading === item.cadet_id && <Icon icon="hugeicons:fan-01" className="w-3 h-3 animate-spin" />}
                                Approve
                              </button>
                            )}
                            {item.can_reject_down && (
                              <button onClick={() => setRejectDownModal({ open: true, item, reason: "", loading: false, error: "" })} className="px-2 py-1 text-[9px] bg-orange-600 text-white rounded hover:bg-orange-700 flex items-center gap-1">
                                <Icon icon="hugeicons:arrow-down-01" className="w-3 h-3" /> Reject
                              </button>
                            )}
                          </div>
                        </td>
              ...
              <table className="w-full border-collapse border border-black text-xs">
                ...
                <tbody>
                  {result.achieved_marks.map((mark: any, index: number) => {
                    ...
                    return (
                      <tr key={mark.id} className="transition-colors">
                        {canApproveAction && pendingCadetIds.length > 0 && (
                          <td className="border border-black px-2 py-2 text-center no-print">
                            {isPending ? (
                              <input type="checkbox" checked={selectedCadetIds.includes(mark.cadet_id)} onChange={() => toggleCadet(mark.cadet_id)} className="w-4 h-4" />
                            ) : (
                        ...
                        <td className="border border-black px-2 py-2 text-center">{mark.cadet?.assigned_ranks?.find((ar: any) => ar.rank)?.rank?.short_name || mark.cadet?.assigned_ranks?.[0]?.rank?.name || "OC"}</td>
                        <td className="border border-black px-2 py-2 font-bold uppercase">{mark.cadet?.name || "N/A"}</td>
                        <td className="border border-black px-2 py-2 text-center">{mark.cadet?.assigned_branchs?.[0]?.branch?.short_name || mark.cadet?.assigned_branchs?.[0]?.branch?.name || "—"}</td>
                        {allDetails.map((d: any) => {
                          const v = marksBreakdown[d.id] || 0;
                          return (
                        ...
                        {canApprove && (
                          <td className="border border-black px-2 py-2 no-print">
                            {visibleAuthorities.length === 0 ? (
                              <span className="inline-flex items-center px-1.5 py-0.5 text-[9px] font-bold rounded-full uppercase bg-yellow-100 text-yellow-800">Pending</span>
                            ) : (
                              <div className="flex flex-col gap-1">
                                {visibleAuthorities.map((authority: any, idx: number) => {
                                  ...
                                  const allMatchingRecords = cadetApprovals.filter((a: any) =>
                                    Number(a.cadet_id) === Number(mark.cadet_id) &&
                                  ...
                                  if (allMatchingRecords.length === 0) {
                                    ...
                                    if (authModuleApproval) return (
                                      <div key={authority.id} className="flex items-center gap-1">
                                        ...
                                        <span className={`inline-flex items-center px-1.5 py-0.5 text-[9px] font-bold rounded-full uppercase ${colors[authModuleApproval.status] ?? ""}`}>{authModuleApproval.status} (Mod)</span>
                                      </div>
                                    );
                                  ...
                                  const st = allMatchingRecords[0].status;
                                  return <div key={authority.id} className="flex items-center gap-1"><span className="text-[9px] text-gray-500 font-bold">{label}:</span><span className={`inline-flex items-center px-1.5 py-0.5 text-[9px] font-bold rounded-full uppercase ${colors[st] ?? ""}`}>{st}</span></div>;
                                })}
                              </div>
                            )}
                          </td>
                        ...
                        {canApproveAction && !isForwarded && (
                          <td className="border border-black px-2 py-2 text-center no-print">
                            {isPending ? (
                              <button onClick={() => openApprovalModal([mark.cadet_id])} className="px-2 py-1 text-[10px] font-semibold bg-green-600 text-white rounded hover:bg-green-700">Approve</button>
                            ) : status === "approved" ? (
                              <button onClick={() => openApprovalModal([mark.cadet_id])} className="px-2 py-1 text-[10px] font-semibold bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Change</button>
                            ) : canReapprove ? (
                              <button onClick={() => openApprovalModal([mark.cadet_id])} className="px-2 py-1 text-[10px] font-semibold bg-orange-100 text-orange-700 rounded hover:bg-orange-200">Re-approve</button>
                            ) : isRejected ? (
                  ...
                  <tr className="font-black text-xs">
                    <td colSpan={fixedColCount + allDetails.length} className="border border-black px-2 py-2 text-right uppercase">Total</td>
                    <td className="border border-black px-2 py-2 text-center text-blue-800">
                      {result.achieved_marks.reduce((sum: number, m: any) => sum + parseFloat(String(m.achieved_mark || 0)), 0).toFixed(2)}
                    </td>
                    <td className="border border-black px-2 py-2 text-center text-green-800">
                      {result.achieved_marks.reduce((sum: number, m: any) => {
                        const gd = generatedCadetMarks.get(m.cadet_id);
                        return sum + parseFloat(String(m.achieved_mark || 0)) + (gd?.combined_converted || 0);
                      }, 0).toFixed(2)}
                    </td>
      ...
      <Modal isOpen={approvalModal.open} onClose={() => setApprovalModal(prev => ({ ...prev, open: false }))} showCloseButton className="max-w-lg">
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">Module: <span className="font-medium text-gray-700">GSTO Assessment</span></p>
              ...
              <table className="w-full text-[10px] border-collapse">
                <thead>
                  <tr className="bg-gray-50">
                    ...
                    <th className="border border-gray-200 px-2 py-1 text-center">Total OA</th>
                  </tr>
                </thead>
                <tbody>
                  {approvalModal.cadetIds.map((cadetId, idx) => {
                    ...
                    const totalOA = parseFloat(String(mark.achieved_mark || 0)) + (gd?.combined_converted || 0);
                    return (
                      <tr key={cadetId} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50/30"}>
                        ...
                        <td className="border border-gray-200 px-2 py-1 text-center font-mono">{mark.cadet?.cadet_number || "N/A"}</td>
                        <td className="border border-gray-200 px-2 py-1 font-bold text-gray-900">{mark.cadet?.name || "N/A"}</td>
                        <td className="border border-gray-200 px-2 py-1 text-center font-bold">{totalOA.toFixed(2)}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="flex gap-3 mb-4">
              {(["approved", "rejected"] as const).map((s) => (
                <button key={s} onClick={() => setApprovalModal(prev => ({ ...prev, status: s, rejectedReason: "", error: "" }))}
                  className={`flex-1 py-2 rounded-lg border text-sm font-semibold capitalize transition-colors ${approvalModal.status === s ? (s === "approved" ? "bg-green-600 text-white border-green-600" : "bg-red-600 text-white border-red-600") : "bg-white text-gray-700 border-gray-300 hover:bg-gray-50"}`}>
          ...
          <div className="flex gap-3 justify-end pt-2 border-t border-gray-100 mt-2">
            <button onClick={() => setApprovalModal(prev => ({ ...prev, open: false }))} disabled={approvalModal.loading} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 text-sm hover:bg-gray-50 disabled:opacity-50">Cancel</button>
        ...
        <div className="p-6">
          ...
          {myAuthority && (
            <div className="flex justify-between items-center gap-2 mb-4">
              <div>
                <h3 className="text-sm font-semibold text-gray-900 mb-1">Your Authority Level</h3>
                <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 text-sm font-medium">{(myAuthority as any).role?.name}</span>
              </div>
              <Icon icon="hugeicons:arrow-right-02" className="w-5 h-5 text-blue-600 shrink-0" />
              <div>
                <h3 className="text-sm font-semibold text-gray-900 mb-1">Next Authority Level</h3>
                <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 text-sm font-medium">{getNextAuthority()?.role?.name || "—"}</span>
              </div>
            </div>
          )}

          <p className="text-sm text-gray-600 mb-5">This will mark the module result as forwarded to the higher authority for further review and approval.</p>
          ...
          <div className="flex gap-3 justify-end pt-3 border-t border-gray-100">
            <button onClick={() => setForwardModal(prev => ({ ...prev, open: false }))} disabled={forwardModal.loading} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 text-sm hover:bg-gray-50 disabled:opacity-50">Cancel</button>
      ...
      <Modal isOpen={moduleApprovalModal.open} onClose={() => setModuleApprovalModal(prev => ({ ...prev, open: false }))} showCloseButton className="max-w-lg">
        <div className="p-6">
          ...
          {(["approved", "rejected"] as const).map((s) => (
            <button key={s} onClick={() => setModuleApprovalModal(prev => ({ ...prev, status: s, rejectedReason: "", error: "" }))}
              className={`flex-1 mr-2 py-2 rounded-lg border text-sm font-semibold capitalize transition-colors ${moduleApprovalModal.status === s ? (s === "approved" ? "bg-green-600 text-white border-green-600" : "bg-red-600 text-white border-red-600") : "bg-white text-gray-700 border-gray-300 hover:bg-gray-50"}`}>
              {s === "approved" ? "Approve" : "Reject"}
            </button>
          ))}
          ...
          {moduleApprovalModal.status === "rejected" && (
            <div className="mt-4 mb-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">Reason for Rejection <span className="text-red-500">*</span></label>
              <textarea rows={3} value={moduleApprovalModal.rejectedReason} onChange={(e) => setModuleApprovalModal(prev => ({ ...prev, rejectedReason: e.target.value }))} placeholder="Enter rejection reason..." className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-400 resize-none" />
            </div>
          )}

          <p className="text-sm text-gray-600 my-4">This will approve the module result at your authority level and forward it to the next authority for further review.</p>
          ...
          <div className="flex gap-3 justify-end pt-3 border-t border-gray-100">
            <button onClick={() => setModuleApprovalModal(prev => ({ ...prev, open: false }))} disabled={moduleApprovalModal.loading} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 text-sm hover:bg-gray-50 disabled:opacity-50">Cancel</button>
            <button onClick={confirmModuleApproval} disabled={moduleApprovalModal.loading} className="px-6 py-2 bg-green-600 text-white rounded-lg text-sm font-semibold hover:bg-green-700 flex items-center gap-2 disabled:opacity-50">
      ...
      {/* ── Reject Down Modal ── */}
      <Modal isOpen={rejectDownModal.open} onClose={() => setRejectDownModal(prev => ({ ...prev, open: false }))} showCloseButton className="max-w-md">
        <div className="p-6">
          <h2 className="text-lg font-bold text-gray-900 mb-1 flex items-center gap-2">
            <Icon icon="hugeicons:alert-02" className="w-5 h-5 text-orange-500" /> Reject to Lower Authority
          </h2>
          ...
          <div className="mb-4">
            ...
            <textarea value={rejectDownModal.reason} onChange={e => setRejectDownModal(prev => ({ ...prev, reason: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-orange-400 resize-none" rows={3} placeholder="Enter reason for rejection..." />
          </div>
          {rejectDownModal.error && (
            <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">{rejectDownModal.error}</div>
          )}
          <div className="flex gap-3 justify-end pt-3 border-t border-gray-100">
            <button onClick={() => setRejectDownModal(prev => ({ ...prev, open: false }))} disabled={rejectDownModal.loading} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 text-sm hover:bg-gray-50 disabled:opacity-50">Cancel</button>
            <button onClick={handleRejectDown} disabled={rejectDownModal.loading} className="px-6 py-2 bg-orange-600 text-white rounded-lg text-sm font-semibold hover:bg-orange-700 flex items-center gap-2 disabled:opacity-50">
      ...
      {/* ── Resubmit Modal ── */}
      <Modal isOpen={resubmitModal.open} onClose={() => setResubmitModal(prev => ({ ...prev, open: false }))} showCloseButton className="max-w-md">
        <div className="p-6">
          <h2 className="text-lg font-bold text-gray-900 mb-3 flex items-center gap-2">
            <Icon icon="hugeicons:refresh" className="w-5 h-5 text-green-600" /> Resubmit for Review
          </h2>
          {resubmitModal.item && (
            <p className="text-sm text-gray-600 mb-4">
              Resubmit cadet <span className="font-medium text-gray-900">{resubmitModal.item.cadet_name}</span> for re-review after correction?
            </p>
          )}
          {resubmitModal.error && (
            <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">{resubmitModal.error}</div>
          )}
          <div className="flex gap-3 justify-end pt-3 border-t border-gray-100">
            <button onClick={() => setResubmitModal(prev => ({ ...prev, open: false }))} disabled={resubmitModal.loading} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 text-sm hover:bg-gray-50 disabled:opacity-50">Cancel</button>
            <button onClick={handleConfirmResubmit} disabled={resubmitModal.loading} className="px-6 py-2 bg-green-600 text-white rounded-lg text-sm font-semibold hover:bg-green-700 flex items-center gap-2 disabled:opacity-50">
      ...
      {/* ── View Rejected Modal ── */}
      <Modal isOpen={viewRejectedModal.open} onClose={() => setViewRejectedModal({ open: false, item: null })} showCloseButton className="max-w-md">
        <div className="p-6">
          <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
            <Icon icon="hugeicons:view" className="w-5 h-5 text-orange-500" /> Rejection Details
          </h2>
          {viewRejectedModal.item && (
            <div className="space-y-3 text-sm">
              <div className="flex"><span className="w-32 text-gray-500 shrink-0">Cadet</span><span className="font-medium text-gray-900">{viewRejectedModal.item.cadet_name}</span></div>
              <div className="flex"><span className="w-32 text-gray-500 shrink-0">BD No</span><span className="font-medium text-gray-900">{viewRejectedModal.item.cadet_bd_no}</span></div>
              <div className="flex"><span className="w-32 text-gray-500 shrink-0">Rejected By</span><span className="font-medium text-red-700">{viewRejectedModal.item.rejected_by}</span></div>
              <div className="flex"><span className="w-32 text-gray-500 shrink-0">Status</span><span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${viewRejectedModal.item.state === "updated_pending_review" ? "bg-blue-100 text-blue-800" : "bg-red-100 text-red-800"}`}>{viewRejectedModal.item.message}</span></div>
              <div>
                <p className="text-gray-500 mb-1">Rejection Reason:</p>
                <p className="bg-red-50 border border-red-200 rounded-lg p-3 text-red-800">{viewRejectedModal.item.rejected_reason || "No reason provided."}</p>
              </div>
            </div>
          )}
          <div className="flex justify-end pt-4 border-t border-gray-100 mt-4">
            <button onClick={() => setViewRejectedModal({ open: false, item: null })} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 text-sm hover:bg-gray-50">Close</button>
          </div>
        </div>
      </Modal>
    </div>
