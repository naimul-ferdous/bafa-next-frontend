...
export default function CtwGstoAssessmentResultsPage() {
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await ctwGstoAssessmentResultService.getAllResults(moduleId, {
        page: currentPage,
        per_page: perPage,
  ...
  const loadOfficialResults = useCallback(async () => {
    ...
    try {
      ...
      const data = await ctwGstoAssessmentResultService.getGroupedResults({
  ...
  const handleAddResult = () => router.push("/ctw/results/assessment_observation/gsto/create");
  const handleEditResult = (result: any) => router.push(`/ctw/results/assessment_observation/gsto/${result.id}/edit`);
  const handleViewResult = (result: any) => router.push(`/ctw/results/assessment_observation/gsto/${result.id}`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await ctwGstoAssessmentResultService.deleteResult(deletingResult.id);
      await loadResults();
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex flex-col items-center gap-1">
            <button onClick={() => router.push(`/ctw/results/assessment_observation/gsto/course/${row.course_details.id}/semester/${row.semester_details.id}`)} className="p-1 text-blue-600 hover:bg-blue-50 rounded flex items-center gap-1 text-xs" title="View Submission">
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 space-y-6">
      <div className="text-center mb-8">
        ...
        <h2 className="text-md font-semibold text-gray-700 mt-2 uppercase">CTW GSTO Assessment Observations</h2>
      </div>
