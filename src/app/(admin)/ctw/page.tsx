...
import type { CtwAdminStats } from "@/libs/services/ctwDashboardService";
import type { CtwInstructorStats, CtwInstructorSubjectProgress } from "@/libs/services/ctwInstructorStatsService";
import type { CtwCombinedViewShortData } from "@/libs/services/ctwCombinedViewShortService";
import activityLogService, { ActivityLogEntry } from "@/libs/services/activityLogService";
import ctwDashboardPageService, { AuthorityModuleStatus, AuthorityModuleRow } from "@/libs/services/ctwDashboardPageService";
...
const DonutProgressChart = ({ approved, entered, notEntered, total, waiting, pending }: DonutProgressChartProps) => {
  ...
  const options: ApexOptions = {
    ...
    tooltip: { y: { formatter: (v: number) => `${v} module(s)` } },
  };
...
// ─── All CTW module cards config ──────────────────────────────────────────────
const ALL_MODULE_CARDS: (StatusCardProps & { href: string })[] = [
  { title: "Modules", subtitle: "Training Setup", description: "Manage training modules and their configurations.", href: "/ctw/modules", icon: "hugeicons:layers-01", accentColor: "from-blue-500 to-indigo-600", bgImage: "corner-1.png" },
  { title: "Marksheets", subtitle: "Mark Schema", description: "Define mark schemas, grading criteria and weights.", href: "/ctw/modules/marksheets", icon: "hugeicons:notebook", accentColor: "from-violet-500 to-purple-700", bgImage: "corner-2.png" },
  { title: "OLQ Assessment", subtitle: "Officer Qualities", description: "Record and review officer-like qualities for cadets.", href: "/ctw/assessments/olq/results", icon: "hugeicons:star", accentColor: "from-amber-400 to-orange-500", bgImage: "corner-4.png" },
  { title: "Counseling", subtitle: "Cadet Support", description: "Log and track cadet counseling sessions.", href: "/ctw/assessments/counselings/results", icon: "hugeicons:user-multiple", accentColor: "from-sky-500 to-blue-600", bgImage: "corner-5.png" },
  { title: "Pen Picture", subtitle: "Character Logs", description: "Document character assessments and behaviour records.", href: "/ctw/assessments/penpicture/results", icon: "hugeicons:edit-02", accentColor: "from-rose-500 to-pink-600", bgImage: "corner-1.png" },
  { title: "Warnings", subtitle: "Cadet Warnings", description: "Issue, review and track formal cadet warnings.", href: "/ctw/assessments/warnings", icon: "hugeicons:alert-02", accentColor: "from-red-500 to-rose-600", bgImage: "corner-2.png" },
  { title: "Medical Disposal", subtitle: "Health Evaluation", description: "Record health evaluations and medical disposal outcomes.", href: "/ctw/assessments/medical-disposal/disposal", icon: "hugeicons:stethoscope", accentColor: "from-teal-500 to-cyan-600", bgImage: "corner-4.png" },
  { title: "Instructor Detailment", subtitle: "Detailment Board", description: "Manage and assign instructor detailment schedules.", href: "/ctw/instructor-detailment", icon: "hugeicons:teacher", accentColor: "from-slate-500 to-slate-700", bgImage: "corner-5.png" },
  { title: "Approval Authorities", subtitle: "Approval Setup", description: "Configure authorities and approval workflows.", href: "/ctw/results/authorities", icon: "hugeicons:shield-user", accentColor: "from-slate-600 to-slate-800", bgImage: "corner-1.png" },
];
...
export function CTWDashboardView() {
  ...
  const canOlq = useCan("/ctw/assessments/olq/results");
  const canCounseling = useCan("/ctw/assessments/counselings/results");
  const canResults = useCan("/ctw/results");

  const [adminStats, setAdminStats] = useState<CtwAdminStats | null>(null);
  const [instructorStats, setInstructorStats] = useState<CtwInstructorStats | null>(null);
  const [moduleProgress, setModuleProgress] = useState<CtwInstructorSubjectProgress | null>(null);
  const [combinedShort, setCombinedShort] = useState<CtwCombinedViewShortData | null>(null);
  const [olqCombinedShort, setOlqCombinedShort] = useState<CtwCombinedViewShortData | null>(null);
  const [counselingCombinedShort, setCounselingCombinedShort] = useState<CtwCombinedViewShortData | null>(null);
  ...
  const [assignedModules, setAssignedModules] = useState<CtwInstructorAssignModule[]>([]);
  const [authorityModuleStatus, setAuthorityModuleStatus] = useState<AuthorityModuleStatus | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await ctwDashboardPageService.get({
          chartView,
          userId: user?.id,
        ...
        if (!cancelled) {
          ...
          if (data.module_progress) setModuleProgress(data.module_progress);
          if (data.assigned_modules) setAssignedModules(data.assigned_modules);
          if (data.authority_module_status) setAuthorityModuleStatus(data.authority_module_status);
          setLoadSuccess(true);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3">
          <div className="lg:col-span-2 p-6 sm:p-7">
            ...
            <p className="text-sm text-slate-500 font-medium mb-6">
              {isAdmin
                ? "Here's your CTW system overview. Keep the training pipeline running strong!"
              ...
              {isAdmin ? (
                <>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Modules</div>
                      <div className="text-lg font-black text-teal-500 leading-none">{loading ? "—" : adminStats?.modules ?? 0}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                ...
                <>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Modules</div>
                      <div className="text-lg font-black text-teal-500 leading-none">{loading ? "—" : userIsInstructor ? (instructorStats?.total_courses ?? 0) : (authorityModuleStatus?.total_modules ?? adminStats?.assigned_subjects ?? 0)}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-black text-teal-500 leading-none">{loading ? "—" : userIsInstructor ? (instructorStats?.total_cadets ?? 0) : (authorityModuleStatus?.total_cadets ?? adminStats?.cadets_in_results ?? 0)}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center shrink-0">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-black text-teal-500 leading-none">
                        {loading ? "—" : userIsInstructor ? (instructorStats?.total_results ?? 0) : (
                          authorityModuleStatus ? (
                            authorityModuleStatus.my_authority 
                              ? `${authorityModuleStatus.summary.approved}/${authorityModuleStatus.total_modules}`
                              : `${authorityModuleStatus.total_modules - authorityModuleStatus.summary.not_entered}/${authorityModuleStatus.total_modules}`
      ...
      {!isAdmin && !userIsInstructor && (
        <div className="space-y-4">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard icon="hugeicons:layers-01" iconBg="from-blue-500 to-indigo-600" bgImage="corner-1.png" title="Total Modules" value={(authorityModuleStatus?.total_modules ?? 0).toLocaleString()} trend={4.9} trendUp href="/ctw/modules" />
              <StatCard icon="hugeicons:user-multiple" iconBg="from-violet-500 to-purple-700" bgImage="corner-2.png" title="Total Cadets" value={(authorityModuleStatus?.total_cadets ?? 0).toLocaleString()} trend={2.7} trendUp href="/users/cadets" />
              <StatCard
                icon="hugeicons:layers-01"
                iconBg="from-indigo-500 to-violet-600"
                bgImage="corner-4.png"
                title="Running Courses"
                value={(authorityModuleStatus?.total_running_courses ?? 0).toLocaleString()}
                trend={3.4}
                trendUp
                href="/ctw/course"
              />
              <StatCard
                ...
                title={authorityModuleStatus?.my_authority ? "Approved Modules" : "Inputted Modules"}
                value={
                  authorityModuleStatus?.my_authority
                    ? `${authorityModuleStatus?.summary.approved ?? 0}/${authorityModuleStatus?.total_modules ?? 0}`
                    : `${authorityModuleStatus ? authorityModuleStatus.total_modules - authorityModuleStatus.summary.not_entered : 0}/${authorityModuleStatus?.total_modules ?? 0}`
                }
                trend={1.8}
                trendUp
                href="/ctw/results/view"
              />
            </div>
          )}
                ...
                {!loading && canResults("view") && cvRows.length > 0 && (
                  <div className="bg-white rounded-[1.5rem] border border-slate-100 shadow-sm p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <Link href="/ctw/results/view" className="text-[10px] font-semibold text-blue-500 hover:underline">View All</Link>
                    </div>
                    <div className="flex flex-col divide-y space-y-2 divide-slate-100">
                      {cvRows.map((row) => {
                        ...
                        return (
                          <Link href={'/ctw/results/view'} key={`${row.course_id}-${row.semester_id}-${row.program_id}`} className="flex justify-between items-center gap-3 py-3 px-3 border border-gray-200 rounded-xl hover:bg-slate-50/60 transition-colors">
                  ...
                  {!loading && canOlq("view") && olqCvRows.length > 0 && (
                    <div className="bg-white rounded-[1.5rem] border border-slate-100 shadow-sm p-5">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <Link href="/ctw/assessments/olq/results/view" className="text-[10px] font-semibold text-blue-500 hover:underline">View All</Link>
                      </div>
                      <div className="flex flex-col divide-y space-y-2 divide-slate-100">
                        {olqCvRows.map((row, index) => {
                          ...
                          return (
                            <Link key={`olq-${row.course_id}-${row.semester_id}-${index}`} href={`/ctw/assessments/olq/results/course/${row.course_id}/semester/${row.semester_id}`} className="flex justify-between items-center gap-3 p-3 border border-gray-200 rounded-xl hover:bg-slate-50/60 transition-colors cursor-pointer">
                              <div className="min-w-0">
                  ...
                  {!loading && canCounseling("view") && (
                    <div className="bg-white rounded-[1.5rem] border border-slate-100 shadow-sm p-5">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <Link href="/ctw/assessments/counselings/results/view" className="text-[10px] font-semibold text-blue-500 hover:underline">View All</Link>
                      </div>
                      <div className="flex flex-col divide-y space-y-2 divide-slate-100">
                        ...
                        {counselingCvRows.map((row, index) => {
                          ...
                          return (
                            <Link key={`counseling-${row.course_id}-${row.semester_id}-${index}`} href={`/ctw/assessments/counselings/results/course/${row.course_id}/semester/${row.semester_id}`} className="flex justify-between items-center border border-gray-200 gap-3 p-3 rounded-xl hover:bg-slate-50/60 transition-colors cursor-pointer">
                              <div className="min-w-0">
                                <div className="text-sm font-bold text-slate-800 truncate">{row.course_name}</div>
                                <div className="text-xs text-slate-400 mt-0.5 truncate">{row.semester_name}</div>
                              </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              {/* Right — Module Result Overview */}
              <div className="bg-white rounded-[1.5rem] sm:rounded-[2rem] shadow-sm border border-slate-100 p-6 flex flex-col">
                <h2 className="text-base font-bold text-slate-800 mb-4">Module Result Overview</h2>
                {authorityModuleStatus ? (() => {
                  const approved = authorityModuleStatus.summary.approved;
                  const pending = authorityModuleStatus.summary.pending;
                  const waiting = authorityModuleStatus.summary.waiting;
                  const notEntered = authorityModuleStatus.summary.not_entered;
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-slate-400 gap-2">
                    <Icon icon="hugeicons:layers-01" className="w-8 h-8" />
      ...
      {!isAdmin && userIsInstructor && (
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard icon="hugeicons:layers-01" iconBg="from-blue-500 to-indigo-600" bgImage="corner-1.png" title="Total Modules" value={instructorStats?.total_courses ?? 0} trend={4.9} trendUp href="/ctw/modules" />
              <StatCard icon="hugeicons:layers-01" iconBg="from-indigo-500 to-violet-600" bgImage="corner-2.png" title="Courses" value={instructorStats?.total_courses ?? 0} trend={2.7} trendUp href="/ctw/course" />
              ...
              <StatCard icon="hugeicons:chart-bar-line" iconBg="from-emerald-500 to-teal-600" bgImage="corner-5.png" title="Results" value={`${instructorStats?.total_results ?? 0}/${instructorStats?.total_courses ?? 0}`} trend={1.8} trendUp href="/ctw/results/view" />
            </div>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard icon="hugeicons:layers-01" iconBg="from-indigo-500 to-violet-600" bgImage="corner-2.png" title="Courses" value={adminStats?.courses ?? 0} trend={2.7} trendUp href="/ctw/course" />
              <StatCard icon="hugeicons:layers-01" iconBg="from-blue-500 to-indigo-600" bgImage="corner-1.png" title="Modules" value={adminStats?.assigned_subjects ?? 0} trend={4.9} trendUp href="/ctw/modules" />
              ...
              <StatCard icon="hugeicons:chart-bar-line" iconBg="from-emerald-500 to-teal-600" bgImage="corner-5.png" title="Results" value={adminStats?.results ?? 0} trend={1.8} trendUp href="/ctw/results/view" />
            </div>
          )}
          ...
          {!loading && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="bg-white rounded-[1.5rem] sm:rounded-[2rem] shadow-sm border border-slate-100 p-6 flex flex-col">
                ...
                {userIsInstructor ? (() => {
                  const approved = moduleProgress?.total_approved ?? 0;
                  const enteredNotApproved = moduleProgress?.total_entered_not_approved ?? 0;
                  const notEntered = moduleProgress?.total_not_entered ?? (assignedModules.length);
                  const totalAssigned = moduleProgress?.total_assigned ?? assignedModules.length;
            ...
            <div className={`grid grid-cols-1 ${userIsInstructor ? "xl:grid-cols-4" : ""} gap-4 items-start`}>
              ...
              {userIsInstructor && (
                <div className="xl:col-span-3 bg-white shadow-sm rounded-[1.5rem] p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <div className="font-bold text-slate-800">Assigned Modules</div>
                    </div>
                    <Link href="/ctw/modules" className="text-sm font-semibold text-blue-500 hover:underline">View All</Link>
                  </div>
                  {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-10 text-slate-400 gap-2">
                      <Icon icon="hugeicons:layers-01" className="w-8 h-8" />
                      <p className="text-xs font-medium">No modules assigned yet.</p>
                    </div>
                  ) : (
                      ...
                      <table className="w-full border-collapse border border-gray-200">
                        <thead>
                          <tr>
                            ...
                            <th className="px-3 py-2 text-left font-semibold text-slate-500 border border-gray-200">Module</th>
                        ...
                        <tbody>
                          {assignedModules.map((row, index) => {
                            const isFirstCourse = index === 0 || assignedModules[index - 1].course_id !== row.course_id;
                            let courseSpan = 0;
                            if (isFirstCourse) {
                              for (let i = index; i < assignedModules.length; i++) {
                                if (assignedModules[i].course_id === row.course_id) courseSpan++;
                                else break;
                              }
                            }
                            const isFirstSemester = index === 0 || assignedModules[index - 1].course_id !== row.course_id || assignedModules[index - 1].semester_id !== row.semester_id;
                            let semSpan = 0;
                            if (isFirstSemester) {
                              for (let i = index; i < assignedModules.length; i++) {
                                if (assignedModules[i].course_id === row.course_id && assignedModules[i].semester_id === row.semester_id) semSpan++;
                                else break;
                              }
                            }
                            const isFirstProgram = index === 0 || assignedModules[index - 1].course_id !== row.course_id || assignedModules[index - 1].semester_id !== row.semester_id || assignedModules[index - 1].program_id !== row.program_id;
                            let progSpan = 0;
                            if (isFirstProgram) {
                              for (let i = index; i < assignedModules.length; i++) {
                                if (assignedModules[i].course_id === row.course_id && assignedModules[i].semester_id === row.semester_id && assignedModules[i].program_id === row.program_id) progSpan++;
                                else break;
                              }
                            }
                            return (
                              <tr key={`as-${row.id}`} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-3 py-2 text-slate-700 border border-gray-200">{row.module?.name ?? row.ctw_results_module_id}</td>
                                <td className="px-3 py-2 text-center text-slate-600 border border-gray-200">{row.module?.period ?? "—"}</td>
                                <td className="px-3 py-2 text-center font-semibold text-slate-800 border border-gray-200">{row.module?.full_mark ?? "—"}</td>
                              </tr>
                            );
                          })}
                        </tbody>
